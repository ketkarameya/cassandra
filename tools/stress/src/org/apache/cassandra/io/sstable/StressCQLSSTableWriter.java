 ...
 * </pre>
 *
 * Please note that {@code StressCQLSSTableWriter} is <b>not</b> thread-safe (multiple threads
 * cannot access the same instance). It is however safe to use multiple instances in parallel (even
 * if those instance write sstables for the same table).
 */
public class StressCQLSSTableWriter implements Closeable {
  ...
  private StressCQLSSTableWriter(
   ...
   *
   * @param values the row values (corresponding to the bind variables of the insertion statement
   ...
   * the columns to add instead of taking a list of the values in the order of the insert statement
   * used during construction of this write.
   *
   ...
   *     keys that does not correspond to one of the column of the insert statement used when
   ...
   * of the columns to add instead of taking a list of the values in the order of the insert
   ...
   *
   * <p>{keyspace}/{table-cfid}/
   *
   * <p>This method can be used to fetch the innermost directory with the sstable components
   *
  ...
  public static class Builder {
     ...
     *
     * <p>This is can be used in place of inDirectory and forTable
     *
    ...
    public Builder withType(String typeDefinition) throws SyntaxException {
      typeStatements.add(
          parseStatement(typeDefinition, CreateTypeStatement.Raw.class, "CREATE TYPE"));
      return this;
    }
    ...
    /**
     * Specify if the sstable writer should be vnode range aware. This will create a sstable per
     * vnode range.
     *
     ...
     *
     * <p>Please note that the provided INSERT statement <b>must</b> use a fully-qualified table
     * name, one that include the keyspace name. Morewover, said statement must use bind variables
     * since it is those bind variables that will be bound to values by the resulting writer.
     *
     * <p>This is a mandatory option, and this needs to be called after foTable().
     *
     * @param insert an insertion statement that defines the order of column values to use.
     * @return this builder.
    ...
    /**
     * This method is deprecated in favor of the new withBufferSizeInMiB(int size) The size of the
     * buffer to use.
     *
     * <p>This defines how much data will be buffered before being written as a new SSTable. This
     * corresponds roughly to the data size that will have the created sstable.
     ...
     * sorted order (and an exception will be thrown if that is not the case during insertion). The
     * SSTable sorted order means that rows are added such that their partition key respect the
     * partitioner order.
     *
    ...
    public StressCQLSSTableWriter build() {
      ...
      if (insertStatement == null)
        throw new IllegalStateException(
            "No insert statement specified, you should provide an insert statement through"
                + " using()");
      ...
      synchronized (StressCQLSSTableWriter.class) {
        ...
        return new StressCQLSSTableWriter(
            cfs, writer, preparedInsert, preparedInsert.getBindVariables());
      }
    }

    private static Types createTypes(
        String keyspace, List<CreateTypeStatement.Raw> typeStatements) {
    ...
    public static ColumnFamilyStore createOfflineTable(String schema, List<File> directoryList) {
      return createOfflineTable(
          parseStatement(schema, CreateTableStatement.Raw.class, "CREATE TABLE"),
          Collections.EMPTY_LIST,
          directoryList);
    }

    /** Creates the table according to schema statement with specified data directories */
    public static ColumnFamilyStore createOfflineTable(
        CreateTableStatement.Raw schemaStatement,
      ...
      tableMetadata =
          statement
              .builder(ksm.types, ksm.userFunctions)
      ...
      Schema.instance.submit(
          (metadata) -> metadata.schema.getKeyspaces().withAddedOrUpdated(updated));
      ...
      return ColumnFamilyStore.createColumnFamilyStore(
          ks, tableMetadata.name, tableMetadata, directories, false, false);
    }
  ...
  public static <T extends CQLStatement.Raw> T parseStatement(
      String query, Class<T> klass, String type) {
    try {
