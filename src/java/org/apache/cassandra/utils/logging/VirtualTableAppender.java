...
public final class VirtualTableAppender extends AppenderBase<LoggingEvent> {
  ...
  protected void append(LoggingEvent eventObject) {
    if (!forbiddenLoggers.contains(eventObject.getLoggerName())) {
      if (logs == null) {
        ...
        if (logs == null) addToBuffer(eventObject);
        else logs.add(eventObject);
      } else logs.add(eventObject);
    }
  }
  ...
  public void flushBuffer() {
    Optional.ofNullable(getVirtualTable())
        .ifPresent(
            vtable -> {
  ...
  private void addToBuffer(LoggingEvent eventObject) {
    ...
    if (messageBuffer.size() < LOGS_VIRTUAL_TABLE_DEFAULT_ROWS) messageBuffer.add(eventObject);
  }
}
