...
/**
 * Writes leaf postings and auxiliary posting lists for bbtree nodes. If a node has a posting list
 * attached, it will contain every row id from all leaves reachable from that node.
 *
 * <p>Writer is stateful, because it needs to collect data from the balanced tree data structure
 * first to find set of eligible nodes and leaf nodes reachable from them.
 *
 * <p>The leaf blocks are written in value order (in the order we pass them to the {@link
 * BlockBalancedTreeWriter}). This allows us to skip reading the leaves, instead just order leaf
 * blocks by their offset in the index file, and correlate them with buffered posting lists.
 */
@NotThreadSafe
public class BlockBalancedTreePostingsWriter implements BlockBalancedTreeWalker.TraversalCallback {
  ...
  /**
   * Minimum number of reachable leaves for a given node to be eligible for an auxiliary posting
   * list.
   */
  private final int minimumPostingsLeaves;

  /**
   * Skip, or the sampling interval, for selecting a balanced tree level that is eligible for an
   * auxiliary posting list. Sampling starts from 0, but the balanced tree root node is at level 1.
   * For skip = 4, eligible levels are 4, 8, 12, etc. (no level 0, because there is no node at level
   * 0).
   */
   ...
   * @param pathToRoot the path to the root leaf above this leaf. Contains all the intermediate leaf
   *     node IDs.
   */
  @Override
  ...
  /**
   * Writes merged posting lists for eligible internal nodes and leaf postings for each leaf in the
   * tree. The merged postings list for an internal node contains all postings from the postings
   * lists of leaf nodes in the subtree rooted at that node.
   *
   * <p>After writing out the postings, it writes a map of node ID -> postings file pointer for all
   * nodes with an attached postings list. It then returns the file pointer to this map.
   */
  public long finish(
      IndexOutputWriter out, List<PackedLongValues> leafPostings, IndexIdentifier indexIdentifier)
      throws IOException {
    checkState(
        leafPostings.size() == leafOffsetToNodeID.size(),
        "Expected equal number of postings lists (%s) and leaf offsets (%s).",
        leafPostings.size(),
        leafOffsetToNodeID.size());
    ...
    try (PostingsWriter postingsWriter = new PostingsWriter(out)) {
      ...
      leafOffsetToNodeID.forEach(
          (fp, nodeID) -> leafToPostings.put(nodeID, postingsIterator.next()));

      long postingsRamBytesUsed =
          leafPostings.stream().mapToLong(PackedLongValues::ramBytesUsed).sum();
      ...
      logger.debug(
          indexIdentifier.logMessage(
              "Writing posting lists for {} internal and {} leaf balanced tree nodes. Leaf postings"
                  + " memory usage: {}."),
      ...
      PriorityQueue<PeekablePostingList> postingLists =
          new PriorityQueue<>(
              minimumPostingsLeaves, Comparator.comparingLong(PeekablePostingList::peek));
      for (int nodeID : Iterables.concat(internalNodeIDs, leafNodeIDs)) {
        ...
        for (Integer leaf : leaves)
          postingLists.add(
              PeekablePostingList.makePeekable(
                  new PackedLongsPostingList(leafToPostings.get(leaf))));
      ...
      logger.debug(
          indexIdentifier.logMessage(
              "Flushed {} of posting lists for balanced tree nodes in {} ms."),
