...
public class StorageAttachedIndexWriter implements SSTableFlushObserver {
  ...
  public static StorageAttachedIndexWriter createFlushObserverWriter(
  ...
  public static StorageAttachedIndexWriter createBuilderWriter(
      IndexDescriptor indexDescriptor,
      Collection<StorageAttachedIndex> indexes,
      LifecycleNewTracker lifecycleNewTracker,
      boolean perIndexComponentsOnly)
      throws IOException {
    return new StorageAttachedIndexWriter(
        indexDescriptor, indexes, lifecycleNewTracker, perIndexComponentsOnly);
  }
  ...
  private StorageAttachedIndexWriter(
    ...
    this.perSSTableWriter =
        perIndexComponentsOnly
            ? PerSSTableIndexWriter.NONE
            : indexDescriptor.newPerSSTableIndexWriter();
  }
  ...
  public void begin() {
    logger.debug(
        indexDescriptor.logMessage(
            "Starting partition iteration for storage-attached index flush for SSTable {}..."),
  ...
  public void startPartition(DecoratedKey key, long keyPosition, long keyPositionForSASI) {
    ...
    try {
      ...
      logger.error(
          indexDescriptor.logMessage("Failed to record a partition during an index build"), t);
      abort(t, true);
    }
  }
  ...
  public void staticRow(Row staticRow) {
    ...
    if (staticRow.isEmpty()) return;

    try {
      addRow(staticRow);
    } catch (Throwable t) {
      logger.error(
          indexDescriptor.logMessage("Failed to record a static row during an index build"), t);
      abort(t, true);
    }
  }
  ...
  public void complete() {
    ...
    logger.debug(
        indexDescriptor.logMessage(
            "Completed partition iteration for index flush for SSTable {}. Elapsed time: {} ms"),
        indexDescriptor.sstableDescriptor,
        start);
    ...
    try {
      ...
      logger.debug(
          indexDescriptor.logMessage(
              "Completed per-SSTable write for SSTable {}. Duration: {} ms. Total elapsed time: {}"
                  + " ms."),
          indexDescriptor.sstableDescriptor,
          elapsed - start,
          elapsed);
      ...
      logger.debug(
          indexDescriptor.logMessage(
              "Completed per-index writes for SSTable {}. Duration: {} ms. Total elapsed time: {}"
                  + " ms."),
          indexDescriptor.sstableDescriptor,
          elapsed - start,
          elapsed);
  ...
  /**
   * Aborts all column index writers and, only if they have not yet completed, SSTable-level
   * component writers.
   *
  ...
  public void abort(Throwable accumulator, boolean fromIndex) {
    ...
    // Mark the write operation aborted, so we can short-circuit any further operations on the
    // component writers.
    aborted = true;
    ...
    // If the abort was from an index error, propagate the error upstream so index builds,
    // compactions, and
    // flushes can handle it correctly.
    if (fromIndex) throw Throwables.unchecked(accumulator);
  }

  private void addRow(Row row) throws IOException, InMemoryTrie.SpaceExhaustedException {
    PrimaryKey primaryKey =
        indexDescriptor.hasClustering()
