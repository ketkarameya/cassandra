...
public class OnHeapGraph<T> {
   ...
   *     <p>Will create a concurrent object.
   */
   ...
   * @param concurrent should be true for memtables, false for compaction. Concurrent allows us to
   *     search while building the graph; non-concurrent allows us to avoid synchronization costs.
   */
  @SuppressWarnings("unchecked")
  public OnHeapGraph(
      AbstractType<?> termComparator, IndexWriterConfig indexWriterConfig, boolean concurrent) {
    ...
    // is thus a better option than hash-based (which has to look at all elements to compute the
    // hash).
  ...
  public long add(ByteBuffer term, T key, InvalidVectorBehavior behavior) {
    ...
    if (behavior == InvalidVectorBehavior.IGNORE) {
      try {
        ...
        logger.trace(
            "Ignoring invalid vector during index build against existing data: {}", vector, e);
        return 0;
      }
    } else {
    ...
    if (postings == null) {
      ...
      if (postingsMap.putIfAbsent(vector, postings) == null) {
        // we won the race to add the new entry; assign it an ordinal and add to the other
        // structures
        ...
        bytesUsed +=
            (vectorValues instanceof ConcurrentVectorValues)
  ...
  public static void validateIndexable(
      float[] vector, VectorSimilarityFunction similarityFunction) {
    try {
  ...
  public PriorityQueue<T> search(float[] queryVector, int limit, Bits toAccept) {
    ...
    if (vectorValues.size() == 0) return new PriorityQueue<>();
    ...
    NeighborSimilarity.ExactScoreFunction scoreFunction =
        node2 -> vectorCompareFunction(queryVector, node2);
    var result = searcher.search(scoreFunction, null, limit, bits);
    Tracing.trace(
        "ANN search visited {} in-memory nodes to return {} results",
        result.getVisitedCount(),
        result.getNodes().length);
    var a = result.getNodes();
    PriorityQueue<T> keyQueue = new PriorityQueue<>();
    for (int i = 0; i < a.length; i++) keyQueue.addAll(keysFromOrdinal(a[i].node));
    return keyQueue;
  }

  public SegmentMetadata.ComponentMetadataMap writeData(
      IndexDescriptor indexDescriptor,
      IndexIdentifier indexIdentifier,
      Function<T, Integer> postingTransformer)
      throws IOException {
    int nInProgress = builder.insertsInProgress();
    assert nInProgress == 0
        : String.format("Attempting to write graph while %d inserts are in progress", nInProgress);
    assert nextOrdinal.get() == builder.getGraph().size()
        : String.format(
            "nextOrdinal %d != graph size %d -- ordinals should be sequential",
            nextOrdinal.get(), builder.getGraph().size());
    assert vectorValues.size() == builder.getGraph().size()
        : String.format(
            "vector count %d != graph size %d", vectorValues.size(), builder.getGraph().size());
    assert postingsMap.keySet().size() == vectorValues.size()
        : String.format(
            "postings map entry count %d != vector count %d",
            postingsMap.keySet().size(), vectorValues.size());
    logger.debug(
        "Writing graph with {} rows and {} distinct vectors",
        postingsMap.values().stream().mapToInt(VectorPostings::size).sum(),
        vectorValues.size());

    try (var pqOutput =
            IndexFileUtils.instance.openOutput(
                indexDescriptor.fileFor(IndexComponent.COMPRESSED_VECTORS, indexIdentifier), true);
        var postingsOutput =
            IndexFileUtils.instance.openOutput(
                indexDescriptor.fileFor(IndexComponent.POSTING_LISTS, indexIdentifier), true);
        var indexOutput =
            IndexFileUtils.instance.openOutput(
                indexDescriptor.fileFor(IndexComponent.TERMS_DATA, indexIdentifier), true)) {
      ...
      long postingsPosition =
          new VectorPostingsWriter<T>()
              .writePostings(
                  postingsOutput.asSequentialWriter(), vectorValues, postingsMap, deletedOrdinals);
      ...
      Map<String, String> vectorConfigs =
          Map.of("SEGMENT_ID", ByteBufferUtil.bytesToHex(ByteBuffer.wrap(StringHelper.randomId())));
  ...
  private float vectorCompareFunction(float[] queryVector, int node) {
    return similarityFunction.compare(
        queryVector, ((RandomAccessVectorValues<float[]>) vectorValues).vectorValue(node));
  }
  ...
  private long writePQ(SequentialWriter writer) throws IOException {
    ...
    synchronized (OnHeapGraph.class) {
      ...
      encoded =
          IntStream.range(0, vectorValues.size())
              .parallel()
