...
public class QueryController {
  ...
  public QueryController(
    ...
    this.mergeRange =
        ranges.size() == 1
            ? first.keyRange()
            : first.keyRange().withNewRight(last.keyRange().right);
  ...
  public UnfilteredRowIterator queryStorage(
      PrimaryKey key, ReadExecutionController executionController) {
    if (key == null) throw new IllegalArgumentException("non-null key required");
   ...
   *
   * <p>This is achieved by creating an on-disk view of the query that maps the expressions to the
   * {@link SSTableIndex}s that will satisfy the expression.
   *
   * <p>Each (expression, SSTable indexes) pair is then passed to {@link
   * IndexSearchResultIterator#build(Expression, Collection, AbstractBounds, QueryContext, boolean,
   * Runnable)} to search the in-memory index associated with the expression and the SSTable
   * indexes, the results of which are unioned and returned.
   *
   * <p>The results from each call to {@link IndexSearchResultIterator#build(Expression, Collection,
   * AbstractBounds, QueryContext, boolean, Runnable)} are added to a {@link
   * KeyRangeIntersectionIterator} and returned if strict filtering is allowed.
   *
   * <p>If strict filtering is not allowed, indexes are split into two groups according to the
   * repaired status of their backing SSTables. Results from searches over the repaired group are
   * added to a {@link KeyRangeIntersectionIterator}, which is then added, along with results from
   * searches on the unrepaired set, to a top-level {@link KeyRangeUnionIterator}, and returned.
   * This is done to ensure that AND queries do not prematurely filter out matches on un-repaired
   * partial updates. Post-filtering must also take this into account. (see {@link
   * FilterTree#isSatisfiedBy(DecoratedKey, Row, Row)}) Note that Memtable-attached indexes are
   * treated as part of the unrepaired set.
   */
  public KeyRangeIterator.Builder getIndexQueryResults(Collection<Expression> expressions) {
    // VSTODO move ANN out of expressions and into its own abstraction? That will help get generic
    // ORDER BY support
    ...
    try {
      ...
      if (command.rowFilter().isStrict()) {
        // If strict filtering is enabled, evaluate indexes for both repaired and un-repaired
        // SSTables together.
        // This usually means we are making this local index query in the context of a user query
        // that reads
        ...
        for (Pair<Expression, Collection<SSTableIndex>> queryViewPair : queryView.view)
          builder.add(
              IndexSearchResultIterator.build(
                  queryViewPair.left,
                  queryViewPair.right,
                  mergeRange,
                  queryContext,
        ...
        for (Pair<Expression, Collection<SSTableIndex>> queryViewPair : queryView.view) {
          ...
          for (SSTableIndex index : queryViewPair.right)
            if (index.getSSTable().isRepaired()) repaired.add(index);
            else unrepaired.add(index);

          // Always build an iterator for the un-repaired set, given this must include Memtable
          // indexes...
          IndexSearchResultIterator unrepairedIterator =
              IndexSearchResultIterator.build(
                  queryViewPair.left, unrepaired, mergeRange, queryContext, true, () -> {});
          ...
          // ...then only add an iterator to the repaired intersection if repaired SSTable indexes
          // exist.
          if (!repaired.isEmpty())
            repairedBuilder.add(
                IndexSearchResultIterator.build(
                    queryViewPair.left, repaired, mergeRange, queryContext, false, () -> {}));
        }
  ...
  private void maybeTriggerGuardrails(QueryViewBuilder.QueryView queryView) {
    ...
    if (Guardrails.saiSSTableIndexesPerQuery.failsOn(referencedIndexes, null)) {
      String msg =
          String.format(
              "Query %s attempted to read from too many indexes (%s) but max allowed is %s; "
  ...
  /**
   * Returns whether this query is not selecting the {@link PrimaryKey}. The query does not select
   * the key if both of the following statements are false: 1. The table associated with the query
   * is not using clustering keys 2. The clustering index filter for the command wants the row.
   *
   * <p>Item 2 is important in paged queries where the {@link
   * org.apache.cassandra.db.filter.ClusteringIndexSliceFilter} for subsequent paged queries may not
   * select rows that are returned by the index search because that is initially partition based.
   *
  ...
  public KeyRangeIterator getTopKRows(RowFilter.Expression expression) {
    ...
    assert index != null;
    var planExpression =
        Expression.create(index).add(Operator.ANN, expression.getIndexValue().duplicate());
    // search memtable before referencing sstable indexes; otherwise we may miss newly flushed
    // memtable index
    KeyRangeIterator memtableResults =
        index
            .memtableIndexManager()
            .searchMemtableIndexes(queryContext, planExpression, mergeRange);

    QueryViewBuilder.QueryView queryView =
        new QueryViewBuilder(Collections.singleton(planExpression), mergeRange).build();
    Runnable onClose = () -> queryView.referencedIndexes.forEach(SSTableIndex::releaseQuietly);

    try {
      List<KeyRangeIterator> sstableIntersections =
          queryView.view.stream().map(this::createRowIdIterator).collect(Collectors.toList());

      return IndexSearchResultIterator.build(
          sstableIntersections,
          memtableResults,
  ...
  public KeyRangeIterator getTopKRows(KeyRangeIterator source, RowFilter.Expression expression) {
    return new KeyRangeOrderingIterator(
        source, orderChunkSize, list -> this.getTopKRows(list, expression));
  }

  private KeyRangeIterator getTopKRows(
      List<PrimaryKey> rawSourceKeys, RowFilter.Expression expression) {
    VectorQueryContext vectorQueryContext = queryContext.vectorContext();
    // Filter out PKs now. Each PK is passed to every segment of the ANN index, so filtering
    // shadowed keys
    ...
    var sourceKeys =
        rawSourceKeys.stream()
            .filter(vectorQueryContext::shouldInclude)
    ...
    // memtable index
    KeyRangeIterator memtableResults =
        index.memtableIndexManager().limitToTopResults(queryContext, sourceKeys, planExpression);
    QueryViewBuilder.QueryView queryView =
        new QueryViewBuilder(Collections.singleton(planExpression), mergeRange).build();
    Runnable onClose = () -> queryView.referencedIndexes.forEach(SSTableIndex::releaseQuietly);
  ...
  private KeyRangeIterator createRowIdIterator(
      Pair<Expression, Collection<SSTableIndex>> indexExpression) {
    var subIterators =
        indexExpression.right.stream()
            .map(
                index -> {
                  try {
  ...
  private ClusteringIndexFilter makeFilter(PrimaryKey key) {
    ...
    assert cfs.metadata().comparator.size() == 0 && !key.kind().hasClustering
        ...
        : "PrimaryKey "
            + key
            + " clustering does not match table. There should be a clustering of size "
            + cfs.metadata().comparator.size();

    // If we have skinny partitions or the key is for a static row then we need to get the partition
    // as
    // requested by the original query.
    if (cfs.metadata().comparator.size() == 0 || key.kind() == PrimaryKey.Kind.STATIC)
      return clusteringIndexFilter;
    else
      return new ClusteringIndexNamesFilter(
          FBUtilities.singleton(key.clustering(), cfs.metadata().comparator),
