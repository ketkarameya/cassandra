...
public class MemtableIndexManager {
  ...
  public long index(DecoratedKey key, Row row, Memtable mt) {
    ...
    if (index.termType().isNonFrozenCollection()) {
      Iterator<ByteBuffer> bufferIterator =
          index.termType().valuesOf(row, FBUtilities.nowInSeconds());
    ...
    index
        ...
        .update(Clock.Global.nanoTime() - start, TimeUnit.NANOSECONDS);
    return bytes;
  }
  ...
  public KeyRangeIterator searchMemtableIndexes(
      QueryContext queryContext, Expression e, AbstractBounds<PartitionPosition> keyRange) {
  ...
  public KeyRangeIterator limitToTopResults(
      QueryContext context, List<PrimaryKey> source, Expression e) {
  ...
  public long estimatedMemIndexMemoryUsed() {
    return liveMemtableIndexMap.values().stream()
        .mapToLong(MemtableIndex::estimatedMemoryUsed)
        .sum();
  }
