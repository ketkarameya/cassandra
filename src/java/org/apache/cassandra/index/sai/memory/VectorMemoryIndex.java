...
public class VectorMemoryIndex extends MemoryIndex {
  ...
  public long update(
    ...
    if (oldRemaining == 0 && newRemaining == 0) return 0;
    ...
    if (different) {
      ...
      // make the changes in this order, so we don't have a window where the row is not in the index
      // at all
      if (newRemaining > 0)
        bytesUsed += graph.add(newValue, primaryKey, OnHeapGraph.InvalidVectorBehavior.FAIL);
      if (oldRemaining > 0) bytesUsed -= graph.remove(oldValue, primaryKey);

      // remove primary key if it's no longer indexed
      if (newRemaining <= 0 && oldRemaining > 0) primaryKeys.remove(primaryKey);
    }
    return bytesUsed;
  }
  ...
  private void updateKeyBounds(PrimaryKey primaryKey) {
    ...
    else if (primaryKey.compareTo(minimumKey) < 0) minimumKey = primaryKey;
    if (maximumKey == null) maximumKey = primaryKey;
    else if (primaryKey.compareTo(maximumKey) > 0) maximumKey = primaryKey;
  }

  @Override
  public KeyRangeIterator search(
      QueryContext queryContext, Expression expr, AbstractBounds<PartitionPosition> keyRange) {
    assert expr.getIndexOperator() == Expression.IndexOperator.ANN
        : "Only ANN is supported for vector search, received " + expr.getIndexOperator();
    ...
    if (!RangeUtil.coversFullRing(keyRange)) {
      // if left bound is MIN_BOUND or KEY_BOUND, we need to include all token-only PrimaryKeys with
      // same token
      boolean leftInclusive = keyRange.left.kind() != PartitionPosition.Kind.MAX_BOUND;
      // if right bound is MAX_BOUND or KEY_BOUND, we need to include all token-only PrimaryKeys
      // with same token
      ...
      PrimaryKey right =
          isMaxToken ? null : index.keyFactory().create(keyRange.right.getToken()); // upper bound

      Set<PrimaryKey> resultKeys =
          isMaxToken
              ? primaryKeys.tailSet(left, leftInclusive)
      ...
      if (resultKeys.isEmpty()) return KeyRangeIterator.empty();

      int bruteForceRows =
          maxBruteForceRows(vectorQueryContext.limit(), resultKeys.size(), graph.size());
      Tracing.trace(
          "Search range covers {} rows; max brute force rows is {} for memtable index with {}"
              + " nodes, LIMIT {}",
          resultKeys.size(),
          bruteForceRows,
          graph.size(),
      ...
      if (resultKeys.size() < Math.max(vectorQueryContext.limit(), bruteForceRows))
        ...
        bits =
            new KeyRangeFilteringBits(
                keyRange, vectorQueryContext.bitsetForShadowedPrimaryKeys(graph));
    } else {
      // partition/range deletion won't trigger index update, so we have to filter shadow primary
      // keys in memtable index
    ...
    if (keyQueue.isEmpty()) return KeyRangeIterator.empty();
    return new ReorderingRangeIterator(keyQueue);
  }

  @Override
  public KeyRangeIterator limitToTopResults(
      List<PrimaryKey> primaryKeys, Expression expression, int limit) {
    if (minimumKey == null)
      // This case implies maximumKey is empty too.
      return KeyRangeIterator.empty();

    List<PrimaryKey> results =
        primaryKeys.stream()
    ...
    Tracing.trace(
        "SAI materialized {} rows; max brute force rows is {} for memtable index with {} nodes,"
            + " LIMIT {}",
        results.size(),
        maxBruteForceRows,
        graph.size(),
    ...
    if (results.size() <= maxBruteForceRows) {
      if (results.isEmpty()) return KeyRangeIterator.empty();
    ...
    var raw =
        (int)
            (0.7
                ...
                * pow(graphSize, 0.33)
                * pow(log(limit), 2)
                * pow(log((double) graphSize / sizeRestriction), 2)
                / pow(sizeRestriction, 0.13));
    // we will always visit at least min(limit, graphSize) nodes, and we can't visit more nodes than
    // exist in the graph
  ...
  public Iterator<Pair<ByteComparable, PrimaryKeys>> iterator() {
    // This method is only used when merging an in-memory index with a RowMapping. This is done a
    // different
    // way with the graph using the writeData method below.
    throw new UnsupportedOperationException();
  }

  public SegmentMetadata.ComponentMetadataMap writeDirect(
  ...
  private class KeyRangeFilteringBits implements Bits {
    ...
    @Nullable private final Bits bits;
    ...
    public boolean get(int ordinal) {
      if (bits != null && !bits.get(ordinal)) return false;
  ...
  private class ReorderingRangeIterator extends KeyRangeIterator {
    ...
    protected void performSkipTo(PrimaryKey nextKey) {
      while (!keyQueue.isEmpty() && keyQueue.peek().compareTo(nextKey) < 0) keyQueue.poll();
    }
    ...
    protected PrimaryKey computeNext() {
      if (keyQueue.isEmpty()) return endOfData();
