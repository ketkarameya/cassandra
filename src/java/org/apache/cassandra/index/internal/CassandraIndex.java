...
/**
 * Index implementation which indexes the values for a single column in the base table and which
 * stores its index data in a local, hidden table.
 */
public abstract class CassandraIndex implements Index {
  ...
  /**
   * Used to construct an the clustering for an entry in the index table based on values from the
   * base data. The clustering columns in the index table encode the values required to retrieve the
   * correct data from the base table and varies depending on the kind of the indexed column. See
   * indexCfsMetadata for more details Used whenever a row in the index table is written or deleted.
   *
   ...
   */
  protected abstract <T> CBuilder buildIndexClusteringPrefix(
      ByteBuffer partitionKey, ClusteringPrefix<T> prefix, CellPath path);

  /**
   * Used at search time to convert a row in the index table into a simple struct containing the
   * values required to retrieve the corresponding row from the base table.
   *
  ...
  /**
   * Check whether a value retrieved from an index is still valid by comparing it to current row
   * from the base table. Used at read time to identify out of date index entries so that they can
   * be excluded from search results and repaired
   *
   ...
   * @return a ByteBuffer containing the value to be inserted in the index. This will be used to
   *     make the partition key in the index table
   */
  protected abstract ByteBuffer getIndexedValue(
  ...
  public void validate(ReadCommand command) throws InvalidRequestException {
    ...
    if (target.isPresent()) {
      ...
      checkFalse(
          indexValue.remaining() > FBUtilities.MAX_UNSIGNED_SHORT,
  ...
  private void setMetadata(IndexMetadata indexDef) {
    ...
    indexCfs =
        ColumnFamilyStore.createColumnFamilyStore(
            baseCfs.keyspace, tm.name, tm, baseCfs.getTracker().loadsstables);
      ...
      switch (getIndexMetadata().kind) {
        ...
        default:
          throw new IllegalStateException(
              String.format(
                  "Unsupported index type %s for index %s on %s",
                  metadata.kind, metadata.name, indexedColumn.name.toString()));
      }
    }
  ...
  public Indexer indexerFor(
    ...
    return new Indexer() {
      ...
      public void insertRow(Row row) {
        if (row.isStatic() && !indexedColumn.isStatic() && !indexedColumn.isPartitionKey()) return;
      ...
      public void updateRow(Row oldRow, Row newRow) {
        ...
        if (newRow.isStatic() != indexedColumn.isStatic()) return;

        if (isPrimaryKeyIndex())
          indexPrimaryKey(
              newRow.clustering(), getPrimaryKeyIndexLiveness(newRow), newRow.deletion());
      ...
      private void indexPrimaryKey(
        ...
        if (!deletion.isLive()) delete(key.getKey(), clustering, deletion.time(), ctx);
      }
      ...
      private LivenessInfo getPrimaryKeyIndexLiveness(Row row) {
        ...
        for (Cell<?> cell : row.cells()) {
          ...
          if (cell.isLive(nowInSec)) {
            if (cellTimestamp > timestamp) timestamp = cellTimestamp;
          }
        }
  ...
  /**
   * Specific to internal indexes, this is called by a searcher when it encounters a stale entry in
   * the index
   *
  ...
  private void delete(
      ByteBuffer rowKey, Clustering<?> clustering, Cell<?> cell, WriteContext ctx, long nowInSec) {
    DecoratedKey valueKey = getIndexKeyFor(getIndexedValue(rowKey, clustering, cell));
    doDelete(
        valueKey,
  ...
  private void delete(
      ByteBuffer rowKey, Clustering<?> clustering, DeletionTime deletion, WriteContext ctx) {
  ...
  private ByteBuffer getIndexedValue(ByteBuffer rowKey, Clustering<?> clustering, Cell<?> cell) {
    return getIndexedValue(
        rowKey, clustering, cell == null ? null : cell.path(), cell == null ? null : cell.buffer());
  }

  private Clustering<?> buildIndexClustering(
      ByteBuffer rowKey, Clustering<?> clustering, Cell<?> cell) {
    return buildIndexClusteringPrefix(rowKey, clustering, cell == null ? null : cell.path())
        .build();
  }
  ...
  private void buildBlocking() {
    ...
    try (ColumnFamilyStore.RefViewFragment viewFragment =
      ...
      if (sstables.isEmpty()) {
        logger.info(
            "No SSTable data for {}.{} to build index {} from, marking empty index as built",
  ...
  /**
   * Construct the TableMetadata for an index table, the clustering columns in the index table vary
   * dependent on the kind of the indexed value.
   *
   * @param baseCfsMetadata
   * @param indexMetadata
   * @return
   */
  public static TableMetadata indexCfsMetadata(
      TableMetadata baseCfsMetadata, IndexMetadata indexMetadata) {
    Pair<ColumnMetadata, IndexTarget.Type> target =
    ...
    TableMetadata.Builder builder =
        TableMetadata.builder(
                baseCfsMetadata.keyspace,
                baseCfsMetadata.indexTableName(indexMetadata),
                baseCfsMetadata.id)
            .kind(TableMetadata.Kind.INDEX)
            .partitioner(new LocalPartitioner(indexedValueType))
            .addPartitionKeyColumn(
                indexedColumn.name,
                isCompatible ? indexedColumn.type : utils.getIndexedPartitionKeyType(indexedColumn))
            .addClusteringColumn(
                "partition_key",
                isCompatible
                    ? baseCfsMetadata.partitioner.partitionOrdering()
                    : indexedTablePartitionKeyType);
  ...
  public static CassandraIndex newIndex(ColumnFamilyStore baseCfs, IndexMetadata indexMetadata) {
    return getFunctions(indexMetadata, TargetParser.parse(baseCfs.metadata(), indexMetadata))
        .newIndexInstance(baseCfs, indexMetadata);
  }

  static CassandraIndexFunctions getFunctions(
      IndexMetadata indexDef, Pair<ColumnMetadata, IndexTarget.Type> target) {
