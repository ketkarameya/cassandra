...
public class OnDiskIndex implements Iterable<OnDiskIndex.DataTerm>, Closeable {
  ...
  public RangeIterator<Long, Token> search(Expression exp) {
    ...
    if (exp.getOp() == Expression.Op.PREFIX
        && mode == OnDiskIndexBuilder.Mode.CONTAINS
        && !hasMarkedPartials)
    ...
    if (exclusions.size() == 0) return searchRange(expression);
    ...
    for (Expression e : ranges) {
      ...
      if (range != null) builder.add(range);
    }
  ...
  private RangeIterator<Long, Token> searchRange(Expression range) {
    ...
    int upperBlock =
        upper == null
            ...
            : (lower != null && comparator.compare(lower.value, upper.value) == 0)
                ? lowerBlock
                : getDataBlock(upper.value);

    return (mode != OnDiskIndexBuilder.Mode.SPARSE
            || lowerBlock == upperBlock
            || upperBlock - lowerBlock <= 1)
        ? searchPoint(lowerBlock, range)
        : searchRange(lowerBlock, lower, upperBlock, upper);
  }

  private RangeIterator<Long, Token> searchRange(
      int lowerBlock, Expression.Bound lower, int upperBlock, Expression.Bound upper) {
    // if lower is at the beginning of the block that means we can just do a single iterator per
    // block
    SearchResult<DataTerm> lowerPosition =
        (lower == null) ? null : searchIndex(lower.value, lowerBlock);
    SearchResult<DataTerm> upperPosition =
        (upper == null) ? null : searchIndex(upper.value, upperBlock);

    RangeUnionIterator.Builder<Long, Token> builder = RangeUnionIterator.builder();

    // optimistically assume that first and last blocks are full block reads, saves at least 3
    // 'else' conditions
    ...
    if (lowerPosition != null && (lowerPosition.index > 0 || !lower.inclusive)) {
      ...
      int start =
          (lower.inclusive || lowerPosition.cmp != 0)
              ? lowerPosition.index
              : lowerPosition.index + 1;
    ...
    if (upperPosition != null) {
      ...
      // The save as with 'lower' but here we need to check if the upper is the last element of the
      // block,
      ...
      if (upperPosition.index != lastIndex || !upper.inclusive) {
        int end =
            (upperPosition.cmp < 0 || (upperPosition.cmp == 0 && upper.inclusive))
                ? upperPosition.index + 1
                : upperPosition.index;
    ...
    int totalSuperBlocks =
        (lastFullBlockIdx - firstFullBlockIdx) / OnDiskIndexBuilder.SUPER_BLOCK_SIZE;
    ...
    // e.g. if the block range was (1, 9) and super-block-size = 4, we need to read 1, 2, 3, 4 - 7
    // is covered by
    ...
    int superBlockAlignedStart =
        firstFullBlockIdx == 0
            ? 0
            : (int) FBUtilities.align(firstFullBlockIdx, OnDiskIndexBuilder.SUPER_BLOCK_SIZE);
    for (int blockIdx = firstFullBlockIdx;
        blockIdx < Math.min(superBlockAlignedStart, lastFullBlockIdx);
        blockIdx++) builder.add(getBlockIterator(blockIdx));
  ...
  public Iterator<DataTerm> iteratorAt(ByteBuffer query, IteratorOrder order, boolean inclusive) {
    ...
    return new TermIterator(
        levels.length == 0 ? 0 : getBlockIdx(findPointer(query), query), e, order);
  }
  ...
  protected class DataLevel extends Level<DataBlock> {
    ...
    public OnDiskSuperBlock getSuperBlock(int idx) {
      assert idx < superBlockCnt
          : String.format(
              "requested index %d is greater than super block count %d", idx, superBlockCnt);
  ...
  protected class DataBlock extends OnDiskBlock<DataTerm> {
    ...
    public RangeIterator<Long, Token> getRange(int start, int end) {
      ...
      PrefetchedTokensIterator prefetched =
          sparse.isEmpty() ? null : new PrefetchedTokensIterator(sparse);
  ...
  public class DataTerm extends Term implements Comparable<DataTerm> {
    ...
    protected DataTerm(
        MappedBuffer content, OnDiskIndexBuilder.TermSize size, TokenTree perBlockIndex) {
  ...
  protected static class PointerTerm extends Term {
    public PointerTerm(
        MappedBuffer content, OnDiskIndexBuilder.TermSize size, boolean hasMarkedPartials) {
      super(content, size, hasMarkedPartials);
    }
  ...
  private static class PrefetchedTokensIterator extends RangeIterator<Long, Token> {
    ...
    protected void performSkipTo(Long nextToken) {
      currentIterator =
          Iterators.peekingIterator(tokens.tailMap(nextToken, true).values().iterator());
    }
