...
public class SASIIndex implements Index, INotificationConsumer {
  ...
  public static final String USAGE_WARNING =
  ...
  private static class SASIIndexBuildingSupport implements IndexBuildingSupport {
    public SecondaryIndexBuilder getIndexBuildTask(
      ...
      NavigableMap<SSTableReader, Map<ColumnMetadata, ColumnIndex>> sstables =
  ...
  private static final SASIIndexBuildingSupport INDEX_BUILDER_SUPPORT =
      new SASIIndexBuildingSupport();
  ...
  public SASIIndex(ColumnFamilyStore baseCfs, IndexMetadata config) {
    ...
    SortedMap<SSTableReader, Map<ColumnMetadata, ColumnIndex>> toRebuild =
        new TreeMap<>(SSTableReader.idComparator);
  ...
  public Indexer indexerFor(
    ...
    return new Indexer() {
      ...
      public void insertRow(Row row) {
        if (isNewData())
          adjustMemtableSize(
              index.index(key, row), CassandraWriteContext.fromContext(context).getGroup());
      }
      ...
      public void adjustMemtableSize(long additionalSpace, OpOrder.Group opGroup) {
        baseCfs
            ...
            .getView()
            .getCurrentMemtable()
            .markExtraOnHeapUsed(additionalSpace, opGroup);
      }
    };
  }
  ...
  public SSTableFlushObserver getFlushObserver(Descriptor descriptor, LifecycleNewTracker tracker) {
    return newWriter(
        baseCfs.metadata().partitionKeyType,
        descriptor,
        Collections.singletonMap(index.getDefinition(), index),
        tracker.opType());
  }
  ...
  protected static PerSSTableIndexWriter newWriter(
