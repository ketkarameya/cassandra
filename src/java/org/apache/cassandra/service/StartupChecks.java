...
/**
 * Verifies that the system and environment is in a fit state to be started. Used in
 * CassandraDaemon#setup() to check various settings and invariants.
 *
 * <p>Each individual test is modelled as an implementation of StartupCheck, these are run at the
 * start of CassandraDaemon#setup() before any local state is mutated. The default checks are a mix
 * of informational tests (inspectJvmOptions), initialization (checkProcessEnvironment,
 * checkCacheServiceInitialization) and invariant checking (checkValidLaunchDate,
 * checkSystemKeyspaceState, checkSSTablesFormat).
 *
 * <p>In addition, if checkSystemKeyspaceState determines that the release version has changed since
 * last startup (i.e. the node has been upgraded) it snapshots the system keyspace to make it easier
 * to back out if necessary.
 *
 * <p>If any check reports a failure, then the setup method exits with an error (after logging any
 * output from the tests). If all tests report success, setup can continue. We should be careful in
 * future to ensure anything which mutates local state (such as writing new sstables etc) only
 * happens after we've verified the initial setup.
 */
public class StartupChecks {
   ...
   *
   * @throws StartupException if any test determines that the system is not in an valid state to
   *     startup
  ...
  public void verify(StartupChecksOptions options) throws StartupException {
    for (StartupCheck test : preFlightChecks) test.execute(options);
  ...
  public static final StartupCheck checkKernelBug1057843 =
      new StartupCheck() {
        ...
        public void execute(StartupChecksOptions startupChecksOptions) throws StartupException {
          if (startupChecksOptions.isDisabled(getStartupCheckType())) return;

          if (!FBUtilities.isLinux) return;
          ...
          if (!directIOWritePaths.isEmpty() && IGNORE_KERNEL_BUG_1057843_CHECK.getBoolean()) {
            logger.info(
                "Ignoring check for the kernel bug 1057843 against the following paths configured"
                    + " to be accessed with Direct IO: {}",
                directIOWritePaths);
            return;
          }
          ...
          for (Path path : directIOWritePaths) {
            try {
              ...
              throw new StartupException(
                  StartupException.ERR_WRONG_MACHINE_STATE,
          ...
          Range<Semver> affectedKernels =
              Range.closedOpen(
                  new Semver("6.1.64", Semver.SemverType.LOOSE),
                  new Semver("6.1.66", Semver.SemverType.LOOSE));

          Semver kernelVersion = FBUtilities.getKernelVersion();
          if (!affectedKernels.contains(kernelVersion.withClearedSuffixAndBuild())) return;

          throw new StartupException(
              StartupException.ERR_WRONG_MACHINE_STATE,
              String.format(
                  "Detected kernel version %s with affected file system types %s and direct IO"
                      + " enabled for paths %s. This combination is known to cause data corruption."
                      + " To start Cassandra in this environment, you have to disable direct IO for"
                      + " the affected paths. If you are sure the verification provided a false"
                      + " positive result, you can suppress it by setting '"
                      + IGNORE_KERNEL_BUG_1057843_CHECK.getKey()
                      + "' system property to 'true'. Please see"
                      + " https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=1057843 for more"
                      + " information.",
                  kernelVersion,
                  affectedFileSystemTypes,
                  affectedPaths));
        }
      };
  ...
  public static final StartupCheck checkJemalloc =
      new StartupCheck() {
        ...
        public void execute(StartupChecksOptions options) {
          if (options.isDisabled(getStartupCheckType())) return;
          ...
          else if ("-".equals(jemalloc)) logger.info("jemalloc preload explicitly disabled");
      ...
      new StartupCheck() {
        ...
        public void execute(StartupChecksOptions options) {
          ...
          try {
            ...
            logger.warn(
                "lz4-java was unable to load native libraries; this will lower the performance of"
                    + " lz4 (network/sstables/etc.): {}",
  ...
  public static final StartupCheck checkValidLaunchDate =
      new StartupCheck() {
        /**
         * The earliest legit timestamp a casandra instance could have ever launched. Date roughly
         * taken from
         * http://perspectives.mvdirona.com/2008/07/12/FacebookReleasesCassandraAsOpenSource.aspx We
         * use this to ensure the system clock is at least somewhat correct at startup.
         */
        ...
        public void execute(StartupChecksOptions options) throws StartupException {
          ...
          if (now < EARLIEST_LAUNCH_DATE)
            throw new StartupException(
                StartupException.ERR_WRONG_MACHINE_STATE,
                String.format(
                    "current machine time is %s, but that is seemingly incorrect. exiting now.",
  ...
  public static final StartupCheck checkJMXPorts =
      new StartupCheck() {
        ...
        public void execute(StartupChecksOptions options) {
          ...
          if (jmxPort == null) {
            logger.warn(
                "JMX is not enabled to receive remote connections. Please see cassandra-env.sh for"
                    + " more info.");
            jmxPort = CassandraRelevantProperties.CASSANDRA_JMX_LOCAL_PORT.toString();
            if (jmxPort == null)
              logger.error(
                  CASSANDRA_JMX_LOCAL_PORT.getKey()
      ...
      new StartupCheck() {
        ...
        public void execute(StartupChecksOptions options) {
          ...
          if (COM_SUN_MANAGEMENT_JMXREMOTE_PORT.isPresent()) {
            logger.warn(
                "Use of com.sun.management.jmxremote.port at startup is deprecated. "
      ...
      new StartupCheck() {
        ...
        public void execute(StartupChecksOptions options) {
          ...
          if (!DatabaseDescriptor.hasLargeAddressSpace())
            logger.warn(
                "32bit JVM detected.  It is recommended to run Cassandra on a 64bit JVM for better"
                    + " performance.");
          ...
          if (!(javaVmName.contains("HotSpot") || javaVmName.contains("OpenJDK"))) {
            logger.warn(
                "Non-Oracle JVM detected.  Some features, such as immediate unmap of compacted"
                    + " SSTables, may not work as intended");
          } else {
        ...
        private void checkOutOfMemoryHandling() {
          if (JavaUtils.supportExitOnOutOfMemory(JAVA_VERSION.getString())) {
            if (!jvmOptionsContainsOneOf(
                "-XX:OnOutOfMemoryError=",
                "-XX:+ExitOnOutOfMemoryError",
                "-XX:+CrashOnOutOfMemoryError"))
              logger.warn(
                  "The JVM is not configured to stop on OutOfMemoryError which can cause data"
                      + " corruption. Use one of the following JVM options to configure the"
                      + " behavior on OutOfMemoryError:  -XX:+ExitOnOutOfMemoryError,"
            ...
            if (!jvmOptionsContainsOneOf("-XX:OnOutOfMemoryError="))
              logger.warn(
                  "The JVM is not configured to stop on OutOfMemoryError which can cause data"
                      + " corruption. Either upgrade your JRE to a version greater or equal to 8u92"
                      + " and use -XX:+ExitOnOutOfMemoryError/-XX:+CrashOnOutOfMemoryError or use"
                      + " -XX:OnOutOfMemoryError=\"<cmd args>;<cmd args>\" on your current JRE.");
          }
        }
         ...
         * @return {@code true} if one of the specified options is being used, {@code false}
         *     otherwise.
         */
        private boolean jvmOptionsContainsOneOf(String... optionNames) {
          ...
          for (String argument : inputArguments) {
            for (String optionName : optionNames) if (argument.startsWith(optionName)) return true;
          }
          return false;
        }
      };
      ...
      new StartupCheck() {
        ...
        public void execute(StartupChecksOptions options) throws StartupException {
          ...
          if (!NativeLibrary.isAvailable())
            throw new StartupException(
                StartupException.ERR_WRONG_MACHINE_STATE,
                "The native library could not be initialized properly. ");
        }
      };

  public static final StartupCheck checkProcessEnvironment =
      new StartupCheck() {
        @Override
        public void execute(StartupChecksOptions options) {
  ...
  public static final StartupCheck checkReadAheadKbSetting =
      new StartupCheck() {
        ...
        /**
         * Function to get the block device system path(Example: /dev/sda) from the data directories
         * defined in cassandra config.(cassandra.yaml)
         *
        ...
        @Override
        public void execute(StartupChecksOptions options) {
          if (options.isDisabled(getStartupCheckType()) || !FBUtilities.isLinux) return;
          ...
          for (Map.Entry<String, String> entry : blockDevices.entrySet()) {
            ...
            try {
              ...
              if (readAheadKBPath == null || Files.notExists(readAheadKBPath)) {
                logger.debug(
                    "No 'read_ahead_kb' setting found for device {} of data directory {}.",
                    blockDeviceDirectory,
                    dataDirectory);
                continue;
              }
              ...
              if (readAheadKbSetting > MAX_RECOMMENDED_READ_AHEAD_KB_SETTING) {
                logger.warn(
                    "Detected high '{}' setting of {} for device '{}' of data directory '{}'. It is"
                        + " recommended to set this value to 8KB (or lower) on SSDs or 64KB (or"
                        + " lower) on HDDs to prevent excessive IO usage and page cache churn on"
      ...
      new StartupCheck() {
        ...
        public void execute(StartupChecksOptions options) {
          ...
          if (maxMapCount < EXPECTED_MAX_MAP_COUNT)
            logger.warn(
                "Maximum number of memory map areas per process (vm.max_map_count) {} "
                    + "is too low, recommended value: {}, you can change it with sysctl.",
                maxMapCount,
                EXPECTED_MAX_MAP_COUNT);
        }
      };
  ...
  public static final StartupCheck checkSSTablesFormat =
      new StartupCheck() {
        ...
        public void execute(StartupChecksOptions options) throws StartupException {
          ...
          nonSSTablePaths.add(
              FileUtils.getCanonicalPath(DatabaseDescriptor.getCommitLogLocation()));
          nonSSTablePaths.add(
              FileUtils.getCanonicalPath(DatabaseDescriptor.getSavedCachesLocation()));
          nonSSTablePaths.add(FileUtils.getCanonicalPath(DatabaseDescriptor.getHintsDirectory()));

          FileVisitor<Path> sstableVisitor =
              new SimpleFileVisitor<Path>() {
                public FileVisitResult visitFile(Path path, BasicFileAttributes attrs) {
                  File file = new File(path);
                  if (!Descriptor.isValidFile(file)) return FileVisitResult.CONTINUE;

                  try {
                    Descriptor desc = Descriptor.fromFileWithComponent(file, false).left;
                    if (!desc.isCompatible()) invalid.add(file.toString());
                ...
                public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs)
                    throws IOException {
                  String[] nameParts =
                      FileUtils.getCanonicalPath(new File(dir)).split(java.io.File.separator);
                  if (nameParts.length >= 2) {
                    String tablePart = nameParts[nameParts.length - 1];
                    String ksPart = nameParts[nameParts.length - 2];

                    if (tablePart.contains("-")) tablePart = tablePart.split("-")[0];

                    // In very old versions of Cassandra, we wouldn't necessarily delete sstables
                    // from dropped system tables
                    // which were removed in various major version upgrades (e.g system.Versions in
                    // 1.2)
                    if (ksPart.equals(SchemaConstants.SYSTEM_KEYSPACE_NAME)
                      ...
                      // We can have snapshots of our system tables or snapshots created with a -t
                      // tag of "system" that would trigger
                      // this potential warning, so we warn more softly in the case that it's
                      // probably a snapshot.
                      if (canonicalPath.contains("snapshot")) {
                        logger.info(
                            "Found unknown system directory {}.{} at {} that contains the word"
                                + " snapshot. This may be left over from a previous version of"
                                + " Cassandra or may be normal.  Consider removing after inspection"
                        ...
                        logger.warn(
                            "Found unknown system directory {}.{} at {} - this is likely left over"
                                + " from a previous version of Cassandra and should be removed"
                                + " after inspection.",
                            ksPart,
                            tablePart,
                            canonicalPath);
          ...
          if (!invalid.isEmpty())
            throw new StartupException(
                StartupException.ERR_WRONG_DISK_STATE,
                String.format(
                    "Detected unreadable sstables %s, please check "
          ...
          if (!withIllegalGenId.isEmpty())
            throw new StartupException(
                StartupException.ERR_WRONG_CONFIG,
                "UUID sstable identifiers are disabled but some sstables have been "
                    ...
                    + Joiner.on(", ").join(withIllegalGenId)
                    + ". If you decide to delete sstables, and have that data replicated over other"
                    + " healthy nodes, those will be broughtback during repair");
        }
      };
  ...
  public static final StartupCheck checkSystemKeyspaceState =
      new StartupCheck() {
        ...
        public void execute(StartupChecksOptions options) throws StartupException {
          if (options.isDisabled(getStartupCheckType())) return;
          // check the system keyspace to keep user from shooting self in foot by changing
          // partitioner, cluster name, etc.
          // we do a one-off scrub of the system keyspace first; we can't load the list of the rest
          // of the keyspaces,
          ...
          try {
            ...
            throw new StartupException(
                StartupException.ERR_WRONG_CONFIG, "Fatal exception during initialization", e);
          }
        }
      };
          ...
          if (storedDc != null) {
            ...
            if (!storedDc.equals(currentDc)) {
              String formatMessage =
                  "Cannot start node if snitch's data center (%s) differs from previous data center"
                      + " (%s). Please fix the snitch configuration, decommission and rebootstrap"
                      + " this node";

              throw new StartupException(
                  StartupException.ERR_WRONG_CONFIG,
                  String.format(formatMessage, currentDc, storedDc));
            }
          }
        }
      };
        ...
        public void execute(StartupChecksOptions options) throws StartupException {
          ...
          if (storedRack != null) {
            ...
            if (!storedRack.equals(currentRack)) {
              String formatMessage =
                  "Cannot start node if snitch's rack (%s) differs from previous rack (%s). Please"
                      + " fix the snitch configuration, decommission and rebootstrap this node";

              throw new StartupException(
                  StartupException.ERR_WRONG_CONFIG,
                  String.format(formatMessage, currentRack, storedRack));
            }
          }
        }
      };
  ...
  static Optional<String> checkLegacyAuthTablesMessage() {
    ...
    if (!existing.isEmpty())
      return Optional.of(
          String.format(
              "Legacy auth tables %s in keyspace %s still exist and have not been properly"
                  + " migrated.",
