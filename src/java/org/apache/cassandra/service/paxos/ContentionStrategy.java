...
/**
 * A strategy for making back-off decisions for Paxos operations that fail to make progress because
 * of other paxos operations. The strategy is defined by four factors:
 *
 * <ul>
 *   <li>{@link #min}
 *   <li>{@link #max}
 ...
 * </ul>
 *
 * <p>The first three represent time periods, and may be defined dynamically based on a simple
 * calculation over:
 *
 * <ul>
 *   <li>{@code pX()} recent experienced latency distribution for successful operations, e.g. {@code
 *       p50(rw)} the maximum of read and write median latencies, {@code p999(r)} the 99.9th
 *       percentile of read latencies
 ...
 *
 *     <p>Furthermore, the dynamic calculations can be bounded with a min/max, like so: {@code
 ...
 *
 *     <p>These calculations are put together to construct a range from which we draw a random
 *     number. The period we wait for {@code X} will be drawn so that {@code min <= X < max}.
 *
 ...
 *
 *     <p>The {@link #waitRandomizer} property specifies the manner in which a random value is drawn
 *     from the range. It is defined using one of the following specifiers:
 * <li>uniform
 * <li>exp($power$) or exponential($power$)
 * <li>qexp($power$) or qexponential($power$) or quantizedexponential($power$)
 *
 *     <p>The uniform specifier is self-explanatory, selecting all values in the range with equal
 *     probability. The exponential specifier draws values towards the end of the range with higher
 *     probability, raising a floating point number in the range [0..1.0) to the power provided, and
 *     translating the resulting value to a uniform value in the range. The quantized exponential
 *     specifier partitions the range into {@code attempts} buckets, then applies the pure
 *     exponential approach to draw values from [0..attempts), before drawing a uniform value from
 *     the corresponding bucket
 *
 *     <p>Finally, there is also a {@link #traceAfterAttempts} property that permits initiating
 *     tracing of operations that experience a certain minimum number of failed paxos rounds due to
 *     contention. A setting of 0 or 1 will initiate a trace session after the first failed ballot.
 */
public class ContentionStrategy {
  ...
  private static final Pattern RANDOMIZER =
      Pattern.compile(
          "uniform|exp(onential)?[(](?<exp>[0-9.]+)[)]|q(uantized)?exp(onential)?[(](?<qexp>[0-9.]+)[)]");
  private static final String DEFAULT_WAIT_RANDOMIZER =
      "qexp(1.5)"; // at least 0ms, and at least 66% of median latency
  private static final String DEFAULT_MIN =
      "0 <= p50(rw)*0.66"; // at least 0ms, and at least 66% of median latency
  private static final String DEFAULT_MAX =
      "10ms <= p95(rw)*1.8^attempts <= 100ms"; // p95 latency with exponential back-off at rate of
                                               // 1.8^attempts
  private static final String DEFAULT_MIN_DELTA =
      "5ms <= p50(rw)*0.5"; // at least 5ms, and at least 50% of median latency

  private static volatile ContentionStrategy current;

  // Factories can be useful for testing purposes, to supply custom implementations of selectors and
  // modifiers.
  static final LatencySelectorFactory selectors = new LatencySelectorFactory() {};
  static final LatencyModifierFactory modifiers = new LatencyModifierFactory() {};
  static final WaitRandomizerFactory randomizers = new WaitRandomizerFactory() {};

  static {
    current =
        new ContentionStrategy(
            defaultWaitRandomizer(),
            defaultMinWait(),
            defaultMaxWait(),
            defaultMinDelta(),
            Integer.MAX_VALUE);
  ...
  static interface LatencyModifierFactory {
    ...
    default LatencyModifier multiply(double constant) {
      return (l, a) -> saturatedCast(l * constant);
    }

    default LatencyModifier multiplyByAttempts(double multiply) {
      return (l, a) -> saturatedCast(l * multiply * a);
    }
  ...
  static interface LatencySelector {
    abstract long select(
        LatencySupplier readLatencyHistogram, LatencySupplier writeLatencyHistogram);
  }

  static interface LatencySelectorFactory {
    default LatencySelector constant(long latency) {
      return (read, write) -> latency;
    }

    default LatencySelector read(double percentile) {
      return (read, write) -> read.get(percentile);
    }

    default LatencySelector write(double percentile) {
      return (read, write) -> write.get(percentile);
    }

    default LatencySelector maxReadWrite(double percentile) {
      return (read, write) -> max(read.get(percentile), write.get(percentile));
    }
  }
  ...
  static interface WaitRandomizerFactory {
    ...
    } // DO NOT USE METHOD HANDLES (want to fetch afresh each time)
    ...
    default WaitRandomizer exponential(double power) {
      return new Exponential(uniformLongSupplier(), uniformDoubleSupplier(), power);
    }

    default WaitRandomizer quantizedExponential(double power) {
      return new QuantizedExponential(uniformLongSupplier(), uniformDoubleSupplier(), power);
    }
    ...
    abstract static class AbstractExponential implements WaitRandomizer {
      final LongBinaryOperator uniformLong;
      final DoubleSupplier uniformDouble;
      final double power;

      public AbstractExponential(
          LongBinaryOperator uniformLong, DoubleSupplier uniformDouble, double power) {
        this.uniformLong = uniformLong;
    ...
    static class Exponential extends AbstractExponential {
      ...
      @Override
      public long wait(long min, long max, int attempts) {
        if (attempts == 1) return uniformLong.applyAsLong(min, max);
    ...
    static class QuantizedExponential extends AbstractExponential {
      ...
      public long wait(long min, long max, int attempts) {
        ...
        if (attempts == 1 || quanta == 0) return uniformLong.applyAsLong(min, max);
  ...
  static class TimeLimitedLatencySupplier extends AtomicReference<SnapshotAndTime>
  ...
  static class Bound {
    ...
    Bound(long min, long max, long onFailure, LatencyModifier modifier, LatencySelector selector) {
      Preconditions.checkArgument(
          min <= max, "min (%s) must be less than or equal to max (%s)", min, max);
      this.min = min;
      this.max = max;
      ...
      this.reads =
          new TimeLimitedLatencySupplier(casReadMetrics.latency::getSnapshot, 10L, SECONDS);
      this.writes =
          new TimeLimitedLatencySupplier(casWriteMetrics.latency::getSnapshot, 10L, SECONDS);
    }
  ...
  public ContentionStrategy(
      String waitRandomizer, String min, String max, String minDelta, int traceAfterAttempts) {
  ...
  long computeWaitUntilForContention(
      int attempts,
      TableMetadata table,
    ...
    if (attempts >= traceAfterAttempts && !Tracing.isTracing()) {
      ...
      logger.info(
          "Tracing contended paxos {} for key {} on {}.{} with trace id {}",
          type.lowercase,
  ...
  static boolean waitForContention(
      long deadline,
      int attempts,
      TableMetadata table,
      DecoratedKey partitionKey,
  ...
  static long waitUntilForContention(
      int attempts,
      TableMetadata table,
      DecoratedKey partitionKey,
      ConsistencyLevel consistency,
  ...
  static ParsedStrategy parseStrategy(String spec) {
    ...
    ContentionStrategy strategy =
        new ContentionStrategy(waitRandomizer, min, max, minDelta, traceAfterAttempts);
  ...
  private static LatencySelector parseLatencySelector(Matcher m, LatencySelectorFactory selectors) {
    ...
    if (perc == null) return selectors.constant(parseInMicros(m.group("constbase")));

    double percentile = parseDouble("0." + perc);
    String rw = m.group("rw");
    if (rw.length() == 2) return selectors.maxReadWrite(percentile);
    else if ("r".equals(rw)) return selectors.read(percentile);
  ...
  private static LatencyModifier parseLatencyModifier(Matcher m, LatencyModifierFactory modifiers) {
    ...
    if (mod == null) return modifiers.identity();

    double modifier = parseDouble(mod);

    String modkind = m.group("modkind");
    if (modkind == null) return modifiers.multiply(modifier);

    if (modkind.startsWith("*")) return modifiers.multiplyByAttempts(modifier);
    else if (modkind.startsWith("^")) return modifiers.multiplyByAttemptsExp(modifier);
    else throw new IllegalArgumentException("Unrecognised attempt modifier: " + modkind);
  }
  ...
  static WaitRandomizer parseWaitRandomizer(String input, WaitRandomizerFactory randomizers) {
    ...
    if (!m.matches()) throw new IllegalArgumentException(input + " does not match" + RANDOMIZER);

    String exp;
    exp = m.group("exp");
    if (exp != null) return randomizers.exponential(Double.parseDouble(exp));
    exp = m.group("qexp");
    if (exp != null) return randomizers.quantizedExponential(Double.parseDouble(exp));
  ...
  @VisibleForTesting
  static Bound parseBound(
      String input,
      boolean isMin,
      LatencySelectorFactory selectors,
      LatencyModifierFactory modifiers) {
    Matcher m = BOUND.matcher(input);
    if (!m.matches()) throw new IllegalArgumentException(input + " does not match " + BOUND);
    ...
    return new Bound(
        ...
        max,
        isMin ? min : max,
        parseLatencyModifier(m, modifiers),
        parseLatencySelector(m, selectors));
  }

  private static long parseInMicros(String input, long orElse) {
    if (input == null) return orElse;
  ...
  private static long parseInMicros(String input) {
    ...
    if (!m.matches()) throw new IllegalArgumentException(input + " does not match " + TIME);

    String text;
    if (null != (text = m.group(1))) return parseInt(text) * 1000;
    else if (null != (text = m.group(2))) return parseInt(text);
    else return 0;
  }
  ...
  static long maxQueryTimeoutMicros() {
    return max(
        max(getCasContentionTimeout(MICROSECONDS), getWriteRpcTimeout(MICROSECONDS)),
        getReadRpcTimeout(MICROSECONDS));
  }
