...
public class Rebuild {
  ...
  public static void rebuild(
    ...
    if (sourceDc != null) {
      if (sourceDc.equals(DatabaseDescriptor.getLocalDataCenter())
          && excludeLocalDatacenterNodes) // fail if source DC is local and --exclude-local-dc is
        // set
        throw new IllegalArgumentException(
            "Cannot set source data center to be local data center, when excludeLocalDataCenter"
                + " flag is set");
    ...
    try {
      ...
      if (excludeLocalDatacenterNodes)
        streamer.addSourceFilter(
            new RangeStreamer.ExcludeLocalDatacenterFilter(DatabaseDescriptor.getEndpointSnitch()));
          ...
          for (String stringHost : stringHosts) {
            try {
              ...
              if (getBroadcastAddressAndPort().equals(endpoint)) {
                throw new IllegalArgumentException(
                    "This host was specified as a source for rebuilding. Sources for a rebuild can"
                        + " only be other nodes in the cluster.");
              }
  ...
  private static RangesAtEndpoint rangesForRebuildWithTokens(String tokens, String keyspace) {
    ...
    RangesAtEndpoint.Builder streamRanges =
        new RangesAtEndpoint.Builder(getBroadcastAddressAndPort(), ranges.size());
    for (Range<Token> specifiedRange : ranges) {
      ...
      if (!foundParentRange) {
        throw new IllegalArgumentException(
            String.format(
                "The specified range %s is not a range that is owned by this node. Please ensure"
                    + " that all token ranges specified to be rebuilt belong to this node.",
  ...
  private static MovementMap movementMap(ClusterMetadata metadata, String keyspace, String tokens) {
    ...
    if (keyspace == null) {
      placements.forEach(
          (params, placement) ->
              movementMapBuilder.put(params, addMovementsForParams(placement, null)));
    } else if (tokens == null) {
  ...
  private static EndpointsByReplica addMovementsForParams(
      DataPlacement placement, RangesAtEndpoint ranges) {
    EndpointsByReplica.Builder movements = new EndpointsByReplica.Builder();
    RangesAtEndpoint localReplicas =
        ranges != null ? ranges : placement.reads.byEndpoint().get(getBroadcastAddressAndPort());
    for (Replica localReplica : localReplicas) {
      placement
          .reads
          .forRange(localReplica.range().right)
          .forEach(
              r -> {
                if (!r.equals(localReplica)) movements.put(localReplica, r);
              });
    }
