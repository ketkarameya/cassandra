...
public class DataResurrectionCheck implements StartupCheck {
  ...
  public void execute(StartupChecksOptions options) throws StartupException {
    ...
    if (!heartbeatFile.exists()) {
      LOGGER.debug(
          "Heartbeat file {} not found! Skipping heartbeat startup check.",
    ...
    try {
      ...
      throw new StartupException(
          ERR_WRONG_DISK_STATE, "Failed to deserialize heartbeat file " + heartbeatFile);
    }

    if (heartbeat.lastHeartbeat == null) return;
    ...
    for (String keyspace : getKeyspaces()) {
      ...
      for (TableGCPeriod userTable : getTablesGcPeriods(keyspace)) {
        if (excludedTables.contains(Pair.create(keyspace, userTable.table))) continue;
    ...
    if (!violations.isEmpty()) {
      String invalidTables =
          violations.stream().map(p -> format("%s.%s", p.left, p.right)).collect(joining(","));

      String exceptionMessage =
          format(
              "There are tables for which gc_grace_seconds is older "
  ...
  public void postAction(StartupChecksOptions options) {
    ...
    // as it might happen that other checks after it might fail, but we would be heartbeating
    // already.
    if (options.isEnabled(StartupChecks.StartupCheckType.check_data_resurrection)) {
      ...
      ScheduledExecutors.scheduledTasks.scheduleAtFixedRate(
          () -> {
            Heartbeat heartbeat =
                new Heartbeat(Instant.ofEpochMilli(Clock.Global.currentTimeMillis()));
            try {
              ...
              DataResurrectionCheck.LOGGER.error(
                  "Unable to serialize heartbeat to " + heartbeatFile, ex);
            }
          },
          0,
  ...
  public Set<String> getExcludedKeyspaces(Map<String, Object> config) {
    ...
    if (excludedKeyspacesConfigValue == null) return Collections.emptySet();
    else
  ...
  List<String> getKeyspaces() {
    return SchemaKeyspace.fetchNonSystemKeyspaces().stream()
  ...
  List<TableGCPeriod> getTablesGcPeriods(String userKeyspace) {
    ...
    if (!keyspaceMetadata.isPresent()) return Collections.emptyList();
