...
public class SSTableZeroCopyWriter extends SSTable implements SSTableMultiWriter {
  ...
  public SSTableZeroCopyWriter(
      Builder<?, ?> builder, LifecycleNewTracker lifecycleNewTracker, SSTable.Owner owner) {
    super(builder, owner);
    ...
    if (!unsupported.isEmpty())
      throw new AssertionError(
          format("Unsupported streaming components detected: %s", unsupported));

    for (Component c : components) componentWriters.put(c.name, makeWriter(descriptor, c));
  }
  ...
  private void write(DataInputPlus in, long size, SequentialWriter out) throws FSWriteError {
    ...
    try {
      ...
      out
          .sync(); // finish will also call sync(). Leaving here to get stuff flushed as early as
                   // possible
    } catch (IOException e) {
  ...
  public void writeComponent(Component component, DataInputPlus in, long size)
    ...
    logger.info(
        "Writing component {} to {} length {}",
        component,
        writer.getPath(),
        prettyPrintMemory(size));

    if (in instanceof AsyncStreamingInputPlus) write((AsyncStreamingInputPlus) in, size, writer);
    else write(in, size, writer);
  }
  ...
  private void write(AsyncStreamingInputPlus in, long size, SequentialWriter writer)
    ...
    try {
      ...
      // FSWriteError triggers disk failure policy, but if we get a connection issue we do not want
      // to do that
