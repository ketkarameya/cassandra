...
public final class Types implements Iterable<UserType> {
  ...
  public Iterable<UserType> referencingUserType(ByteBuffer name) {
    return Iterables.filter(
        types.values(), t -> t.referencesUserType(name) && !t.name.equals(name));
  }
   ...
   * @return an empty {@link Optional} if the type name is not found; a non-empty optional of {@link
   *     UserType} otherwise
   */
  ...
  public Types without(ByteBuffer name) {
    UserType type =
        get(name)
            .orElseThrow(() -> new IllegalStateException(format("Type %s doesn't exists", name)));
  ...
  public boolean equals(Object o) {
    ...
    while (thisIter.hasNext()) {
      ...
      if (!thisNext.getKey().equals(otherNext.getKey())) return false;

      if (!thisNext.getValue().equals(otherNext.getValue())) return false;
    }
    return true;
  }
  ...
  public static final class RawBuilder {
     ...
     *
     * <p>Constructs a DAG of graph dependencies and resolves them 1 by 1 in topological order.
     */
    public Types build() {
      if (definitions.isEmpty()) return Types.none();

      /*
       * build a DAG of UDT dependencies
       */
      Map<RawUDT, Integer> vertices =
          Maps.newHashMapWithExpectedSize(
              definitions.size()); // map values are numbers of referenced types
      ...
      for (RawUDT udt1 : definitions)
        for (RawUDT udt2 : definitions)
          if (udt1 != udt2 && udt1.referencesUserType(udt2)) adjacencyList.put(udt2, udt1);
      ...
      for (Map.Entry<RawUDT, Integer> entry : vertices.entrySet())
        if (entry.getValue() == 0) resolvableTypes.add(entry.getKey());
      ...
      if (types.types.size() != definitions.size())
        throw new ConfigurationException(
            format("Cannot resolve UDTs for keyspace %s: some types are missing", keyspace));
    ...
    public void add(String name, List<String> fieldNames, List<String> fieldTypes) {
      List<CQL3Type.Raw> rawFieldTypes =
          fieldTypes.stream().map(CQLTypeParser::parseRaw).collect(toList());
    ...
    private static final class RawUDT {
      ...
      UserType prepare(String keyspace, Types types) {
        List<FieldIdentifier> preparedFieldNames =
            fieldNames.stream().map(FieldIdentifier::forInternalString).collect(toList());
  ...
  static final class TypesDiff extends Diff<Types, UserType> {
    ...
    private TypesDiff(
        Types created, Types dropped, ImmutableCollection<Altered<UserType>> altered) {
  ...
  public static class Serializer {
    public void serialize(Types t, DataOutputPlus out, Version version) throws IOException {
      ...
      for (UserType type : t.types.values()) {
        ...
        for (String s : fieldNames) out.writeUTF(s);
        out.writeInt(fieldTypes.size());
        for (String s : fieldTypes) out.writeUTF(s);
      }
    }

    public Types deserialize(String keyspace, DataInputPlus in, Version version)
        throws IOException {
      ...
      for (int i = 0; i < count; i++) {
        ...
        for (int x = 0; x < fieldNamesSize; x++) fieldNames.add(in.readUTF());
        int fieldTypeSize = in.readInt();
        List<String> fieldTypes = new ArrayList<>(fieldTypeSize);
        for (int x = 0; x < fieldTypeSize; x++) fieldTypes.add(in.readUTF());
    ...
    public long serializedSize(Types t, Version version) {
      ...
      for (UserType type : t.types.values()) {
        ...
        for (String s : fieldNames) size += sizeof(s);
        size += sizeof(fieldTypes.size());
        for (String s : fieldTypes) size += sizeof(s);
      }
      return size;
    }
  }
