...
/**
 * For backwards compatibility, in the first instance an IndexMetadata must have TargetType.COLUMN
 * and its Set of target columns must contain only a single ColumnIdentifier. Hence, this is what is
 * enforced by the public factory methods on IndexMetadata. These constraints, along with the
 * internal datastructures here will be relaxed as support is added for multiple target columns
 * per-index and for indexes with TargetType.ROW
 */
public final class Indexes implements Iterable<IndexMetadata> {
   ...
   * @return an empty {@link Optional} if the named index is not found; a non-empty optional of
   ...
   * @return an empty {@link Optional} if no index with the specified id is found; a non-empty
   *     optional of {@link IndexMetadata} otherwise
   */
  ...
  public Indexes without(String name) {
    IndexMetadata index =
        get(name)
            .orElseThrow(() -> new IllegalStateException(format("Index %s doesn't exist", name)));
