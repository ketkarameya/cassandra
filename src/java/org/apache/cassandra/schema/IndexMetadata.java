...
public final class IndexMetadata {
  ...
  static {
    ...
    indexNameAliases.put(
        StorageAttachedIndex.class.getSimpleName().toLowerCase(),
  ...
  public static IndexMetadata fromSchemaMetadata(
      String name, Kind kind, Map<String, String> options) {
    return new IndexMetadata(name, options, kind);
  }

  public static IndexMetadata fromIndexTargets(
      List<IndexTarget> targets, String name, Kind kind, Map<String, String> options) {
  ...
  public void validate(TableMetadata table) {
    ...
    if (kind == Kind.CUSTOM) {
      ...
      if (!Index.class.isAssignableFrom(indexerClass))
        throw new ConfigurationException(
            String.format(
                "Specified Indexer class (%s) does not implement the Indexer interface",
                className));
  ...
  private void validateCustomIndexOptions(
      TableMetadata table, Class<? extends Index> indexerClass, Map<String, String> options) {
    try {
      ...
      try {
        unknownOptions =
            (Map)
                indexerClass
                    .getMethod("validateOptions", Map.class, TableMetadata.class)
                    .invoke(null, filteredOptions, table);
      } catch (NoSuchMethodException e) {
        unknownOptions =
            (Map)
                indexerClass.getMethod("validateOptions", Map.class).invoke(null, filteredOptions);
      }
      ...
      logger.info(
          "Indexer {} does not have a static validateOptions method. Validation ignored",
  ...
  public void appendCqlTo(CqlBuilder builder, TableMetadata table, boolean ifNotExists) {
    if (isCustom()) {
      ...
      if (!copyOptions.isEmpty()) builder.append(" WITH OPTIONS = ").append(copyOptions);
    } else {
      ...
      builder.append(" USING '").append(CassandraIndex.NAME).append("'");
    }
    builder.append(';');
  }

  public static class Serializer {
    public void serialize(IndexMetadata metadata, DataOutputPlus out, int version)
        throws IOException {
      UUIDSerializer.serializer.serialize(metadata.id, out, version);
    }

    public IndexMetadata deserialize(DataInputPlus in, int version, TableMetadata table)
        throws IOException {
  ...
  public static class MetadataSerializer
      implements org.apache.cassandra.tcm.serialization.MetadataSerializer<IndexMetadata> {
