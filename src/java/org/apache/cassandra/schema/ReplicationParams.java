...
public final class ReplicationParams {
  ...
  private ReplicationParams(
      Class<? extends AbstractReplicationStrategy> klass, Map<String, String> options) {
    this.klass = klass;
  ...
  /**
   * For backward-compatibility reasons we are persisting replication params for cluster metadata as
   * non-meta replication params. This means that when we are creating mutations, meta params will
   * be written to as Network Topology Strategy (in local DC, if no DCs are specified), and when
   * schema is loaded from system tables, we will create a meta strategy instance for cluster
   * metadata keyspace.
   */
  ...
  static ReplicationParams simple(String replicationFactor) {
    return new ReplicationParams(
        SimpleStrategy.class, ImmutableMap.of("replication_factor", replicationFactor));
  }
  ...
  public static ReplicationParams simpleMeta(int replicationFactor, Set<String> knownDatacenters) {
    ...
    if (knownDatacenters.isEmpty()) throw new IllegalStateException("No known datacenters");
  ...
  public static ReplicationParams ntsMeta(Map<String, Integer> replicationFactor) {
    ...
    if (aggregate <= 0)
      throw new IllegalArgumentException(
          "Aggregate replication factor should be strictly positive: " + replicationFactor);
  ...
  public static ReplicationParams meta(ClusterMetadata metadata) {
    ReplicationParams metaParams =
        metadata.schema.getKeyspaceMetadata(SchemaConstants.METADATA_KEYSPACE_NAME)
            .params
            .replication;
  ...
  public static ReplicationParams fromMapWithDefaults(
      Map<String, String> map, Map<String, String> previousOptions) {
  ...
  public void appendCqlTo(CqlBuilder builder) {
    String classname =
        "org.apache.cassandra.locator".equals(klass.getPackage().getName())
            ? klass.getSimpleName()
            : klass.getName();
  ...
  public static class Serializer implements MetadataSerializer<ReplicationParams> {
    public void serialize(ReplicationParams t, DataOutputPlus out, Version version)
        throws IOException {
    ...
    public ReplicationParams deserialize(DataInputPlus in, Version version) throws IOException {
      ...
      for (int i = 0; i < size; i++) options.put(in.readUTF(), in.readUTF());
      return new ReplicationParams(
          FBUtilities.classForName(klassName, "ReplicationStrategy"), options);
    }
