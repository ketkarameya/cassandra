...
public final class CompactionParams {
  ...
  public static final Map<String, String> DEFAULT_THRESHOLDS =
      ImmutableMap.of(
          Option.MIN_THRESHOLD.toString(), Integer.toString(DEFAULT_MIN_THRESHOLD),
  ...
  static {
    ...
    if (defaultCompaction == null) {
      DEFAULT =
          new CompactionParams(
              SizeTieredCompactionStrategy.class,
              DEFAULT_THRESHOLDS,
              DEFAULT_ENABLED,
              DEFAULT_PROVIDE_OVERLAPPING_TOMBSTONES_PROPERTY_VALUE);
    } else {
      DEFAULT = create(classFromName(defaultCompaction.class_name), defaultCompaction.parameters);
    }
  }
  ...
  public static CompactionParams create(
      Class<? extends AbstractCompactionStrategy> klass, Map<String, String> options) {
    boolean isEnabled =
        options.containsKey(Option.ENABLED.toString())
            ? Boolean.parseBoolean(options.get(Option.ENABLED.toString()))
            : DEFAULT_ENABLED;
    String overlappingTombstoneParm =
        options
            .getOrDefault(
                Option.PROVIDE_OVERLAPPING_TOMBSTONES.toString(),
    ...
    if (!tombstoneOptional.isPresent()) {
      throw new ConfigurationException(
          format(
              "Invalid value %s for 'provide_overlapping_tombstones' compaction sub-option - must"
                  + " be one of the following [%s].",
              overlappingTombstoneParm, StringUtils.join(TombstoneOption.values(), ", ")));
    }
  ...
  public void validate() {
    try {
      Map<?, ?> unknownOptions =
          (Map) klass.getMethod("validateOptions", Map.class).invoke(null, options);
      ...
      logger.warn(
          "Compaction strategy {} does not have a static validateOptions method. Validation"
              + " ignored",
          klass.getName());
      ...
      throw new ConfigurationException(
          format(
              ...
              klass.getName(), cause.getClass().getName(), cause.getMessage()),
          e);
    } catch (IllegalAccessException e) {
      throw new ConfigurationException(
          "Cannot access method validateOptions in " + klass.getName(), e);
    }
    ...
    if (minThreshold != null && !StringUtils.isNumeric(minThreshold)) {
      throw new ConfigurationException(
          format(
              ...
              minThreshold, Option.MIN_THRESHOLD));
    }
    ...
    if (maxThreshold != null && !StringUtils.isNumeric(maxThreshold)) {
      throw new ConfigurationException(
          format(
              "Invalid value %s for '%s' compaction sub-option - must be an integer",
              maxThreshold, Option.MAX_THRESHOLD));
    }
    ...
    if (minCompactionThreshold() <= 1) {
      throw new ConfigurationException(
          format(
              "Min compaction threshold cannot be less than 2 (got %d)", minCompactionThreshold()));
    }

    if (minCompactionThreshold() > maxCompactionThreshold()) {
      throw new ConfigurationException(
          format(
              "Min compaction threshold (got %d) cannot be greater than max compaction threshold"
                  + " (got %d)",
    ...
    if (className == null) {
      throw new ConfigurationException(
          format(
              ...
              Option.CLASS, TableParams.Option.COMPACTION));
    }
  ...
  public static Class<? extends AbstractCompactionStrategy> classFromName(String name) {
    ...
    if (!AbstractCompactionStrategy.class.isAssignableFrom(strategyClass)) {
      throw new ConfigurationException(
          format(
              "Compaction strategy class %s is not derived from AbstractReplicationStrategy",
              className));
    }
  ...
  public static boolean supportsThresholdParams(Class<? extends AbstractCompactionStrategy> klass) {
    try {
      Map<String, String> unrecognizedOptions =
          (Map<String, String>)
              klass.getMethod("validateOptions", Map.class).invoke(null, DEFAULT_THRESHOLDS);
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof CompactionParams)) return false;
