 ...
 *
 * <p>Please notice this class is _not_ thread safe and all methods which reads or updates the data
 * in schema keyspace should be accessed only from the implementation of {SchemaUpdateHandler} in
 * synchronized blocks.
 */
@NotThreadSafe
public final class SchemaKeyspace {
  ...
  private static final boolean IGNORE_CORRUPTED_SCHEMA_TABLES_PROPERTY_VALUE =
      IGNORE_CORRUPTED_SCHEMA_TABLES.getBoolean();

  /**
   * The tables to which we added the cdc column. This is used in {@link #makeUpdateForSchema} below
   * to make sure we skip that column is cdc is disabled as the columns breaks pre-cdc to post-cdc
  ...
  public static Collection<Mutation> convertSchemaDiffToMutations(
      KeyspacesDiff diff, long timestamp) {
    Map<String, Mutation> mutations = new HashMap<>();

    diff.dropped.forEach(
        k -> mutations.put(k.name, makeDropKeyspaceMutation(k, timestamp).build()));
    diff.created.forEach(
        k -> mutations.put(k.name, makeCreateKeyspaceMutation(k, timestamp).build()));
    diff.altered.forEach(
        kd -> {
          ...
          kd.types
              .altered(Difference.SHALLOW)
              .forEach(td -> addTypeToSchemaMutation(td.after, builder));
          ...
          kd.tables
              .altered(Difference.SHALLOW)
              .forEach(td -> addAlterTableToSchemaMutation(td.before, td.after, builder));
          ...
          kd.views
              .altered(Difference.SHALLOW)
              .forEach(vd -> addAlterViewToSchemaMutation(vd.before, vd.after, builder));
          ...
          kd.udfs
              .altered(Difference.SHALLOW)
              .forEach(fd -> addFunctionToSchemaMutation(fd.after, builder));
          ...
          kd.udas
              .altered(Difference.SHALLOW)
              .forEach(ad -> addAggregateToSchemaMutation(ad.after, builder));
  ...
  static void saveSystemKeyspacesSchema() {
    ...
    for (String schemaTable : ALL) {
      String query =
          String.format(
              "DELETE FROM %s.%s USING TIMESTAMP ? WHERE keyspace_name = ?",
              SchemaConstants.SCHEMA_KEYSPACE_NAME, schemaTable);
      ...
      ALL.forEach(
          table ->
              FBUtilities.waitOnFuture(
                  getSchemaCFS(table).forceFlush(ColumnFamilyStore.FlushReason.INTERNALLY_FORCED)));
  }
  ...
  private static ColumnFamilyStore getSchemaCFS(String schemaTableName) {
    return Keyspace.open(SchemaConstants.SCHEMA_KEYSPACE_NAME)
        .getColumnFamilyStore(schemaTableName);
  }
  ...
  static Collection<Mutation> convertSchemaToMutations() {
    ...
    return mutationMap.values().stream()
        .map(Mutation.PartitionUpdateCollector::build)
        .collect(Collectors.toList());
  }

  private static void convertSchemaToMutations(
      Map<DecoratedKey, Mutation.PartitionUpdateCollector> mutationMap, String schemaTableName) {
      ...
      while (iter.hasNext()) {
        try (UnfilteredRowIterator partition = iter.next()) {
          ...
          Mutation.PartitionUpdateCollector puCollector =
              mutationMap.computeIfAbsent(
                  key,
                  k ->
                      new Mutation.PartitionUpdateCollector(
                          SchemaConstants.SCHEMA_KEYSPACE_NAME, key));
          puCollector.add(
              makeUpdateForSchema(partition, cmd.columnFilter()).withOnlyPresentColumns());
        }
      }
    }
  }
  ...
  /**
   * Creates a PartitionUpdate from a partition containing some schema table content. This is mainly
   * calling {@code PartitionUpdate.fromIterator} except for the fact that it deals with the problem
   * described in #12236.
   */
  private static PartitionUpdate makeUpdateForSchema(
      UnfilteredRowIterator partition, ColumnFilter filter) {
    // This method is used during schema migration tasks, and if cdc is disabled, we want to force
    // excluding the
    // 'cdc' column from the TABLES/VIEWS schema table because it is problematic if received by
    // older nodes (see #12236
    // and #12697). Otherwise though, we just simply "buffer" the content of the partition into a
    // PartitionUpdate.
    if (DatabaseDescriptor.isCDCEnabled()
        || !TABLES_WITH_CDC_ADDED.contains(partition.metadata().name))
    ...
    // 'PartitionUpdate.fromIterator()' will ignore any columns that are marked as 'fetched' but not
    // 'queried'.
    ColumnFilter.Builder builder =
        ColumnFilter.allRegularColumnsBuilder(partition.metadata(), false);
  ...
  private static Mutation.SimpleBuilder makeCreateKeyspaceMutation(
      String name, KeyspaceParams params, long timestamp) {
    Mutation.SimpleBuilder builder =
        Mutation.simpleBuilder(Keyspaces.keyspace, decorate(Keyspaces, name)).timestamp(timestamp);
    ...
    builder
        ...
        .add(
            ...
            (params.replication.isMeta() ? params.replication.asNonMeta() : params.replication)
                .asMap());
  ...
  @VisibleForTesting
  static Mutation.SimpleBuilder makeCreateKeyspaceMutation(
      KeyspaceMetadata keyspace, long timestamp) {
    Mutation.SimpleBuilder builder =
  ...
  private static Mutation.SimpleBuilder makeDropKeyspaceMutation(
      KeyspaceMetadata keyspace, long timestamp) {
    Mutation.SimpleBuilder builder =
        Mutation.simpleBuilder(
                SchemaConstants.SCHEMA_KEYSPACE_NAME, decorate(Keyspaces, keyspace.name))
            .timestamp(timestamp);
  ...
  @VisibleForTesting
  static Mutation.SimpleBuilder makeCreateTableMutation(
      KeyspaceMetadata keyspace, TableMetadata table, long timestamp) {
    // Include the serialized keyspace in case the target node missed a CREATE KEYSPACE migration
    // (see CASSANDRA-5631).
    Mutation.SimpleBuilder builder =
        makeCreateKeyspaceMutation(keyspace.name, keyspace.params, timestamp);
  ...
  private static void addTableToSchemaMutation(
      TableMetadata table, boolean withColumnsAndTriggers, Mutation.SimpleBuilder builder) {
  ...
  private static void addTableParamsToRowBuilder(TableParams params, Row.SimpleBuilder builder) {
    ...
    // Only add CDC-enabled flag to schema if it's enabled on the node. This is to work around RTE's
    // post-8099 if a 3.8+
    // node sends table schema to a < 3.8 versioned node with an unknown column.
    if (DatabaseDescriptor.isCDCEnabled()) builder.add("cdc", params.cdc);

    // As above, only add the memtable column if the table uses a non-default memtable configuration
    // to avoid RTE
    ...
    if (!params.allowAutoSnapshot) builder.add("allow_auto_snapshot", false);
    ...
    if (!params.incrementalBackups) builder.add("incremental_backups", false);
  }

  private static void addAlterTableToSchemaMutation(
      TableMetadata oldTable, TableMetadata newTable, Mutation.SimpleBuilder builder) {
    ...
    MapDifference<String, TriggerMetadata> triggerDiff =
        triggersDiff(oldTable.triggers, newTable.triggers);
    ...
    MapDifference<String, IndexMetadata> indexesDiff =
        indexesDiff(oldTable.indexes, newTable.indexes);
  ...
  @VisibleForTesting
  static Mutation.SimpleBuilder makeUpdateTableMutation(
      KeyspaceMetadata keyspace, TableMetadata oldTable, TableMetadata newTable, long timestamp) {
    Mutation.SimpleBuilder builder =
        makeCreateKeyspaceMutation(keyspace.name, keyspace.params, timestamp);
  ...
  private static void addDropTableToSchemaMutation(
      TableMetadata table, Mutation.SimpleBuilder builder) {
  ...
  private static void addColumnToSchemaMutation(
      TableMetadata table, ColumnMetadata column, Mutation.SimpleBuilder builder) {
    ...
    if (SchemaConstants.isReplicatedSystemKeyspace(table.keyspace)) {
      // The propagation of system distributed keyspaces at startup can be problematic for old nodes
      // without DDM,
      // since those won't know what to do with the mask mutations. Thus, we don't support DDM on
      // those keyspaces.
      assert mask == null
          : "Dynamic data masking shouldn't be used on system distributed keyspaces";
    } else {
      Row.SimpleBuilder maskBuilder =
          builder.update(ColumnMasks).row(table.name, column.name.toString());
      ...
      if (mask == null) {
        ...
        // Some arguments of the masking function can be null, but the CQL's list type that stores
        // them doesn't
        ...
        maskBuilder
            .add("function_keyspace", maskFunctionName.keyspace)
  ...
  private static void dropColumnFromSchemaMutation(
      TableMetadata table, ColumnMetadata column, Mutation.SimpleBuilder builder) {
    // Note: we do want to use name.toString(), not name.bytes directly for backward compatibility
    // (For CQL3, this won't make a difference).
    builder.update(Columns).row(table.name, column.name.toString()).delete();
  }

  private static void addDroppedColumnToSchemaMutation(
      TableMetadata table, DroppedColumn column, Mutation.SimpleBuilder builder) {
    builder
        .update(DroppedColumns)
  ...
  private static void dropDroppedColumnFromSchemaMutation(
      TableMetadata table, DroppedColumn column, Mutation.SimpleBuilder builder) {
    builder.update(DroppedColumns).row(table.name, column.column.name.toString()).delete();
  }

  private static void addTriggerToSchemaMutation(
      TableMetadata table, TriggerMetadata trigger, Mutation.SimpleBuilder builder) {
    builder
        .update(Triggers)
  ...
  private static void addViewToSchemaMutation(
      ViewMetadata view, boolean includeColumns, Mutation.SimpleBuilder builder) {
  ...
  private static void addDropViewToSchemaMutation(
      ViewMetadata view, Mutation.SimpleBuilder builder) {
  ...
  private static void addAlterViewToSchemaMutation(
      ViewMetadata before, ViewMetadata after, Mutation.SimpleBuilder builder) {
  ...
  private static void addIndexToSchemaMutation(
      TableMetadata table, IndexMetadata index, Mutation.SimpleBuilder builder) {
    builder
        .update(Indexes)
  ...
  private static void dropIndexFromSchemaMutation(
      TableMetadata table, IndexMetadata index, Mutation.SimpleBuilder builder) {
    builder.update(Indexes).row(table.name, index.name).delete();
  }

  private static void addUpdatedIndexToSchemaMutation(
      TableMetadata table, IndexMetadata index, Mutation.SimpleBuilder builder) {
    addIndexToSchemaMutation(table, index, builder);
  }

  private static void addFunctionToSchemaMutation(
      UDFunction function, Mutation.SimpleBuilder builder) {
    builder
        .update(Functions)
  ...
  private static void addDropFunctionToSchemaMutation(
      UDFunction function, Mutation.SimpleBuilder builder) {
    builder.update(Functions).row(function.name().name, function.argumentsList()).delete();
  }

  private static void addAggregateToSchemaMutation(
      UDAggregate aggregate, Mutation.SimpleBuilder builder) {
    builder
        .update(Aggregates)
        ...
        .add(
            "final_func",
            aggregate.finalFunction() != null ? aggregate.finalFunction().name().name : null)
        .add(
            "initcond",
            aggregate.initialCondition() != null
                // must use the frozen state type here, as 'null' for unfrozen collections may mean
                // 'empty'
                ? aggregate
                    .stateType()
                    .freeze()
                    .asCQL3Type()
                    .toCQLLiteral(aggregate.initialCondition())
                : null);
  }

  private static void addDropAggregateToSchemaMutation(
      UDAggregate aggregate, Mutation.SimpleBuilder builder) {
  ...
  private static Keyspaces fetchKeyspacesWithout(Set<String> excludedKeyspaceNames) {
    String query =
        format("SELECT keyspace_name FROM %s.%s", SchemaConstants.SCHEMA_KEYSPACE_NAME, KEYSPACES);
  ...
  private static Types fetchTypes(String keyspaceName) {
    String query =
        format(
            "SELECT * FROM %s.%s WHERE keyspace_name = ?",
            SchemaConstants.SCHEMA_KEYSPACE_NAME, TYPES);
  ...
  private static Tables fetchTables(String keyspaceName, Types types, UserFunctions functions) {
    String query =
        format(
            "SELECT table_name FROM %s.%s WHERE keyspace_name = ?",
    ...
    for (UntypedResultSet.Row row : query(query, keyspaceName)) {
      ...
      try {
        ...
        String errorMsg =
            String.format(
                "No partition columns found for table %s.%s in %s.%s.  This may be due to"
                    + " corruption or concurrent dropping and altering of a table. If this table is"
                    + " supposed to be dropped, {}run the following query to cleanup: \"DELETE FROM"
                    + " %s.%s WHERE keyspace_name = '%s' AND table_name = '%s'; DELETE FROM %s.%s"
                    + " WHERE keyspace_name = '%s' AND table_name = '%s';\" If the table is not"
                    + " supposed to be dropped, restore %s.%s sstables from backups.",
                keyspaceName,
                tableName,
                SchemaConstants.SCHEMA_KEYSPACE_NAME,
                COLUMNS,
        ...
        if (IGNORE_CORRUPTED_SCHEMA_TABLES_PROPERTY_VALUE) {
          ...
          logger.error(
              errorMsg,
              "restart cassandra with -D{}=true and ",
              IGNORE_CORRUPTED_SCHEMA_TABLES.getKey());
          throw exc;
        }
      }
    }
  ...
  private static TableMetadata fetchTable(
      String keyspaceName, String tableName, Types types, UserFunctions functions) {
    String query =
        String.format(
    ...
    if (rows.isEmpty())
      throw new RuntimeException(
          String.format(
              "%s:%s not found in the schema definitions keyspace.", keyspaceName, tableName));
    ...
    TableParams.Builder builder =
        TableParams.builder()
            ...
            .additionalWritePolicy(
                row.has("additional_write_policy")
  ...
  private static List<ColumnMetadata> fetchColumns(
      String keyspace, String table, Types types, UserFunctions functions) {
    String query =
        format(
            "SELECT * FROM %s.%s WHERE keyspace_name = ? AND table_name = ?",
    ...
    if (columns.stream().noneMatch(ColumnMetadata::isPartitionKey))
      throw new MissingColumns(
          "No partition key columns found in schema table for " + keyspace + "." + table);

    return columns;
  }

  @VisibleForTesting
  public static ColumnMetadata createColumnFromRow(
    ...
    ColumnIdentifier name =
        new ColumnIdentifier(row.getBytes("column_name_bytes"), row.getString("column_name"));

    ColumnMask mask = null;
    String query =
        format(
            "SELECT * FROM %s.%s WHERE keyspace_name = ? AND table_name = ? AND column_name = ?",
    ...
    if (!columnMasks.isEmpty()) {
      ...
      FunctionName functionName =
          new FunctionName(
              maskRow.getString("function_keyspace"), maskRow.getString("function_name"));

      List<String> partialArgumentTypes =
          maskRow.getFrozenList("function_argument_types", UTF8Type.instance);
      ...
      Function function =
          FunctionResolver.get(keyspace, functionName, argumentTypes, null, null, null, functions);
      if (function == null) {
        throw new AssertionError(
            format(
                "Unable to find masking function %s(%s) for column %s.%s.%s",
                functionName, argumentTypes, keyspace, table, name));
      } else if (!(function instanceof ScalarFunction)) {
        throw new AssertionError(
            format(
                "Column %s.%s.%s is unexpectedly masked with function %s "
      ...
      List<String> valuesAsCQL =
          maskRow.getFrozenList("function_argument_values", UTF8Type.instance);
      ByteBuffer[] values = new ByteBuffer[valuesAsCQL.size()];
      for (int i = 0; i < valuesAsCQL.size(); i++) {
        if (nulls.get(i)) values[i] = null;
        else values[i] = argumentTypes.get(i + 1).fromString(valuesAsCQL.get(i));
      }
  ...
  private static Map<ByteBuffer, DroppedColumn> fetchDroppedColumns(String keyspace, String table) {
    String query =
        format(
            "SELECT * FROM %s.%s WHERE keyspace_name = ? AND table_name = ?",
            SchemaConstants.SCHEMA_KEYSPACE_NAME, DROPPED_COLUMNS);
  ...
  private static DroppedColumn createDroppedColumnFromRow(UntypedResultSet.Row row) {
    ...
    AbstractType<?> type =
        CQLTypeParser.parse(
            keyspace, row.getString("type"), org.apache.cassandra.schema.Types.none());
    ...
    ColumnMetadata column =
        new ColumnMetadata(
            keyspace,
            table,
            ColumnIdentifier.getInterned(name, true),
            type,
  ...
  private static Indexes fetchIndexes(String keyspace, String table) {
    String query =
        String.format(
            "SELECT * FROM %s.%s WHERE keyspace_name = ? AND table_name = ?",
            SchemaConstants.SCHEMA_KEYSPACE_NAME, INDEXES);
  ...
  private static Triggers fetchTriggers(String keyspace, String table) {
    String query =
        String.format(
            "SELECT * FROM %s.%s WHERE keyspace_name = ? AND table_name = ?",
            SchemaConstants.SCHEMA_KEYSPACE_NAME, TRIGGERS);
  ...
  private static Views fetchViews(String keyspaceName, Types types, UserFunctions functions) {
    String query =
        format(
            "SELECT view_name FROM %s.%s WHERE keyspace_name = ?",
            SchemaConstants.SCHEMA_KEYSPACE_NAME, VIEWS);
  ...
  private static ViewMetadata fetchView(
      String keyspaceName, String viewName, Types types, UserFunctions functions) {
    String query =
        String.format(
            "SELECT * FROM %s.%s WHERE keyspace_name = ? AND view_name = ?",
            SchemaConstants.SCHEMA_KEYSPACE_NAME, VIEWS);
    UntypedResultSet rows = query(query, keyspaceName, viewName);
    if (rows.isEmpty())
      throw new RuntimeException(
          String.format(
              "%s:%s not found in the schema definitions keyspace.", keyspaceName, viewName));
    UntypedResultSet.Row row = rows.one();
    ...
    try {
      ...
      throw new RuntimeException(
          format(
              "Unexpected error while parsing materialized view's where clause for '%s' (got %s)",
              viewName, whereClauseString));
    }
  ...
  private static UDFunction createUDFFromRow(UntypedResultSet.Row row, Types types) {
    ...
    AbstractType<?> returnType =
        CQLTypeParser.parse(ksName, row.getString("return_type"), types).udfType();
    ...
    if (existing instanceof UDFunction) {
      ...
      // Duplicate compilation attempts can occur on the coordinator node handling the CREATE
      // FUNCTION
      // statement, since CreateFunctionStatement needs to execute UDFunction.create but schema
      // migration
    ...
    try {
      return UDFunction.create(
          name, argNames, argTypes, returnType, calledOnNullInput, language, body);
    } catch (InvalidRequestException e) {
      logger.error(
          String.format(
              "Cannot load function '%s' from schema: this function won't be available (on this"
                  + " node)",
              name),
          e);
      return UDFunction.createBrokenFunction(
          name, argNames, argTypes, returnType, calledOnNullInput, language, body, e);
    }
  }

  private static Collection<UDAggregate> fetchUDAs(
      String keyspaceName, Collection<UDFunction> udfs, Types types) {
    String query =
        format(
            "SELECT * FROM %s.%s WHERE keyspace_name = ?",
  ...
  private static UDAggregate createUDAFromRow(
      UntypedResultSet.Row row, Collection<UDFunction> functions, Types types) {
    String ksName = row.getString("keyspace_name");
    String functionName = row.getString("aggregate_name");
    FunctionName name = new FunctionName(ksName, functionName);

    List<AbstractType<?>> argTypes =
        row.getFrozenList("argument_types", UTF8Type.instance).stream()
    ...
    FunctionName finalFunc =
        row.has("final_func") ? new FunctionName(ksName, row.getString("final_func")) : null;
    AbstractType<?> stateType =
        row.has("state_type")
            ? CQLTypeParser.parse(ksName, row.getString("state_type"), types)
            : null;
    ByteBuffer initcond;
    if (row.has("initcond")) {
    ...
    return UDAggregate.create(
        functions, name, argTypes, returnType, stateFunc, finalFunc, stateType, initcond);
  }
  ...
  public static void applyChanges(Collection<Mutation> mutations) {
    ...
    if (SchemaKeyspace.FLUSH_SCHEMA_TABLES) SchemaKeyspace.flush();
  }
  ...
  static Keyspaces fetchKeyspaces(Set<String> toFetch) {
    ...
    String query =
        format(
            "SELECT keyspace_name FROM %s.%s WHERE keyspace_name IN ?",
            SchemaConstants.SCHEMA_KEYSPACE_NAME, KEYSPACES);
