...
public final class KeyspaceMetadata implements SchemaElement {
  ...
  private KeyspaceMetadata(
    ...
    this.replicationStrategy =
        AbstractReplicationStrategy.createReplicationStrategy(keyspaceName, params.replication);
  }
  ...
  public static KeyspaceMetadata create(String name, KeyspaceParams params, Tables tables) {
    return new KeyspaceMetadata(
        name, Kind.REGULAR, params, tables, Views.none(), Types.none(), UserFunctions.none());
  }

  public static KeyspaceMetadata create(
      String name,
      KeyspaceParams params,
      Tables tables,
      Views views,
   ...
   * @return a stream of tables within this keyspace that have column masks using the specified user
   *     function
   */
  ...
  public String findAvailableIndexName(String baseName) {
    if (!hasIndex(baseName)) return baseName;

    int i = 1;
    do {
      String name = baseName + '_' + i++;
      if (!hasIndex(name)) return name;
    } while (true);
  }

  public Optional<TableMetadata> findIndexedTable(String indexName) {
    for (TableMetadata table : tablesAndViews())
      if (table.indexes.has(indexName)) return Optional.of(table);
  ...
  public Optional<TableMetadata> getIndexMetadata(String indexName) {
    ...
    if (metadata != null) return Optional.of(metadata);
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof KeyspaceMetadata)) return false;
  ...
  public void validate(ClusterMetadata metadata) {
    if (!SchemaConstants.isValidName(name)) {
      throw new ConfigurationException(
          format(
              "Keyspace name must not be empty, more than %s characters long, "
    ...
    for (TableMetadata table : tables) {
      for (IndexMetadata index : table.indexes) {
        if (indexNames.contains(index.name))
          throw new ConfigurationException(
              format("Duplicate index name %s in keyspace %s", index.name, name));
  ...
  public static final class KeyspaceDiff {
    ...
    private static Optional<KeyspaceDiff> diff(KeyspaceMetadata before, KeyspaceMetadata after) {
      if (before == after) return Optional.empty();

      if (!before.name.equals(after.name)) {
        String msg =
            String.format(
                "Attempting to diff two keyspaces with different names ('%s' and '%s')",
      ...
      if (before.params.equals(after.params)
          ...
          && types.isEmpty()
          && udfs.isEmpty()
          && udas.isEmpty()) return Optional.empty();
  ...
  public static class Serializer implements MetadataSerializer<KeyspaceMetadata> {
    public void serialize(KeyspaceMetadata t, DataOutputPlus out, Version version)
        throws IOException {
      out.writeUTF(t.name);
