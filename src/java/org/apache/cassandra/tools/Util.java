...
public final class Util {
  ...
  public static class TermHistogram {
    ...
    public TermHistogram(
        TombstoneHistogram histogram,
        String title,
        LongFunction<String> offsetName,
        LongFunction<String> countName) {
      this(
          new TreeMap<Number, Long>() {
            {
              histogram.forEach(
                  (point, value) -> {
    ...
    public String bar(long count, int length, String color, boolean unicode) {
      ...
      if (!Strings.isNullOrEmpty(color)) sb.append(RESET);
    ...
    public void printHistogram(PrintStream out, boolean color, boolean unicode) {
      ...
      out.printf(
          "   %-" + offsetTitleLength + "s %s %-" + maxCountLength + "s  %s  %sHistogram%s %n",
  ...
  public static TableMetadata metadataFromSSTable(Descriptor desc) throws IOException {
    if (!desc.version.isCompatible())
      throw new IOException(
          "Unsupported SSTable version " + desc.getFormat().name() + "/" + desc.version);

    StatsComponent statsComponent =
        StatsComponent.load(desc, MetadataType.STATS, MetadataType.HEADER);
    ...
    TableMetadata.Builder builder =
        TableMetadata.builder("keyspace", "table").partitioner(partitioner).offline();
    ...
    if (SecondaryIndexManager.isIndexColumnFamily(desc.cfname)) {
      ...
      // Just set the Kind of index to CUSTOM, which is an irrelevant parameter that doesn't make
