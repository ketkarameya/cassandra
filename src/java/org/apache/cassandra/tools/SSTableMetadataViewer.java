...
public class SSTableMetadataViewer {
  ...
  public SSTableMetadataViewer(
      boolean color, boolean unicode, int gc, TimeUnit tsUnit, PrintStream out) {
    this.color = color;
    this.tsUnit = tsUnit;
  ...
  public static String toDateString(long time, TimeUnit unit) {
    ...
    return new java.text.SimpleDateFormat("MM/dd/yyyy HH:mm:ss")
        .format(new java.util.Date(unit.toMillis(time)));
  }
  ...
  private void printScannedOverview(Descriptor descriptor, StatsMetadata stats) throws IOException {
    ...
    SSTableReader reader =
        SSTableReader.openNoValidation(null, descriptor, TableMetadataRef.forOfflineTools(cfm));
    try (ISSTableScanner scanner = reader.getScanner()) {
      long bytes = scanner.getLengthInBytes();
      MinMaxPriorityQueue<ValuedByteBuffer> widestPartitions =
          MinMaxPriorityQueue.orderedBy(VCOMP).maximumSize(5).create();
      MinMaxPriorityQueue<ValuedByteBuffer> largestPartitions =
          MinMaxPriorityQueue.orderedBy(VCOMP).maximumSize(5).create();
      MinMaxPriorityQueue<ValuedByteBuffer> mostTombstones =
          MinMaxPriorityQueue.orderedBy(VCOMP).maximumSize(5).create();
  ...
  private void printSStableMetadata(File file, boolean scan) throws IOException {
    ...
    if (stats != null) {
      ...
      field(
          ...
          Long.toString(stats.minTimestamp));
      field(
          "Maximum timestamp",
          toDateString(stats.maxTimestamp, tsUnit),
          Long.toString(stats.maxTimestamp));
      ...
      if (compressorClass != null) field("Compression ratio", stats.compressionRatio);
      ...
      if (validation != null && header != null)
        printMinMaxToken(
            descriptor, FBUtilities.newPartitioner(descriptor), header.getKeyType(), stats);
      ...
      TermHistogram estDropped =
          new TermHistogram(
              ...
              offset ->
                  String.format(
                      "%d %s",
                      offset, Util.wrapQuiet(toDateString(offset, TimeUnit.SECONDS), color)),
              String::valueOf);
      estDropped.printHistogram(out, color, unicode);
      field("Partition Size", "");
      TermHistogram rowSize =
          new TermHistogram(
              stats.estimatedPartitionSize,
              "Size (bytes)",
              offset -> String.format("%d %s", offset, Util.wrapQuiet(toByteString(offset), color)),
              String::valueOf);
      rowSize.printHistogram(out, color, unicode);
      field("Column Count", "");
      TermHistogram cellCount =
          new TermHistogram(
              stats.estimatedCellPerPartitionCount, "Columns", String::valueOf, String::valueOf);
    ...
    if (header != null) {
      ...
      Map<String, String> statics =
          staticColumns.entrySet().stream()
              .collect(
                  Collectors.toMap(
                      e -> UTF8Type.instance.getString(e.getKey()), e -> e.getValue().toString()));
      ...
      if (stats != null) field("IsTransient", stats.isTransient);
    }
  }
  ...
  private static void printUsage() {
    try (PrintWriter errWriter = new PrintWriter(System.err, true)) {
      ...
      formatter.printHelp(
          errWriter,
          120,
          "sstablemetadata <options> <sstable...>",
  ...
  private void printMinMaxToken(
    ...
    if (descriptor.version.hasKeyRange()) {
      if (statsMetadata.firstKey == null || statsMetadata.lastKey == null) return;

      field(
          "First token",
          partitioner.getToken(statsMetadata.firstKey),
          keyType.getString(statsMetadata.firstKey));
      field(
          "Last token",
          partitioner.getToken(statsMetadata.lastKey),
          keyType.getString(statsMetadata.lastKey));
    } else {
      Pair<DecoratedKey, DecoratedKey> firstLast =
          descriptor.getFormat().getReaderFactory().readKeyRange(descriptor, partitioner);
  ...
  public static void main(String[] args) throws IOException {
    ...
    Option unicode =
        new Option(UNICODE, "unicode", false, "Use unicode to draw histograms and progress bars");
    ...
    Option scanEnabled =
        new Option(
            ...
            "scan",
            false,
            "Full sstable scan for additional details. Only available in 3.0+ sstables. Defaults:"
                + " false");
    ...
    SSTableMetadataViewer metawriter =
        new SSTableMetadataViewer(enabledColors, enabledUnicode, gc, ts, System.out);
