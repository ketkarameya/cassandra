...
public class SSTableExport {
  ...
  static {
    ...
    Option partitionJsonLines =
        new Option(PARTITION_JSON_LINES, false, "Output json lines, by partition");
  ...
  public static void main(String[] args) throws ConfigurationException {
    ...
    if (cmd.getArgs().length != 1) {
      ...
      if (cmd.getArgs().length == 0 && (keys != null && keys.length > 0 || !excludes.isEmpty()))
        msg +=
            ", which should be before the -k/-x options so it's not interpreted as a partition"
                + " key.";
    ...
    try {
      ...
      SSTableReader sstable =
          SSTableReader.openNoValidation(null, desc, TableMetadataRef.forOfflineTools(metadata));
      if (cmd.hasOption(ENUMERATE_KEYS_OPTION)) {
        try (KeyIterator iter = sstable.keyIterator()) {
          JsonTransformer.keysToJson(
              null, Util.iterToStream(iter), cmd.hasOption(RAW_TIMESTAMPS), metadata, System.out);
        }
      } else {
        ...
        Stream<UnfilteredRowIterator> partitions =
            Util.iterToStream(currentScanner)
                .filter(
                    i ->
                        excludes.isEmpty()
                            || !excludes.contains(
                                metadata.partitionKeyType.getString(i.partitionKey().getKey())));
              ...
              partition -> {
                ...
                partition.forEachRemaining(
                    row -> {
                      System.out.println(
                          "["
                              + metadata.partitionKeyType.getString(
