...
/**
 * A singleton-ish wrapper over various hints components: - a catalog of all hints stores - a
 * single-threaded write executor - a multi-threaded dispatch executor - the buffer pool for writing
 * hints into - an optional scheduled task to clean up the applicable hints files
 *
 * <p>The front-end for everything hints related.
 */
public final class HintsService implements HintsServiceMBean {
  ...
  HintsService(IFailureDetector failureDetector) {
    ...
    dispatchExecutor =
        new HintsDispatchExecutor(
            hintsDirectory, maxDeliveryThreads, isDispatchPaused, failureDetector::isAlive);

    // periodically empty the current content of the buffers
    int flushPeriod = DatabaseDescriptor.getHintsFlushPeriodInMS();
    triggerFlushingFuture =
    ...
    triggerCleanupFuture =
        ScheduledExecutors.optionalTasks.scheduleWithFixedDelay(
            cleanupTrigger, 1, 1, TimeUnit.HOURS);
  ...
  public void write(Collection<UUID> hostIds, Hint hint) {
    ...
    // we have to make sure that the HintsStore instances get properly initialized - otherwise
    // dispatch will not trigger
  ...
  /**
   * Write a hint for all replicas. Used to re-dispatch hints whose destination is either missing or
   * no longer correct.
   */
  void writeForAllReplicas(Hint hint) {
    ...
    EndpointsForToken replicas =
        ReplicaLayout.forTokenWriteLiveAndDown(Keyspace.open(keyspaceName), token).all();
  ...
  public void flushAndFsyncBlockingly(Iterable<UUID> hostIds) {
    Iterable<HintsStore> stores =
        filter(transform(hostIds, catalog::getNullable), Objects::nonNull);
  ...
  public synchronized void startDispatch() {
    ...
    HintsDispatchTrigger trigger =
        new HintsDispatchTrigger(catalog, writeExecutor, dispatchExecutor, isDispatchPaused);
    // triggering hint dispatch is now very cheap, so we can do it more often - every 10 seconds vs.
    // every 10 minutes,
    // previously; this reduces mean time to delivery, and positively affects batchlog delivery
    // latencies, too
    triggerDispatchFuture =
        ScheduledExecutors.scheduledTasks.scheduleWithFixedDelay(trigger, 10, 10, TimeUnit.SECONDS);
  }
   ...
   *
   * <p>Will abort dispatch sessions that are currently in progress (which is okay, it's
   * idempotent), and make sure the buffers are flushed, hints files written and fsynced.
   */
   ...
   * Returns all pending hints that this node has.
   *
   * @return a list of maps with endpoints' ids, total number of hint files, their oldest and newest
   *     timestamps.
   */
  public List<Map<String, String>> getPendingHints() {
    return getPendingHintsInfo().stream().map(PendingHintsInfo::asMap).collect(Collectors.toList());
  }
  ...
  /**
   * Deletes all hints for the provided destination. Doesn't make snapshots - should be used with
   * care.
   *
   ...
   *
   * <p>Dispatcher can not stop itself (isHostAlive() can not start returning false for the leaving
   * host because this method is called by the same thread as gossip, which blocks gossip), so we
   * can't simply wait for completion.
   *
   * <p>We should also flush the buffer if there are any hints for the node there, and close the
   * writer (if any), so that we don't leave any hint files lying around.
   *
   * <p>Once that is done, we can simply delete all hint files and remove the host id from the
   * catalog.
   *
   * <p>The worst that can happen if we don't get everything right is a hints file (or two)
   * remaining undeleted.
   *
  ...
  public void excise(UUID hostId) {
    ...
    // flush the buffer and then close the writer for the excised host id, to make sure that no new
    // files will appear
    ...
    // interrupt the current dispatch session to end (if any), so that the currently dispatched file
    // gets removed
   ...
   *
   * <p>Flushes the buffer to make sure all hints are on disk and closes the hint writers so we
   * don't leave any hint files around.
   *
   * <p>After that, we serially dispatch all the hints in the HintsCatalog.
   *
   * <p>If we fail delivering all hints, we will ask the hostIdSupplier for a new target host and
   * retry delivering any remaining hints there, once, with a delay of 10 seconds before retrying.
   *
   * @param hostIdSupplier supplier of stream target host ids. This is generally the closest one
   *     according to the DynamicSnitch
   * @return When this future is done, it either has streamed all hints to remote nodes or has
   *     failed with a proper log message
   */
  ...
  /**
   * Find the oldest hint written for a particular node by looking into descriptors and current open
   * writer, if any.
   *
