...
/**
 * Coordinator side logic and state of a consistent repair session. Like {@link
 * ActiveRepairService.ParentRepairSession}, there is only one {@code CoordinatorSession} per user
 * repair command, regardless of the number of tables and token ranges involved.
 */
public class CoordinatorSession extends ConsistentSession {
  ...
  public synchronized void setParticipantState(InetAddressAndPort participant, State state) {
    ...
    Preconditions.checkArgument(
        participantStates.containsKey(participant),
        "Session %s doesn't include %s",
        sessionID,
        participant);
    Preconditions.checkArgument(
        participantStates.get(participant).canTransitionTo(state),
        "Invalid state transition %s -> %s",
        participantStates.get(participant),
        state);
  ...
  public Future<Void> prepare() {
    ...
    for (final InetAddressAndPort participant : participants) {
      sendMessageWithRetries(
          ctx, notDone(prepareFuture), request, Verb.PREPARE_CONSISTENT_REQ, participant);
    }
  ...
  public synchronized void handlePrepareResponse(Message<PrepareConsistentResponse> msg) {
    ...
    if (getState() == State.FAILED) {
      logger.trace(
          "Incremental repair {} has failed, ignoring prepare response from {}",
          sessionID,
          participant);
    ...
    sendAck(ctx, msg);
    if (getParticipantState(participant) != State.PREPARING) return;
    if (!success) {
      logger.warn(
      ...
      logger.trace(
          "Successful prepare response received from {} for repair session {}",
          participant,
          sessionID);
    ...
    if (Iterables.any(participantStates.values(), v -> v == State.PREPARING)) return;
  ...
  public synchronized Future<Void> finalizePropose() {
    ...
    for (final InetAddressAndPort participant : participants) {
      sendMessageWithRetries(
          ctx, notDone(finalizeProposeFuture), request, Verb.FINALIZE_PROPOSE_MSG, participant);
    }
  ...
  public synchronized void handleFinalizePromise(Message<FinalizePromise> message) {
    ...
    if (getState() == State.FAILED) {
      logger.trace(
          "Incremental repair {} has failed, ignoring finalize promise from {}",
          sessionID,
          participant);
    ...
    if (getParticipantState(participant) != State.REPAIRING) {
      // this message is a retry, or we failed the session; in either case there is nothing more to
      // do than ack
      return;
    } else if (!success) {
      logger.warn(
          "Finalization proposal of session {} rejected by {}. Aborting session",
          sessionID,
          participant);
      fail();
    } else {
      logger.trace(
          "Successful finalize promise received from {} for repair session {}",
          participant,
          sessionID);
  ...
  /**
   * Runs the asynchronous consistent repair session. Actual repair sessions are scheduled via a
   * submitter to make unit testing easier
   */
  public Future<CoordinatedRepairResult> execute(
      Supplier<Future<CoordinatedRepairResult>> sessionSubmitter) {
    ...
    Future<CoordinatedRepairResult> repairSessionResults =
        prepareResult.flatMap(
            ignore -> {
              repairStart = ctx.clock().currentTimeMillis();
              if (logger.isDebugEnabled())
                logger.debug(
                    "Incremental repair {} prepare phase completed in {}",
                    sessionID,
                    formatDuration(sessionStart, repairStart));
              setRepairing();
              return sessionSubmitter.get();
            });

    // if any session failed, then fail the future
    Future<CoordinatedRepairResult> onlySuccessSessionResults =
        repairSessionResults.flatMap(
            result -> {
              finalizeStart = ctx.clock().currentTimeMillis();
              if (result.hasFailed()) {
                if (logger.isDebugEnabled())
                  logger.debug(
                      "Incremental repair {} validation/stream phase completed in {}",
                      sessionID,
                      formatDuration(repairStart, finalizeStart));
    ...
    Future<CoordinatedRepairResult> proposeFuture =
        onlySuccessSessionResults.flatMap(
            results ->
                finalizePropose()
                    .map(
                        ignore -> {
                          if (logger.isDebugEnabled())
                            logger.debug(
                                "Incremental repair {} finalization phase completed in {}",
                                sessionID,
                                formatDuration(finalizeStart, ctx.clock().currentTimeMillis()));
                          finalizeCommit();
                          if (logger.isDebugEnabled())
                            logger.debug(
                                "Incremental repair {} phase completed in {}",
                                sessionID,
                                formatDuration(sessionStart, ctx.clock().currentTimeMillis()));
                          return results;
                        }));

    return proposeFuture.addCallback(
        (ignore, failure) -> {
          if (failure != null) {
            if (logger.isDebugEnabled())
              logger.debug(
                  "Incremental repair {} phase failed in {}",
                  sessionID,
                  formatDuration(sessionStart, ctx.clock().currentTimeMillis()));
            fail();
          }
