...
public class ParticipateState extends AbstractCompletable<TimeUUID> {
  ...
  public String toString() {
    ...
    return "ParticipateState{"
        ...
        + initiator
        + ", status="
        + (result == null ? "pending" : result.toString())
        + ", jobs="
        + jobs.values()
        + '}';
  }
  ...
  public static class Job extends AbstractState<Job.State, RepairJobDesc> {
    ...
    public RegisterStatus register(ValidationState state) {
      return register(
          s -> validation.compareAndSet(null, s) ? null : validation(), State.VALIDATION, state);
    }
    ...
    private <I, S extends AbstractState<?, I>> RegisterStatus register(
        Function<S, S> putter, State state, S value) {
