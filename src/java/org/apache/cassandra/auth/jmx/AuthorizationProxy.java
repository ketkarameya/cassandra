...
/**
 * Provides a proxy interface to the platform's MBeanServer instance to perform role-based
 * authorization on method invocation.
 *
 * <p>When used in conjunction with a suitable JMXAuthenticator, which attaches a CassandraPrincipal
 * to authenticated Subjects, this class uses the configured IAuthorizer to verify that the subject
 * has the required permissions to execute methods on the MBeanServer and the MBeans it manages.
 *
 * <p>Because an ObjectName may contain wildcards, meaning it represents a set of individual MBeans,
 ...
 * rather than pushing that down to the configured IAuthorizer. To that end, during authorization it
 * pulls back all permissions for the active subject, filtering them to retain only grants on
 ...
 *
 * <p>Methods are split into 2 categories; those which are to be invoked on the MBeanServer itself
 ...
 * those methods which take an ObjectName as their first argument and those which do not. Invoking a
 * method of the former type, e.g. MBeanServer::getAttribute(ObjectName name, String attribute),
 * implies that the caller is concerned with a specific MBean. Conversely, invoking a method such as
 * MBeanServer::getDomains is primarily a function of the MBeanServer itself. This class makes such
 * a distinction in order to identify which JMXResource the subject requires permissions on.
 *
 * <p>Certain operations are never allowed for users and these are recorded in a deny list so that
 * we can short circuit authorization process if one is attempted by a remote subject.
 */
public class AuthorizationProxy implements InvocationHandler {
  ...
  */
  ...
  public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
    ...
    if (methodName.equals("invoke") && args.length == 4) checkVulnerableMethods(args);
  ...
  /**
   * Performs the actual authorization of an identified subject to execute a remote method
   * invocation.
   *
   * @param subject The principal making the execution request. A null value represents a local
   *     invocation from the JMX connector itself
   * @param methodName Name of the method being invoked
   * @param args Array containing invocation argument. If the first element is an ObjectName
   *     instance, for authz purposes we consider this an invocation of an MBean method, otherwise
   *     it is treated as an invocation of a method on the MBeanServer.
   */
  @VisibleForTesting
  ...
  /**
   * Authorize execution of a method on the MBeanServer which does not take an MBean ObjectName as
   * its first argument. The allowed methods that match this criteria are generally descriptive
   * methods concerned with the MBeanServer itself, rather than with any particular set of MBeans
   * managed by the server and so we check the DESCRIBE permission on the root JMXResource
   * (representing the MBeanServer)
   *
   * @param subject
   * @param methodName
  ...
  /**
   * Authorize execution of a method on an MBean (or set of MBeans) which may be managed by the
   * MBeanServer. Note that this also includes the queryMBeans and queryNames methods of MBeanServer
   * as those both take an ObjectName (possibly a pattern containing wildcards) as their first
   * argument. They both of those methods also accept null arguments, in which case they will be
   * handled by authorizedMBeanServerMethod
   *
   * @param role
   * @param methodName
   * @param args
  ...
  /**
   * Get any grants of the required permission for the authenticated subject, regardless of the
   * resource the permission applies to as we'll do the filtering & matching in the calling method
   *
   * @param subject
   * @param required
  ...
  /**
   * Check whether a required permission has been granted to the authenticated subject on a specific
   * resource
   *
   * @param subject
   * @param permission
   * @param resource
   * @return true if the Subject has been granted the required permission on the specified resource;
   *     false otherwise
   */
  private boolean hasPermission(RoleResource subject, Permission permission, JMXResource resource) {
    return getPermissions.apply(subject).stream()
   ...
   * invocation. At this point, we are sure that whatever the required permission, the Subject has
   * definitely been granted it against this set of JMXResources. The job of this method is only to
   * verify that the target of the invocation is covered by the members of the set.
   *
   * @param target
  ...
  private boolean checkPattern(ObjectName target, Set<JMXResource> permittedResources) {
    ...
    for (JMXResource resource : permittedResources) {
      try {
        Set<ObjectName> matchingNames =
            queryNames.apply(ObjectName.getInstance(resource.getObjectName()));
  ...
  /**
   * Given a set of JMXResources upon which the Subject has been granted a particular permission,
   * check whether any match the ObjectName representing the target of the method invocation. At
   * this point, we are sure that whatever the required permission, the Subject has definitely been
   * granted it against this set of JMXResources. The job of this method is only to verify that the
   * target of the invocation is matched by a member of the set.
   *
   * @param target
   * @param permittedResources
  ...
  /**
   * Mapping between method names and the permission required to invoke them. Note, these names
   * refer to methods on MBean instances invoked via the MBeanServer.
   *
   * @param methodName
   * @return
   */
  ...
  /**
   * Invoke a method on the MBeanServer instance. This is called when authorization is not required
   * (because AllowAllAuthorizer is configured, or because the invocation is being performed by the
   * JMXConnector itself rather than by a connected client), and also when a call from an
   * authenticated subject has been successfully authorized
   *
   * @param method
   * @param args
   * @return
    ...
    try {
      return method.invoke(mbs, args);
        ...
        InvocationTargetException
            e) // Catch any exception that might have been thrown by the mbeans
    {
  ...
  /**
   * Query the configured IAuthorizer for the set of all permissions granted on JMXResources to a
   * specific subject
   *
   * @param subject
   * @return All permissions granted to the specfied subject (including those transitively inherited
   *     from any roles the subject has been granted), filtered to include only permissions granted
   *     on JMXResources
   */
  ...
  private void checkVulnerableMethods(Object args[]) {
    ...
    try {
      ...
      logger.warn(
          "Could not interpret arguments to check vulnerable MBean invocations; did the MBeanServer"
              + " interface change?",
          cce);
      return;
    }

    // When adding compiler directives from a file, most JDKs will log the file contents if invalid,
    // which
  ...
  private void checkJvmtiLoad(ObjectName name, String operation) {
    if (name.getCanonicalName().equals("com.sun.management:type=DiagnosticCommand")
        && operation.equals("jvmtiAgentLoad")) throw new SecurityException("Access is denied!");
  }

  private void checkMLetMethods(ObjectName name, String operation) {
    // Inspired by MBeanServerAccessController, but that class ignores check if a SecurityManager is
    // installed,
    // which we don't want
    ...
    try {
      if (!mbs.isInstanceOf(name, "javax.management.loading.MLet")) return;
  ...
  public static final class JmxPermissionsCache
      extends AuthCache<RoleResource, Set<PermissionDetails>> implements JmxPermissionsCacheMBean {
    ...
    protected void unregisterMBean() {
      ...
      MBeanWrapper.instance.unregisterMBean(
          MBEAN_NAME_BASE + DEPRECATED_CACHE_NAME, MBeanWrapper.OnException.LOG);
    }
  }
