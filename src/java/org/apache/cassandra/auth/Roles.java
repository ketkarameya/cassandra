...
public class Roles {
  ...
  private static final Role NO_ROLE =
      new Role("", false, false, Collections.emptyMap(), Collections.emptySet());
  ...
  /**
   * Identify all roles granted to the supplied Role, including both directly granted and inherited
   * roles. This method is used where we mainly just care about *which* roles are granted to a given
   * role, including when looking up or listing permissions for a role on a given resource.
   *
   ...
   * This includes superuser status and login privileges for the primary role and all roles granted
   * directly to it or inherited. The returned roles may be cached if roles_validity > 0 This method
   * is used where we need to know specific attributes of the collection of granted roles, i.e. when
   * checking for superuser status which may be inherited from *any* granted role.
   *
  ...
  /**
   * Enumerate all the roles in the system, preferably these will be fetched from the cache, which
   * in turn may have been warmed during startup.
   */
  ...
  /**
   * Returns true if the supplied role or any other role granted to it (directly or indirectly) has
   * superuser status.
   *
  ...
  /**
   * Returns true if the supplied role has the login privilege. This cannot be inherited, so returns
   * true iff the named role has that bit set.
   *
  ...
  public static boolean canLogin(final RoleResource role) {
    try {
      for (Role r : getRoleDetails(role)) if (r.resource.equals(role)) return r.canLogin;
  ...
  /**
   * Returns the map of custom options for the named role. These options are not inherited from
   * granted roles, but are set directly.
   *
   * @param role the role identifier
   * @return map of option_name -> value. If no options are set for the named role, the map will be
  ...
  /**
   * Return the NullObject Role instance which can be safely used to indicate no information is
   * available when querying for a specific named role.
   *
  ...
  /**
   * Just a convenience method which compares a role instance with the null object version,
   * indicating if the return from some query/lookup method was a valid Role or indicates that the
   * role does not exist.
   *
   * @param role
   ...
   * This is used by the default implementation of IRoleManager#getRoleDetails so that IRoleManager
   * impls which don't implement an optimized getRoleDetails remain compatible. Depending on the
   * IRoleManager implementation this could be quite heavyweight, so should not be used on any hot
   * path.
   *
   * @param resource identifies the role
   * @param roleManager provides lookup functions to retrieve role info
   * @return Role object including superuser status, login privilege, custom options and the set of
   *     roles granted to identified role.
   */
  public static Role fromRoleResource(RoleResource resource, IRoleManager roleManager) {
    return new Role(
        ...
        roleManager.getRoles(resource, false).stream()
