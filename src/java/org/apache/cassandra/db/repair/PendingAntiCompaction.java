...
/**
 * Performs an anti compaction on a set of tables and token ranges, isolating the unrepaired
 * sstables for a give token range into a pending repair group so they can't be compacted with other
 * sstables while they are being repaired.
 */
public class PendingAntiCompaction {
  ...
  private static final int ACQUIRE_RETRY_SECONDS =
      CassandraRelevantProperties.ACQUIRE_RETRY_SECONDS.getInt();
  ...
  static class AntiCompactionPredicate implements Predicate<SSTableReader> {
    ...
    public boolean apply(SSTableReader sstable) {
      if (!sstable.intersects(ranges)) return false;

      StatsMetadata metadata = sstable.getSSTableMetadata();

      // exclude repaired sstables
      if (metadata.repairedAt != UNREPAIRED_SSTABLE) return false;

      if (!sstable.descriptor.version.hasPendingRepair()) {
        String message =
            String.format(
                "Prepare phase failed because it encountered legacy sstables that don't "
                    + "support pending repair, run upgradesstables before starting incremental "
                    + "repairs, repair session (%s)",
                prsid);
      ...
      if (metadata.pendingRepair != NO_PENDING_REPAIR) {
        if (!ActiveRepairService.instance()
            .consistent
            .local
            .isSessionFinalized(metadata.pendingRepair)) {
          String message =
              String.format(
                  "Prepare phase for incremental repair session %s has failed because it"
                      ...
                      + " session (%s). This is caused by starting an incremental repair session"
                      + " before a previous one has completed. Check nodetool repair_admin for hung"
                      + " sessions and fix them.",
                  prsid, metadata.pendingRepair);
          throw new SSTableAcquisitionException(message);
        }
        return false;
      }
      Collection<CompactionInfo> cis =
          CompactionManager.instance.active.getCompactionsForSSTable(
              sstable, OperationType.ANTICOMPACTION);
      if (cis != null && !cis.isEmpty()) {
        // todo: start tracking the parent repair session id that created the anticompaction to be
        // able to give a better error messsage here:
        StringBuilder sb = new StringBuilder();
        sb.append("Prepare phase for incremental repair session ");
        sb.append(prsid);
        sb.append(
            " has failed because it encountered intersecting sstables belonging to another"
                + " incremental repair session. ");
        sb.append(
            "This is caused by starting multiple conflicting incremental repairs at the same"
                + " time. ");
        sb.append("Conflicting anticompactions: ");
        for (CompactionInfo ci : cis)
          sb.append(ci.getTaskId() == null ? "no compaction id" : ci.getTaskId())
              .append(':')
              .append(ci.getSSTables())
              .append(',');
  ...
  public static class AcquisitionCallable implements Callable<AcquireResult> {
    ...
    public AcquisitionCallable(
      ...
      this(
          ...
          acquireRetrySeconds,
          acquireSleepMillis,
          new AntiCompactionPredicate(ranges, sessionID));
    }
    ...
    AcquisitionCallable(
        ...
        int acquireRetrySeconds,
        int acquireSleepMillis,
        AntiCompactionPredicate predicate) {
      this.cfs = cfs;
    ...
    private AcquireResult acquireTuple() {
      ...
      try {
        ...
        if (sstables.isEmpty()) return new AcquireResult(cfs, null, null);

        LifecycleTransaction txn =
            cfs.getTracker().tryModify(sstables, OperationType.ANTICOMPACTION);
        if (txn != null) return new AcquireResult(cfs, Refs.ref(sstables), txn);
        else
          logger.error(
    ...
    protected AcquireResult acquireSSTables() {
      return cfs.runWithCompactionsDisabled(
          this::acquireTuple, predicate, OperationType.ANTICOMPACTION, false, false, false);
    }

    public AcquireResult call() {
      logger.debug("acquiring sstables for pending anti compaction on session {}", sessionID);
      // try to modify after cancelling running compactions. This will attempt to cancel in flight
      // compactions including the given sstables for
      // up to a minute, after which point, null will be returned
      long start = currentTimeMillis();
      long delay = TimeUnit.SECONDS.toMillis(acquireRetrySeconds);
      // Note that it is `predicate` throwing SSTableAcquisitionException if it finds a conflicting
      // sstable
      // and we only retry when runWithCompactionsDisabled throws when uses the predicate, not when
      // acquireTuple is.
      // This avoids the case when we have an sstable [0, 100] and a user starts a repair on [0, 50]
      // and then [51, 100] before
      // anticompaction has finished but not when the second repair is [25, 75] for example - then
      // we will fail it without retry.
      do {
        try {
          // Note that anticompactions are not disabled when running this. This is safe since
          // runWithCompactionsDisabled
          // is synchronized - acquireTuple and predicate can only be run by a single thread (for
          // the given cfs).
          return acquireSSTables();
        } catch (SSTableAcquisitionException e) {
          logger.warn(
              "Session {} failed acquiring sstables: {}, retrying every {}ms for another {}s",
              sessionID,
              e.getMessage(),
              acquireSleepMillis,
  ...
  static class AcquisitionCallback implements Function<List<AcquireResult>, Future<List<Void>>> {
    ...
    public AcquisitionCallback(
        TimeUUID parentRepairSession, RangesAtEndpoint tokenRanges, BooleanSupplier isCancelled) {
    ...
    Future<Void> submitPendingAntiCompaction(AcquireResult result) {
      return CompactionManager.instance.submitPendingAntiCompaction(
          result.cfs, tokenRanges, result.refs, result.txn, parentRepairSession, isCancelled);
    }

    private static boolean shouldAbort(AcquireResult result) {
      if (result == null) return true;

      // sstables in the acquire result are now marked compacting and are locked to this anti
      // compaction. If any
      // of them are marked repaired or pending repair, acquisition raced with another pending
      // anti-compaction, or
      // possibly even a repair session, and we need to abort to prevent sstables from moving
      // between sessions.
    ...
    public Future<List<Void>> apply(List<AcquireResult> results) {
      if (Iterables.any(results, AcquisitionCallback::shouldAbort)) {
        ...
        for (AcquireResult result : results) {
          if (result != null) {
            logger.info(
                "Releasing acquired sstables for {}.{}",
                result.cfs.metadata.keyspace,
                result.cfs.metadata.name);
            result.abort();
          }
        }
        String message =
            String.format(
                "Prepare phase for incremental repair session %s was unable to acquire exclusive"
                    + " access to the neccesary sstables. This is usually caused by running"
                    + " multiple incremental repairs on nodes that share token ranges",
                parentRepairSession);
        logger.warn(message);
  ...
  public PendingAntiCompaction(
    ...
    this(
        prsId, tables, tokenRanges, ACQUIRE_RETRY_SECONDS, ACQUIRE_SLEEP_MS, executor, isCancelled);
  }
  ...
  public Future<List<Void>> run() {
    ...
    for (ColumnFamilyStore cfs : tables) {
      ...
      FutureTask<AcquireResult> task =
          new FutureTask<>(
              getAcquisitionCallable(
                  cfs, tokenRanges.ranges(), prsId, acquireRetrySeconds, acquireSleepMillis));
  ...
  @VisibleForTesting
  protected AcquisitionCallable getAcquisitionCallable(
      ColumnFamilyStore cfs,
  ...
  @VisibleForTesting
  protected AcquisitionCallback getAcquisitionCallback(
      TimeUUID prsId, RangesAtEndpoint tokenRanges) {
