 ...
 *
 * <p>This corresponds to the restrictions on rows that are not handled by the query {@link
 * ClusteringIndexFilter}. Some of the expressions of this filter may be handled by a 2ndary index,
 * and the rest is simply filtered out from the result set (the later can only happen if the query
 * was using ALLOW FILTERING).
 */
public class RowFilter implements Iterable<RowFilter.Expression> {
  ...
  /**
   * @param needsReconciliation whether or not this filter belongs to a read that requires
  ...
  public void addCustomIndexExpression(
      TableMetadata metadata, IndexMetadata targetIndex, ByteBuffer value) {
  ...
  /**
   * If this filter belongs to a read that requires reconciliation at the coordinator, and it
   * contains an intersection on two or more non-key (and therefore mutable) columns, we cannot
   * strictly apply it to local, unrepaired rows. When this occurs, we must downgrade the
   * intersection of expressions to a union and leave the coordinator to filter strictly before
   ...
   *
   * @return {@code true} if some of the expressions apply to clustering or regular columns, {@code
   *     false} otherwise.
   */
  ...
  /**
   * Note that the application of this transformation does not yet take {@link #isStrict()} into
   * account. This means that even when strict filtering is not safe, expressions will be applied as
   * intersections rather than unions. The filter will always be evaluated strictly in conjunction
   * with replica filtering protection at the coordinator, however, even after CASSANDRA-19007 is
   * addressed.
   *
  ...
  protected Transformation<BaseRowIterator<?>> filter(TableMetadata metadata, long nowInSec) {
    ...
    return new Transformation<>() {
      ...
      protected BaseRowIterator<?> applyToPartition(BaseRowIterator<?> partition) {
        ...
        BaseRowIterator<?> iterator =
            partition instanceof UnfilteredRowIterator
      ...
      public Row applyToRow(Row row) {
        Row purged =
            row.purge(DeletionPurger.PURGE_ALL, nowInSec, metadata.enforceStrictLiveness());
        if (purged == null) return null;

        for (Expression e : rowLevelExpressions)
          if (!e.isSatisfiedBy(metadata, pk, purged)) return null;
  ...
  /**
   * Filters the provided iterator so that only the row satisfying the expression of this filter are
  ...
  public UnfilteredPartitionIterator filter(UnfilteredPartitionIterator iter, long nowInSec) {
    return expressions.isEmpty()
        ? iter
        : Transformation.apply(iter, filter(iter.metadata(), nowInSec));
  }
   ...
   * @return {@code true} if {@code row} in partition {@code partitionKey} satisfies this row
   *     filter.
   */
  public boolean isSatisfiedBy(
      TableMetadata metadata, DecoratedKey partitionKey, Row row, long nowInSec) {
  ...
  /**
   * Returns true if all of the expressions within this filter that apply to the partition key are
   * satisfied by the given key, false otherwise.
   */
  public boolean partitionKeyRestrictionsAreSatisfiedBy(
      DecoratedKey key, AbstractType<?> keyValidator) {
    for (Expression e : expressions) {
  ...
  /**
   * Returns true if all of the expressions within this filter that apply to the clustering key are
   * satisfied by the given Clustering, false otherwise.
   */
  public boolean clusteringKeyRestrictionsAreSatisfiedBy(Clustering<?> clustering) {
    for (Expression e : expressions) {
      ...
      if (!e.operator()
          .isSatisfiedBy(e.column.type, clustering.bufferAt(e.column.position()), e.value)) {
        return false;
      }
    }
    return true;
  ...
  /**
   * Returns this filter but without the provided expression. This method *assumes* that the filter
   * contains the provided expression.
   */
  ...
  /**
   * Returns a copy of this filter but without the provided expression. If this filter doesn't
   * contain the specified expression this method will just return an identical copy of this filter.
   */
  ...
  public abstract static class Expression {
    ...
    /**
     * If this expression is used to query an index, the value to use as partition key for that
     * index query.
     */
    ...
    public void validateForIndexing() {
      checkFalse(
          value.remaining() > FBUtilities.MAX_UNSIGNED_SHORT,
     ...
     * @param row the row to check. It should *not* contain deleted cells (i.e. it should come from
     *     a RowIterator).
     * @return whether the row is satisfied by this expression.
     */
    public abstract boolean isSatisfiedBy(
        TableMetadata metadata, DecoratedKey partitionKey, Row row);
    ...
    private static class Serializer {
      public void serialize(Expression expression, DataOutputPlus out, int version)
          throws IOException {
        ...
        if (expression.kind() == Kind.CUSTOM) {
          IndexMetadata.serializer.serialize(
              ((CustomExpression) expression).targetIndex, out, version);
      ...
      public Expression deserialize(DataInputPlus in, int version, TableMetadata metadata)
          throws IOException {
        ...
        if (kind == Kind.USER) return UserExpression.deserialize(in, version, metadata);

        ByteBuffer name = ByteBufferUtil.readWithShortLength(in);
        Operator operator = Operator.readFrom(in);
        ColumnMetadata column = metadata.getColumn(name);

        // Compact storage tables, when used with thrift, used to allow falling through this
        // withouot throwing an
        // exception. However, since thrift was removed in 4.0, this behaviour was not restored in
        // CASSANDRA-16217
        if (column == null)
          throw new RuntimeException(
              "Unknown (or dropped) column "
      ...
      public long serializedSize(Expression expression, int version) {
        ...
        switch (expression.kind()) {
          ...
          case CUSTOM:
            size +=
                IndexMetadata.serializer.serializedSize(
                        ((CustomExpression) expression).targetIndex, version)
    ...
    public boolean isSatisfiedBy(TableMetadata metadata, DecoratedKey partitionKey, Row row) {
      ...
      if (operator.appliesToColumnValues()) {
        assert !column.isComplex()
            : "Only CONTAINS and CONTAINS_KEY are supported for 'complex' types";

        // In order to support operators on Counter types, their value has to be extracted from
        // internal
        ...
        if (column.type.isCounter()) {
          ...
          ByteBuffer counterValue =
              LongType.instance.decompose(
                  CounterContext.instance().total(foundValue, ByteBufferAccessor.instance));
          return operator.isSatisfiedBy(LongType.instance, counterValue, value);
        } else {
          // Note that CQL expression are always of the form 'x < 4', i.e. the tested value is on
          // the left.
          ByteBuffer foundValue = getValue(metadata, partitionKey, row);
      ...
      if (operator.isTernary()) {
        ...
        return cql
            ? String.format(
                ...
                operator,
                elementType.toCQLString(buffers.get(0)),
                elementType.toCQLString(buffers.get(1)))
            : String.format(
                "%s %s %s AND %s",
                column.name.toString(),
                operator,
                elementType.getString(buffers.get(0)),
  ...
  /**
   * An expression of the form 'column' ['key'] = 'value' (which is only supported when 'column' is
   * a map).
   */
  private static class MapElementExpression extends Expression {
    private final ByteBuffer key;

    public MapElementExpression(
        ColumnMetadata column, ByteBuffer key, Operator operator, ByteBuffer value) {
      super(column, operator, value);
    ...
    public boolean isSatisfiedBy(TableMetadata metadata, DecoratedKey partitionKey, Row row) {
      ...
      if (row.isStatic() != column.isStatic()) return true;
      ...
      if (column.isComplex()) {
        ...
        if (serializedMap == null) return false;
    ...
    protected String toString(boolean cql) {
      ...
      return cql
          ? String.format(
              "%s[%s] = %s", column.name.toCQLString(), nt.toCQLString(key), vt.toCQLString(value))
          : String.format(
              "%s[%s] = %s", column.name.toString(), nt.getString(key), vt.getString(value));
    }
    ...
    public boolean equals(Object o) {
      ...
      if (!(o instanceof MapElementExpression)) return false;
  ...
  /**
   * A custom index expression for use with 2i implementations which support custom syntax and which
   * are not necessarily linked to a single column in the base table.
   */
  public static final class CustomExpression extends Expression {
    ...
    private static ColumnMetadata makeDefinition(TableMetadata table, IndexMetadata index) {
      // Similarly to how we handle non-defined columns in thift, we create a fake column definition
      // to
      // represent the target index. This is definitely something that can be improved though.
      return ColumnMetadata.regularColumn(
          table, ByteBuffer.wrap(index.name.getBytes()), BytesType.instance);
    }
   ...
   * to the classpath is a task for operators (needless to say, this is something of a power user
   * feature). Care must also be taken to register implementations, via the static register method
   * during system startup. An implementation and its corresponding Deserializer must be registered
   * before sending or receiving any messages containing expressions of that type. Use of custom
   * filtering expressions in a mixed version cluster should be handled with caution as the order in
   * which types are registered is significant: if continuity of use during upgrades is important,
   * new types should registered last and obsoleted types should still be registered ( or dummy
   * implementations registered in their place) to preserve consistent identifiers across the
   * cluster).
   *
   * <p>During serialization, the identifier for the Deserializer implementation is prepended to the
   ...
   */
  public abstract static class UserExpression extends Expression {
    ...
    private static final class DeserializerRegistry {
      ...
      public void registerUserExpressionClass(
          Class<? extends UserExpression> expressionClass,
          UserExpression.Deserializer deserializer) {
        int id =
            registeredClasses.computeIfAbsent(expressionClass, (cls) -> counter.getAndIncrement());
        deserializers.put(id, deserializer);

        logger.debug(
            "Registered user defined expression type {} and serializer {} with identifier {}",
    ...
    protected abstract static class Deserializer {
      protected abstract UserExpression deserialize(
          DataInputPlus in, int version, TableMetadata metadata) throws IOException;
    }

    public static void register(
        Class<? extends UserExpression> expressionClass, Deserializer deserializer) {
      deserializers.registerUserExpressionClass(expressionClass, deserializer);
    }

    private static UserExpression deserialize(DataInputPlus in, int version, TableMetadata metadata)
        throws IOException {
      int id = in.readInt();
    ...
    private static void serialize(UserExpression expression, DataOutputPlus out, int version)
        throws IOException {
    ...
    private static long serializedSize(
        UserExpression expression, int version) { // 4 bytes for the expression type id
  ...
  public static class Serializer {
    ...
    public RowFilter deserialize(
        DataInputPlus in, int version, TableMetadata metadata, boolean needsReconciliation)
        throws IOException {
