...
/**
 * We model the graph like this: * Each range we are about to stream is a vertex in the graph * Each
 * node that can provide a range is a vertex in the graph * We add an edge from each range to the
 * node that can provide the range * Then, to be able to solve the maximum flow problem using
 * Ford-Fulkerson we add a super source with edges to all range vertices and a super sink with
 * incoming edges from all the node vertices. * The capacity on the edges between the super source
 * and the range-vertices is 1 * The capacity on the edges between the range-vertices and the node
 * vertices is infinite * The capacity on the edges between the nodes-vertices and the super sink is
 * ceil(#range-vertices/#node-vertices) - if we have more machines than ranges to stream the
 * capacity will be 1 (each machine will stream at most 1 range) * Since the sum of the capacity on
 * the edges from the super source to the range-vertices is less or equal to the sum of the
 * capacities between the node-vertices and super sink we know that to get maximum flow we will use
 * all the range-vertices. (Say we have x ranges, y machines to provide them, total supersource ->
 * range-vertice capacity will be x, total node-vertice -> supersink capacity will be (y * ceil(x /
 * y)) which worst case is x if x==y). The capacity between the range-vertices and node-vertices is
 * infinite. * Then we try to solve the max-flow problem using psjava * If we can't find a solution
 * where the total flow is = number of range-vertices, we bump the capacity between the
 * node-vertices and the super source and try again.
 */
public class RangeFetchMapCalculator {
  ...
  public RangeFetchMapCalculator(
  ...
  Multimap<InetAddressAndPort, Range<Token>> getRangeFetchMapForNonTrivialRanges() {
    ...
    while (flow < getTotalRangeVertices(graph)) {
      if (flow > 0) {
        // We could not find a path with previous graph. Bump the capacity b/w endpoint vertices and
        // destination by 1
      ...
      result =
          fordFulkerson.calc(
              graph, sourceVertex, destinationVertex, IntegerNumberSystem.getInstance());
  ...
  @VisibleForTesting
  Multimap<InetAddressAndPort, Range<Token>> getRangeFetchMapForTrivialRanges(
      Multimap<InetAddressAndPort, Range<Token>> optimisedMap) {
    Multimap<InetAddressAndPort, Range<Token>> fetchMap = HashMultimap.create();
      ...
      while (!added) {
        ...
        for (Replica replica : replicas) {
          if (passFilters(replica, localDCCheck)) {
            added = true;
            // if we pass filters, it means that we don't filter away localhost and we can count it
            // as a source,
            // see RangeFetchMapCalculator#addEndpoints  and RangeStreamer#getRangeFetchMap
            if (replica.isSelf()) continue; // but don't add localhost to avoid streaming locally
        ...
        if (!added)
          logger.info(
              "Using other DC endpoints for streaming for range: {} and keyspace {}",
              trivialRange,
              keyspace);
        localDCCheck = false;
      }
  ...
  */
  ...
  /**
   * Convert the max flow graph to {@code Multimap<InetAddress, Range<Token>>} We iterate over all
   * range vertices and find an edge with flow of more than zero connecting to endpoint vertex.
   *
   * @param graph The graph to convert
   * @param result Flow algorithm result
   * @return Multi Map of Machine to Ranges
   */
  private Multimap<InetAddressAndPort, Range<Token>> getRangeFetchMapFromGraphResult(
      MutableCapacityGraph<Vertex, Integer> graph,
        ...
        for (CapacityEdge<Vertex, Integer> e : graph.getEdges(vertex)) {
          if (flowFunction.get(e) > 0) {
            ...
            if (e.to().isEndpointVertex())
              rangeFetchMapMap.put(
                  ((EndpointVertex) e.to()).getEndpoint(), ((RangeVertex) vertex).getRange());
            else if (e.from().isEndpointVertex())
              rangeFetchMapMap.put(
                  ((EndpointVertex) e.from()).getEndpoint(), ((RangeVertex) vertex).getRange());
          }
        }
  ...
  private void incrementCapacity(
      MutableCapacityGraph<Vertex, Integer> graph, int incrementalCapacity) {
    for (Vertex vertex : graph.getVertices()) {
   ...
   * Also add edges b/w endpoint vertices and destination vertex with capacity of
   * 'destinationCapacity'
   *
   * @param graph Graph to work on
   * @param destinationCapacity The capacity for edges b/w endpoint vertices and destination
   */
  private void addSourceAndDestination(
      MutableCapacityGraph<Vertex, Integer> graph, int destinationCapacity) {
  ...
  /**
   * Find the initial capacity which we want to use b/w machine vertices and destination to keep
   * things optimal
   *
   * @param graph Graph to work on
  ...
  /**
   * Generate a graph with all ranges and endpoints as vertices. It will create edges b/w a range
   * and its filtered source endpoints It will try to use sources from local DC if possible
   *
  ...
  /**
   * Create edges with infinite capacity b/w range vertex and all its source endpoints which clear
   * the filters
   *
   ...
   * @return If we were able to add atleast one source for this range after applying filters to
   *     endpoints
   */
  private boolean addEndpoints(
      MutableCapacityGraph<Vertex, Integer> capacityGraph,
      RangeVertex rangeVertex,
      boolean localDCCheck) {
    boolean sourceFound = false;
    ...
    for (Replica replica : rangesWithSources.get(rangeVertex.getRange())) {
      if (passFilters(replica, localDCCheck)) {
        sourceFound = true;
        // if we pass filters, it means that we don't filter away localhost and we can count it as a
        // source:
        if (replica.isSelf())
  ...
  private boolean isInLocalDC(Replica replica) {
    return DatabaseDescriptor.getLocalDataCenter()
        .equals(DatabaseDescriptor.getEndpointSnitch().getDatacenter(replica));
  }
