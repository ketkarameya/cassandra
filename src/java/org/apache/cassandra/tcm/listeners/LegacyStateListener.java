...
public class LegacyStateListener implements ChangeListener.Async {
  ...
  public void notifyPostCommit(ClusterMetadata prev, ClusterMetadata next, boolean fromSnapshot) {
    if (!fromSnapshot
        ...
        && next.tokenMap.lastModified().equals(prev.tokenMap.lastModified())) return;

    Set<InetAddressAndPort> removedAddr =
        Sets.difference(
            new HashSet<>(prev.directory.allAddresses()),
    ...
    for (NodeId node : next.directory.peerIds()) {
      if (directoryEntryChangedFor(node, prev.directory, next.directory)
          || !prev.tokenMap.tokens(node).equals(next.tokenMap.tokens(node))) changed.add(node);
    }
    ...
    for (NodeId change : changed) {
      ...
      if (next.myNodeId() != null && next.myNodeId().equals(change)) {
        switch (next.directory.peerState(change)) {
          ...
          case JOINED:
            SystemKeyspace.updateTokens(
                next.directory.endpoint(change), next.tokenMap.tokens(change));
        ...
        // Maybe intitialise local epstate whatever the node state because we could be processing
        // after a
        // replay and so may have not seen any previous local states, making this the first mutation
        // of gossip
        // state for the local node.
        Gossiper.instance.maybeInitializeLocalState(SystemKeyspace.incrementAndGetGeneration());
        Gossiper.instance.addLocalApplicationState(
            SCHEMA, StorageService.instance.valueFactory.schema(next.schema.getVersion()));
      }
      ...
      if (next.directory.peerState(change) == LEFT) {
        ...
        // As these are not yet added to the token map they must be extracted from the in progress
        // sequence.
        ...
        if (sequence != null && sequence.kind() == MultiStepOperation.Kind.REPLACE) {
          ...
          logger.info(
              "Node {} will complete replacement of {} for tokens {}",
              replacement,
              replaced,
              tokens);
          if (!replacement.equals(replaced)) {
            for (Token token : tokens)
              logger.warn(
                  "Token {} changing ownership from {} to {}", token, replaced, replacement);
          }
          Gossiper.instance.mergeNodeToGossip(change, next, tokens);
        }
      } else {
