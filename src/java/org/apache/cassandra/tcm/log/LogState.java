...
public class LogState {
  ...
  public static final IVersionedSerializer<LogState> defaultMessageSerializer =
      new MessageSerializer(NodeVersion.CURRENT.serializationVersion());
  ...
  public static IVersionedSerializer<LogState> messageSerializer(Version version) {
    ...
    if (cached != null && cached.serializationVersion.equals(version)) return cached;
  ...
  // Uses Replication rather than an just a list of entries primarily to avoid duplicating the
  // existing serializer
  ...
  public Epoch latestEpoch() {
    if (entries.isEmpty()) {
      if (baseState == null) return Epoch.EMPTY;
  ...
  public LogState flatten() {
    if (baseState == null && entries.isEmpty()) return this;
    ClusterMetadata metadata = baseState;
    if (metadata == null) metadata = new ClusterMetadata(DatabaseDescriptor.getPartitioner());
    for (Entry entry : entries) metadata = entry.transform.execute(metadata).success().metadata;
  ...
  public LogState retainFrom(Epoch epoch) {
    if (baseState != null && baseState.epoch.isAfter(epoch)) return this;
  ...
  public String toString() {
    return "LogState{"
        + "baseState="
        + (baseState != null ? baseState.epoch : "none ")
        + ", entries="
        + entries.size()
        + ": "
        + minMaxEntries()
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(baseState, logState.baseState)
        && Objects.equals(entries, logState.entries);
  }
  ...
  /**
   * Contains the logic for generating a LogState up to an arbitrary epoch. The LogState returned is
   * suitable for point in time recovery of cluster metadata. If any snapshots are available which
   * precede the target epoch, the LogState will include the one with the highest epoch along with
   * subsequent entries up to and including the target epoch. Callers supply: * The epoch to act as
   * the termination point for the LogState. If no metadata snapshot for a lower epoch exists, the
   * LogState will contain all log entries with an epoch less than this. If such a snapshot is
   * found, it will form the baseState of the LogState, and only log entries with epochs greater
   * than the snapshot's and less than or equal to the target will be included. This method should
   * not be called on any hot path - it should only be used for disaster recovery scenarios
   */
  ...
  static final class Serializer implements MetadataSerializer<LogState> {
    ...
    public void serialize(LogState t, DataOutputPlus out, Version version) throws IOException {
      ...
      if (t.baseState != null) ClusterMetadata.serializer.serialize(t.baseState, out, version);
      out.writeInt(t.entries.size());
      for (Entry entry : t.entries) Entry.serializer.serialize(entry, out, version);
    }

    @Override
    public LogState deserialize(DataInputPlus in, Version version) throws IOException {
      if (version.isAtLeast(Version.V2)) ClusterMetadata.checkIdentifier(in.readUnsignedVInt32());
      ClusterMetadata baseState = null;
      if (in.readBoolean()) baseState = ClusterMetadata.serializer.deserialize(in, version);
      int size = in.readInt();
      ImmutableList.Builder<Entry> builder = ImmutableList.builder();
      for (int i = 0; i < size; i++) builder.add(Entry.serializer.deserialize(in, version));
    ...
    public long serializedSize(LogState t, Version version) {
      ...
      for (Entry entry : t.entries) size += Entry.serializer.serializedSize(entry, version);
      return size;
    }
  }
  ...
  /**
   * Log Notification handler is similar to regular replication handler, except that notifying side
   * actually expects the response from the replica, and we need to be fully caught up to the latest
   * epoch in the replication, since replicas should be able to enact the latest epoch as soon as it
   * is watermarked by CMS.
   */
  public static class LogNotifyHandler implements IVerbHandler<LogState> {
    ...
    public void doVerb(Message<LogState> message) throws IOException {
      ...
      if (ClusterMetadataService.instance().isMigrating()) {
        logger.info(
            "Received metadata log notification from {}, marking in progress migration complete",
            message.from());
      ...
      if (log.hasGaps()) {
        ...
        if (highestPending.isPresent()) {
          ...
          ScheduledExecutors.optionalTasks.submit(
              () -> ClusterMetadataService.instance().fetchLogFromCMS(highestPending.get()));
        } else if (ClusterMetadata.current().epoch.isBefore(message.payload.latestEpoch())) {
          throw new IllegalStateException(
              String.format(
                  "Should have caught up to at least %s, but got only %s",
                  message.payload.latestEpoch(), ClusterMetadata.current().epoch));
        }
      } else log.waitForHighestConsecutive();
