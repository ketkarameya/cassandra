...
public class BootstrapAndReplace extends MultiStepOperation<Epoch> {
  ...
  public static BootstrapAndReplace newSequence(
      Epoch preparedAt,
      ...
      boolean streamData) {
    return new BootstrapAndReplace(
        preparedAt,
        lockKey,
        bootstrapTokens,
        START_REPLACE,
  ...
  public Transformation.Result applyTo(ClusterMetadata metadata) {
    return applyMultipleTransformations(
        metadata, next, of(startReplace, midReplace, finishReplace));
  }
        ...
        try {
          ...
          if (streamData) {
            MovementMap movements =
                movementMap(
                    metadata.directory.endpoint(startReplace.replaced()), startReplace.delta());
            ...
            if (!dataAvailable) {
              logger.warn(
                  "Some data streaming failed. Use nodetool to check bootstrap state and resume. "
                      + "For more, see `nodetool help bootstrap`. {}",
                  SystemKeyspace.getBootstrapState());
              return halted();
            }
            SystemKeyspace.setBootstrapState(SystemKeyspace.BootstrapState.COMPLETED);
          } else {
            // The node may have previously been started in write survey mode and may or may not
            // have
            // performed initial streaming (i.e. auto_bootstap: false). When an operator then
            // manually joins
            // it (or it bounces and comes up without the system property), it will hit this
            // condition.
            ...
            // streaming was done before entering write survey mode, the bootstrap is COMPLETE and
            // so no
            // need to log.
            // The ability to join without bootstrapping, especially when combined with write survey
            // mode
  ...
  public ProgressBarrier barrier() {
    ...
    if (next == START_REPLACE) return ProgressBarrier.immediate();
    ClusterMetadata metadata = ClusterMetadata.current();
    InetAddressAndPort replaced =
        metadata.directory.getNodeAddresses(startReplace.replaced()).broadcastAddress;
    return new ProgressBarrier(
        latestModification,
        metadata.directory.location(startReplace.nodeId()),
        metadata.lockedRanges.locked.get(lockKey),
        e -> !e.equals(replaced));
  }
  ...
  /**
   * startDelta.writes.additions contains the ranges we need to stream for each of those ranges, add
   * all possible endpoints (except for the replica we're replacing) to the movement map
   *
   * <p>keys in the map are the ranges the replacement node needs to stream, values are the
   * potential endpoints.
   */
  private static MovementMap movementMap(
      InetAddressAndPort beingReplaced, PlacementDeltas startDelta) {
    ...
    startDelta.forEach(
        (params, delta) -> {
          ...
          delta
              ...
              .forEach(
                  (destination) -> {
                    originalPlacements.reads.forRange(destination.range()).get().stream()
    ...
    switch (next) {
      ...
      default:
        throw new IllegalStateException(
            String.format("Step %s is invalid for sequence %s ", next, REPLACE));
    }
  }
  ...
  private static Transformation.Kind indexToNext(int index) {
    switch (index) {
      ...
      default:
        throw new IllegalStateException(
            String.format("Step %s is invalid for sequence %s ", index, REPLACE));
    }
  }
  ...
  public static void checkUnsafeReplace(boolean shouldBootstrap) {
    if (!shouldBootstrap && !CassandraRelevantProperties.ALLOW_UNSAFE_REPLACE.getBoolean()) {
      throw new RuntimeException(
          "Replacing a node without bootstrapping risks invalidating consistency "
  ...
  public static void gossipStateToHibernate(ClusterMetadata metadata, NodeId nodeId) {
    // order is important here, the gossiper can fire in between adding these two states.  It's ok
    // to send TOKENS without STATUS, but *not* vice versa.
    List<Pair<ApplicationState, VersionedValue>> states = new ArrayList<>();
    VersionedValue.VersionedValueFactory valueFactory = StorageService.instance.valueFactory;
    states.add(
        Pair.create(
            ApplicationState.TOKENS, valueFactory.tokens(metadata.tokenMap.tokens(nodeId))));
  ...
  public static class Serializer
      implements AsymmetricMetadataSerializer<MultiStepOperation<?>, BootstrapAndReplace> {
    ...
    public BootstrapAndReplace deserialize(DataInputPlus in, Version version) throws IOException {
      ...
      PrepareReplace.StartReplace startReplace =
          PrepareReplace.StartReplace.serializer.deserialize(in, version);
      PrepareReplace.MidReplace midReplace =
          PrepareReplace.MidReplace.serializer.deserialize(in, version);
      PrepareReplace.FinishReplace finishReplace =
          PrepareReplace.FinishReplace.serializer.deserialize(in, version);
