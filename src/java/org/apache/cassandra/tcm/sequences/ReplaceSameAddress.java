...
public class ReplaceSameAddress {
  ...
  public static void streamData(
      NodeId nodeId, ClusterMetadata metadata, boolean shouldBootstrap, boolean finishJoiningRing) {
    ...
    logger.warn(
        "Writes will not be forwarded to this node during replacement because it has the same"
            + " address as the node to be replaced ({}). If the previous node has been down for"
            + " longer than max_hint_window, repair must be run after the replacement process in"
            + " order to make this node consistent.",
    ...
    if (shouldBootstrap) {
      boolean dataAvailable =
          BootstrapAndJoin.bootstrap(
              metadata.tokenMap.tokens(nodeId),
