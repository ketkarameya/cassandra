...
public class BootstrapAndJoin extends MultiStepOperation<Epoch> {
  ...
  public static BootstrapAndJoin newSequence(
      Epoch preparedAt,
  ...
  public ProgressBarrier barrier() {
    ...
    if (next == START_JOIN) return ProgressBarrier.immediate();
    ClusterMetadata metadata = ClusterMetadata.current();
    return new ProgressBarrier(
        latestModification,
        metadata.directory.location(startJoin.nodeId()),
  ...
  public Pair<MovementMap, MovementMap> getMovementMaps(ClusterMetadata metadata) {
    MovementMap movementMap =
        movementMap(
            metadata.directory.endpoint(startJoin.nodeId()),
            metadata.placements,
            startJoin.delta());
  ...
  public static boolean bootstrap(
    ...
    assert beingReplaced == null || strictMovements == null
        : "Can't have strict movements during replacements";
    ...
    Future<StreamState> bootstrapStream =
        StorageService.instance.startBootstrap(metadata, beingReplaced, movements, strictMovements);
    try {
      if (bootstrapTimeoutMillis > 0) bootstrapStream.get(bootstrapTimeoutMillis, MILLISECONDS);
      else bootstrapStream.get();
  ...
  private static MovementMap movementMap(
      InetAddressAndPort joining, DataPlacements placements, PlacementDeltas startDelta) {
    MovementMap.Builder movementMapBuilder = MovementMap.builder();
    // we need all original placements for the ranges to stream - after initial split these new
    // ranges exist in placements
          ...
          delta
              ...
              .forEach(
                  (destination) -> {
                    ...
                    oldPlacement.reads.forRange(destination.range()).get().stream()
  ...
  private static MovementMap toStrict(
      MovementMap completeMovementMap, PlacementDeltas finishDelta) {
    MovementMap.Builder movementMapBuilder = MovementMap.builder();
    completeMovementMap.forEach(
        (params, byreplica) -> {
          Set<Replica> strictCandidates =
              Iterables.toSet(finishDelta.get(params).writes.removals.flattenValues());
          EndpointsByReplica.Builder movements = new EndpointsByReplica.Builder();
          for (Replica destination : byreplica.keySet()) {
            byreplica
                ...
                .forEach(
                    (source) -> {
                      if (strictCandidates.contains(source)) movements.put(destination, source);
                    });
          }
          movementMapBuilder.put(params, movements.build());
        });
    ...
    switch (next) {
      ...
      default:
        throw new IllegalStateException(
            String.format("Step %s is invalid for sequence %s ", next, JOIN));
    }
  }
  ...
  private static Transformation.Kind indexToNext(int index) {
    switch (index) {
      ...
      default:
        throw new IllegalStateException(
            String.format("Step %s is invalid for sequence %s ", index, JOIN));
    }
  }
  ...
  public static class Serializer
      implements AsymmetricMetadataSerializer<MultiStepOperation<?>, BootstrapAndJoin> {
    ...
    public BootstrapAndJoin deserialize(DataInputPlus in, Version version) throws IOException {
      ...
      PrepareJoin.FinishJoin finishJoin =
          PrepareJoin.FinishJoin.serializer.deserialize(in, version);
