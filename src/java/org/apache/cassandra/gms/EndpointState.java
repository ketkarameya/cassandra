...
public class EndpointState {
  ...
  static volatile boolean LOOSE_DEF_OF_EMPTY_ENABLED =
  ...
  void removeMajorVersion3LegacyApplicationStates() {
    while (hasLegacyFields()) {
      ...
      if (orig.size() == updatedStates.size()
          || applicationState.compareAndSet(orig, updatedStates)) return;
    }
  }

  private boolean hasLegacyFields() {
    Set<ApplicationState> statesPresent = applicationState.get().keySet();
    if (statesPresent.isEmpty()) return false;
    return (statesPresent.contains(ApplicationState.STATUS)
            && statesPresent.contains(ApplicationState.STATUS_WITH_PORT))
        || (statesPresent.contains(ApplicationState.INTERNAL_IP)
            && statesPresent.contains(ApplicationState.INTERNAL_ADDRESS_AND_PORT))
        || (statesPresent.contains(ApplicationState.RPC_ADDRESS)
            && statesPresent.contains(ApplicationState.NATIVE_ADDRESS_AND_PORT));
  }

  private static Map<ApplicationState, VersionedValue> filterMajorVersion3LegacyApplicationStates(
      Map<ApplicationState, VersionedValue> states) {
  ...
  public boolean isEmptyWithoutStatus() {
    ...
    return hbState.isEmpty() && !hasStatus
        // In the very specific case where hbState.isEmpty and STATUS is missing, this is known to
        // be safe to "fake"
        // the data, as this happens when the gossip state isn't coming from the node but instead
        // from a peer who
        // restarted and is missing the node's state.
        //
        // When hbState is not empty, then the node gossiped an empty STATUS; this happens during
        // bootstrap and it's not
        // possible to tell if this is ok or not (we can't really tell if the node is dead or having
        // networking issues).
        // For these cases allow an external actor to verify and inform Cassandra that it is safe -
        // this is done by
  ...
  public String toString() {
    return "EndpointState: HeartBeatState = "
        + hbState
        + ", AppStateMap = "
  ...
  public boolean isSupersededBy(EndpointState that) {
    ...
    if (thatGeneration > thisGeneration) return true;

    if (thisGeneration > thatGeneration) return false;
