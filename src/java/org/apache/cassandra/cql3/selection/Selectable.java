...
public interface Selectable extends AssignmentTestable {
  public Selector.Factory newSelectorFactory(
      TableMetadata table,
   ...
   *
   * @param keyspace the keyspace on which the statement for which this is a {@code Selectable} is
   *     on.
   * @return the type of this {@code Selectable} if inferrable, or {@code null} otherwise (for
   *     instance, the type isn't inferable for a bind marker. Even for literals, the exact type is
   *     not inferrable since they are valid for many different types and so this will return {@code
   *     null} too).
   */
   ...
   *
   * @return {@code true} if this {@code Selectable} select columns matching the specified
   ...
   * @return {@code true} if the specified Selectables select columns matching the specified
   *     predicate, {@code false} otherwise.
   */
  public static boolean selectColumns(
      List<Selectable> selectables, Predicate<ColumnMetadata> predicate) {
   ...
   *
   * @return {@code true} if any processing is performed on the selected columns, {@code false}
   *     otherwise.
   */
  public default boolean processesSelection() {
    // ColumnMetadata is the only case that returns false (if the column is not masked) and
    // overrides this
    return true;
  }

  // Term.Raw overrides this since some literals can be WEAKLY_ASSIGNABLE
  public default TestResult testAssignment(String keyspace, ColumnSpecification receiver) {
  ...
  default ColumnSpecification specForElementOrSlice(
      Selectable selected,
  ...
  public static class WithTerm implements Selectable {
    /**
     * The names given to unamed bind markers found in selection. In selection clause, we often
     * don't have a good name for bind markers, typically if you have: SELECT (int)? FROM foo; there
     * isn't a good name for that marker. So we give the same name to all the markers. Note that we
     * could try to differenciate the names by using some increasing number in the name (so
     * [selection_1], [selection_2], ...) but it's actually not trivial to do in the current code
     * and it's not really more helpful since if users wants to bind by position (which they will
     * have to in this case), they can do so at the driver level directly. And so we don't bother.
     * Note that users should really be using named bind markers if they want to be able to bind by
     * names.
     */
    private static final ColumnIdentifier bindMarkerNameInSelection =
        new ColumnIdentifier("[selection]", true);
        ...
        if (type == null)
          throw new InvalidRequestException(
              "Cannot infer type for term "
                  + this
                  + " in selection clause (try using a cast to force a type)");
      }

      // The fact we default the name to "[selection]" inconditionally means that any bind marker in
      // a
      // selection will have this name. Which isn't terribly helpful, but it's unclear how to
      // provide
      // something a lot more helpful and in practice user can bind those markers by position or,
      // even better,
      // use bind markers.
      Term term =
          rawTerm.prepare(
              table.keyspace,
              new ColumnSpecification(table.keyspace, table.name, bindMarkerNameInSelection, type));
  ...
  public static class WritetimeOrTTL implements Selectable {
    ...
    public enum Kind {
      ...
      MAX_WRITE_TIME(
          "maxwritetime",
          LongType.instance); // maxwritetime is available after Cassandra 4.1 (exclusive)
    ...
    public Selector.Factory newSelectorFactory(
      ...
      if (column.isPrimaryKeyColumn())
        throw new InvalidRequestException(
            String.format(
                "Cannot use selection function %s on PRIMARY KEY part %s", kind.name, column.name));

      Selector.Factory factory =
          selectable.newSelectorFactory(table, expectedType, defs, boundNames);
      boolean isMultiCell = factory.getColumnSpecification(table).type.isMultiCell();

      return WritetimeOrTTLSelector.newFactory(
          factory, addAndGetIndex(column, defs), kind, isMultiCell);
    }

    @Override
    public AbstractType<?> getExactTypeIfKnown(String keyspace) {
      AbstractType<?> type = kind.returnType;
      return column.type.isMultiCell() && !kind.aggregatesMultiCell()
    ...
    public static class Raw implements Selectable.Raw {
      ...
      public WritetimeOrTTL prepare(TableMetadata table) {
        return new WritetimeOrTTL(
            (ColumnMetadata) column.prepare(table), selected.prepare(table), kind);
      }
    }
  }
  ...
  public static class WithFunction implements Selectable {
    ...
    public Selector.Factory newSelectorFactory(
      ...
      SelectorFactories factories =
          SelectorFactories.createFactoriesAndCollectColumnDefinitions(
              args, function.argTypes(), table, defs, boundNames);
    ...
    public static class Raw implements Selectable.Raw {
      ...
      public static Raw newOperation(char operator, Selectable.Raw left, Selectable.Raw right) {
        return new Raw(
            OperationFcts.getFunctionNameFromOperator(operator), Arrays.asList(left, right));
      }

      public static Raw newNegation(Selectable.Raw arg) {
        return new Raw(
            FunctionName.nativeFunction(OperationFcts.NEGATION_FUNCTION_NAME),
      ...
      public Selectable prepare(TableMetadata table) {
        ...
        // COUNT(x) is equivalent to COUNT(*) for any non-null term x (since count(x) don't care
        // about its
        // argument outside of check for nullness) and for backward compatibilty we want to support
        // COUNT(1),
        // but we actually have COUNT(x) method for every existing (simple) input types so currently
        // COUNT(1)
  ...
  public static class WithCast implements Selectable {
    ...
    public Selector.Factory newSelectorFactory(
      ...
      if (fun == null) {
        throw new InvalidRequestException(
            String.format("%s cannot be cast to %s", defs.get(0).name, type));
      }
  ...
  public static class WithFieldSelection implements Selectable {
    ...
    public AbstractType<?> getExactTypeIfKnown(String keyspace) {
      ...
      if (selectedType == null || !(selectedType instanceof UserType)) return null;
   ...
   *
   * <p>The parser cannot differentiate between a single element between parentheses or a single
   * element tuple. By consequence, we are forced to wait until the type is known to be able to
   * differentiate them.
   */
  public static class BetweenParenthesesOrWithTuple implements Selectable {
    ...
    @Override
    public Factory newSelectorFactory(
        TableMetadata cfm,
        AbstractType<?> expectedType,
        List<ColumnMetadata> defs,
        VariableSpecifications boundNames) {
      ...
      if (type == null) {
        ...
        if (type == null)
          throw invalidRequest(
              "Cannot infer type for term %s in selection clause (try using a cast to force a"
                  + " type)",
              this);
      }
    ...
    private Factory newBetweenParenthesesSelectorFactory(
        TableMetadata cfm,
        AbstractType<?> expectedType,
        List<ColumnMetadata> defs,
        VariableSpecifications boundNames) {
    ...
    private Factory newTupleSelectorFactory(
        TableMetadata cfm,
        TupleType tupleType,
        List<ColumnMetadata> defs,
        VariableSpecifications boundNames) {
      SelectorFactories factories =
          createFactoriesAndCollectColumnDefinitions(
              selectables, tupleType.allTypes(), cfm, defs, boundNames);
    ...
    @Override
    public AbstractType<?> getExactTypeIfKnown(String keyspace) {
      // If there is only one element we cannot know if it is an element between parentheses or a
      // tuple
      // with only one element. By consequence, we need to force the user to specify the type.
      if (selectables.size() == 1) return null;
    ...
    public AbstractType<?> getCompatibleTypeIfKnown(String keyspace) {
      ...
      return Tuples.getExactTupleTypeIfKnown(
          selectables, p -> p.getCompatibleTypeIfKnown(keyspace));
    }
  ...
  public static class WithArrayLiteral implements Selectable {
    ...
    private Selectable target(AbstractType<?> target) {
      // when the target isn't known, fallback to list; cases like "SELECT [1, 2]" can't be known,
      // but used to be list type!
      // If a vector is actually desired, then can use type cast/hints: "SELECT (vector<int, 2>) [k,
      // v1]"
      if (target == null || target instanceof ListType) return new WithList(selectables);
      else if (target.isVector()) return new WithVector(selectables);
  ...
  public static class WithList extends WithArrayLiteral {
    ...
    public Factory newSelectorFactory(
      ...
      SelectorFactories factories =
          createFactoriesAndCollectColumnDefinitions(
              selectables, expectedTypes, cfm, defs, boundNames);
    ...
    @Override
    public AbstractType<?> getCompatibleTypeIfKnown(String keyspace) {
      return Lists.getPreferredCompatibleType(
          selectables, p -> p.getCompatibleTypeIfKnown(keyspace));
    }
  ...
  public static class WithVector extends WithArrayLiteral {
    ...
    @Override
    public AbstractType<?> getCompatibleTypeIfKnown(String keyspace) {
      return Vectors.getPreferredCompatibleType(
          selectables, p -> p.getCompatibleTypeIfKnown(keyspace));
    }
  ...
  public static class WithSet implements Selectable {
    ...
    public Factory newSelectorFactory(
      ...
      // The parser treats empty Maps as Sets so if the type is a MapType we know that the Map is
      // empty
      if (type instanceof MapType) return MapSelector.newFactory(type, Collections.emptyList());
    ...
    @Override
    public AbstractType<?> getCompatibleTypeIfKnown(String keyspace) {
      return Sets.getPreferredCompatibleType(
          selectables, p -> p.getCompatibleTypeIfKnown(keyspace));
    }
   ...
   * {@code ColumnIdentifier} is equivalent to a {@code FieldIdentifier} from a syntax point of
   * view. By consequence, we are forced to wait until the type is known to be able to differentiate
   * them.
   */
  public static class WithMapOrUdt implements Selectable {
    /**
     * The column family metadata. We need to store them to be able to build the proper data once
     * the type has been identified.
     */
    ...
    @Override
    public TestResult testAssignment(String keyspace, ColumnSpecification receiver) {
      return receiver.type.isUDT()
          ? UserTypes.testUserTypeAssignment(receiver, getUdtFields((UserType) receiver.type))
    ...
    public Factory newSelectorFactory(
      ...
      if (type.isUDT()) return newUdtSelectorFactory(cfm, expectedType, defs, boundNames);

      return newMapSelectorFactory(cfm, defs, boundNames, type);
    }

    private Factory newMapSelectorFactory(
        TableMetadata cfm,
        List<ColumnMetadata> defs,
        VariableSpecifications boundNames,
        AbstractType<?> type) {
    ...
    private Factory newUdtSelectorFactory(
        TableMetadata cfm,
        AbstractType<?> expectedType,
        List<ColumnMetadata> defs,
        VariableSpecifications boundNames) {
      ...
      for (Pair<Selectable.Raw, Selectable.Raw> raw : raws) {
        if (!(raw.left instanceof RawIdentifier))
          throw invalidRequest(
              "%s is not a valid field identifier of type %s ", raw.left, ut.getNameAsString());
        ...
        if (fieldPosition == -1)
          throw invalidRequest(
              ...
              fieldName, ut.getNameAsString());

        AbstractType<?> fieldType = ut.fieldType(fieldPosition);
        factories.put(
            fieldName, raw.right.prepare(cfm).newSelectorFactory(cfm, fieldType, defs, boundNames));
      }
    ...
    public boolean selectColumns(Predicate<ColumnMetadata> predicate) {
      for (Pair<Selectable.Raw, Selectable.Raw> raw : raws) {
        ...
        if (!raw.right.prepare(cfm).selectColumns(predicate)) return true;
      }
      return false;
    }
  ...
  public static class WithTypeHint implements Selectable {
    ...
    @Override
    public TestResult testAssignment(String keyspace, ColumnSpecification receiver) {
      if (receiver.type.equals(type)) return AssignmentTestable.TestResult.EXACT_MATCH;
    ...
    public Factory newSelectorFactory(
        ...
        List<ColumnMetadata> defs,
        VariableSpecifications boundNames) {
      final ColumnSpecification receiver =
          new ColumnSpecification(
              cfm.keyspace, cfm.name, new ColumnIdentifier(toString(), true), type);

      if (!selectable.testAssignment(cfm.keyspace, receiver).isAssignable())
        throw new InvalidRequestException(
            String.format(
                "Cannot assign value %s to %s of type %s",
                this, receiver.name, receiver.type.asCQL3Type()));
    ...
    public static class Raw implements Selectable.Raw {
      ...
      public Selectable prepare(TableMetadata cfm) {
        ...
        if (type.isFreezable()) type = type.freeze();
  ...
  /**
   * In the selection clause, the parser cannot differentiate between Maps and UDTs as a column
   * identifier and field identifier have the same syntax. By consequence, we need to wait until the
   * type is known to create the proper Object: {@code ColumnMetadata} or {@code FieldIdentifier}.
   */
  ...
  public static class WithElementSelection implements Selectable {
    public final Selectable selected;
    // Note that we can't yet prepare the Term.Raw yet as we need the ColumnSpecificiation
    // corresponding to Selectable, which
    // we'll only know in newSelectorFactory due to functions (which needs the defs passed to
    // newSelectorFactory to resolve which
    // function is called). Note that this doesn't really matter performance wise since the
    // factories are still created during
    ...
    public Selector.Factory newSelectorFactory(
      ...
      ColumnSpecification boundSpec =
          specForElementOrSlice(selected, receiver, ((CollectionType) type).kind, "Element");
    ...
    public AbstractType<?> getExactTypeIfKnown(String keyspace) {
      AbstractType<?> selectedType = selected.getExactTypeIfKnown(keyspace);
      if (selectedType == null || !(selectedType instanceof CollectionType)) return null;
  ...
  public static class WithSliceSelection implements Selectable {
    ...
    public Selector.Factory newSelectorFactory(
        ...
        List<ColumnMetadata> defs,
        VariableSpecifications boundNames) {
      // Note that a slice gives you the same type as the collection you applied it to, so we can
      // pass expectedType for selected directly
      ...
      ColumnSpecification boundSpec =
          specForElementOrSlice(selected, receiver, ((CollectionType) type).kind, "Slice");

      // If from or to are null, this means the user didn't provide on in the syntax (we had c[x..]
      // or c[..x]).
    ...
    public static class Raw implements Selectable.Raw {
      ...
      public String toString() {
        return String.format(
            "%s[%s..%s]", selected, from == null ? "" : from, to == null ? "" : to);
      }
    }
  }
}
