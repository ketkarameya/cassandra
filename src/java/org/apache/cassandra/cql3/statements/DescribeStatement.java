...
public abstract class DescribeStatement<T> extends CQLStatement.Raw implements CQLStatement {
  ...
  /**
   * The columns returned by the describe queries that only list elements names (e.g. DESCRIBE
   * KEYSPACES, DESCRIBE TABLES...)
   */
  private static final List<ColumnSpecification> LIST_METADATA =
      ImmutableList.of(
          new ColumnSpecification(
              KS, CF, new ColumnIdentifier("keyspace_name", true), UTF8Type.instance),
  ...
  /**
   * The columns returned by the describe queries that returns the CREATE STATEMENT for the
   * different elements (e.g. DESCRIBE KEYSPACE, DESCRIBE TABLE ...)
   */
  private static final List<ColumnSpecification> ELEMENT_METADATA =
      ImmutableList.<ColumnSpecification>builder()
          .addAll(LIST_METADATA)
          .add(
              new ColumnSpecification(
                  KS, CF, new ColumnIdentifier("create_statement", true), UTF8Type.instance))
          .build();

  /** "Magic version" for the paging state. */
  private static final int PAGING_STATE_VERSION = 0x0001;

  static final String SCHEMA_CHANGED_WHILE_PAGING_MESSAGE =
  ...
  @Override
  public final ResultMessage execute(
  ...
  public ResultMessage executeLocally(QueryState state, QueryOptions options) {
    ...
    // completes within a few seconds. This seems good enough for now. Once Cassandra actually
    // supports
    ...
    if (offset > 0L) stream = stream.skip(offset);
    if (pageSize > 0) stream = stream.limit(pageSize);

    List<List<ByteBuffer>> rows =
        stream.map(e -> toRow(e, includeInternalDetails)).collect(Collectors.toList());

    ResultSet.ResultMetadata resultMetadata =
        new ResultSet.ResultMetadata(metadata(state.getClientState()));
  ...
  private long getOffset(PagingState pagingState, UUID schemaVersion) {
    ...
    try (DataInputBuffer in = new DataInputBuffer(pagingState.partitionKey, false)) {
      ...
      checkTrue(
          pagingStateServerVersion.equals(releaseVersion),
  ...
  /**
   * Returns the metadata for the given keyspace or throws a {@link KeyspaceNotDefinedException}
   * exception.
   */
  private static KeyspaceMetadata validateKeyspace(String ks, Keyspaces keyspaces) {
    return keyspaces
        .get(ks)
        .orElseThrow(
            () -> new KeyspaceNotDefinedException(format("'%s' not found in keyspaces", ks)));
  }

  /**
   * {@code DescribeStatement} implementation used for describe queries that only list elements
   * names.
   */
  public static final class Listing extends DescribeStatement<SchemaElement> {
    private final java.util.function.Function<KeyspaceMetadata, Stream<? extends SchemaElement>>
        elementsProvider;

    public Listing(
        java.util.function.Function<KeyspaceMetadata, Stream<? extends SchemaElement>>
            elementsProvider) {
    ...
    protected Stream<? extends SchemaElement> describe(ClientState state, Keyspaces keyspaces) {
      ...
      Stream<KeyspaceMetadata> stream =
          keyspace == null
              ? keyspaces.stream().sorted(SchemaElement.NAME_COMPARATOR)
    ...
    protected List<ByteBuffer> toRow(SchemaElement element, boolean withInternals) {
      return ImmutableList.of(
          bytes(element.elementKeyspaceQuotedIfNeeded()),
  ...
  public static class Element extends DescribeStatement<SchemaElement> {
    ...
    private final BiFunction<KeyspaceMetadata, String, Stream<? extends SchemaElement>>
        elementsProvider;

    public Element(
        String keyspace,
        String name,
        BiFunction<KeyspaceMetadata, String, Stream<? extends SchemaElement>> elementsProvider) {
    ...
    protected Stream<? extends SchemaElement> describe(ClientState state, Keyspaces keyspaces) {
      String ks =
          keyspace == null
              ? checkNotNull(
                  state.getRawKeyspace(), "No keyspace specified and no current keyspace")
              : keyspace;
  ...
  public static DescribeStatement<SchemaElement> keyspace(
      String keyspace, boolean onlyKeyspaceDefinition) {
    return new Element(keyspace, null, (ks, t) -> getKeyspaceElements(ks, onlyKeyspaceDefinition));
  }

  private static Stream<? extends SchemaElement> getKeyspaceElements(
      KeyspaceMetadata ks, boolean onlyKeyspace) {
  ...
  private static Stream<? extends SchemaElement> getTableElements(
      KeyspaceMetadata ks, TableMetadata table) {
  ...
  public static DescribeStatement<SchemaElement> table(String keyspace, String name) {
    return new Element(
        ...
        (ks, t) -> {
          TableMetadata table =
              checkNotNull(
                  ks.getTableNullable(t), "Table '%s' not found in keyspace '%s'", t, ks.name);
  ...
  public static DescribeStatement<SchemaElement> index(String keyspace, String name) {
    return new Element(
        ...
        (ks, index) -> {
          TableMetadata tm =
              ks.findIndexedTable(index)
                  .orElseThrow(
                      () ->
                          invalidRequest(
                              "Table for existing index '%s' not found in '%s'", index, ks.name));
          return tm.indexes
              .get(index)
    ...
    return new Element(
        ...
        (ks, view) -> {
          return ks.views
              ...
              .map(Stream::of)
              .orElseThrow(
                  () -> invalidRequest("Materialized view '%s' not found in '%s'", view, ks.name));
        });
  }
  ...
  public static DescribeStatement<SchemaElement> type(String keyspace, String name) {
    return new Element(
        ...
        (ks, type) -> {
          return ks.types
              .get(ByteBufferUtil.bytes(type))
              .map(Stream::of)
              .orElseThrow(
                  () -> invalidRequest("User defined type '%s' not found in '%s'", type, ks.name));
        });
  }
    ...
    return new Element(
        ...
        (ks, n) -> {
          return checkNotEmpty(
                  ks.userFunctions.getUdfs(new FunctionName(ks.name, n)),
                  "User defined function '%s' not found in '%s'",
                  n,
                  ks.name)
              .stream()
              .sorted(SchemaElement.NAME_COMPARATOR);
  ...
  public static DescribeStatement<SchemaElement> aggregate(String keyspace, String name) {
    return new Element(
        ...
        (ks, n) -> {
          return checkNotEmpty(
                  ks.userFunctions.getUdas(new FunctionName(ks.name, n)),
                  "User defined aggregate '%s' not found in '%s'",
                  n,
                  ks.name)
              .stream()
              .sorted(SchemaElement.NAME_COMPARATOR);
  ...
  public static DescribeStatement<SchemaElement> generic(String keyspace, String name) {
    return new DescribeStatement<SchemaElement>() {
      ...
      private DescribeStatement<SchemaElement> resolve(ClientState state, Keyspaces keyspaces) {
        ...
        // from cqlsh help: "keyspace or a table or an index or a materialized view (in this
        // order)."
        if (keyspace == null) {
    ...
    return new DescribeStatement<List<Object>>() {
      ...
      protected Stream<List<Object>> describe(ClientState state, Keyspaces keyspaces) {
        ...
        if (mustReturnsRangeOwnerships(useKs)) {
          list.add(
              StorageService.instance.getRangeToAddressMap(useKs).entrySet().stream()
                  .sorted(Comparator.comparing(Map.Entry::getKey))
                  .collect(
                      Collectors.toMap(
                          e -> e.getKey().right.toString(),
                          e ->
                              e.getValue().stream()
      ...
      private boolean mustReturnsRangeOwnerships(String useKs) {
        return useKs != null
            && !SchemaConstants.isLocalSystemKeyspace(useKs)
            && !SchemaConstants.isSystemKeyspace(useKs);
      }
      ...
      protected List<ColumnSpecification> metadata(ClientState state) {
        ...
        builder.add(
            new ColumnSpecification(
                KS, CF, new ColumnIdentifier("cluster", true), UTF8Type.instance),
            new ColumnSpecification(
                KS, CF, new ColumnIdentifier("partitioner", true), UTF8Type.instance),
            new ColumnSpecification(
                KS, CF, new ColumnIdentifier("snitch", true), UTF8Type.instance));
        ...
        if (mustReturnsRangeOwnerships(state.getRawKeyspace()))
          builder.add(
              new ColumnSpecification(
                  KS,
                  CF,
                  new ColumnIdentifier("range_ownership", true),
                  MapType.getInstance(
                      UTF8Type.instance, ListType.getInstance(UTF8Type.instance, false), false)));
      ...
      protected List<ByteBuffer> toRow(List<Object> elements, boolean withInternals) {
        ...
        builder.add(
            UTF8Type.instance.decompose((String) elements.get(CLUSTER_NAME_INDEX)),
        ...
        if (elements.size() > 3) {
          MapType<String, List<String>> rangeOwnershipType =
              MapType.getInstance(
                  UTF8Type.instance, ListType.getInstance(UTF8Type.instance, false), false);

          builder.add(
              rangeOwnershipType.decompose(
                  (Map<String, List<String>>) elements.get(RANGE_OWNERSHIPS_INDEX)));
        }
