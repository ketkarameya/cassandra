...
public final class CreateViewStatement extends AlterSchemaStatement {
  ...
  public Keyspaces apply(ClusterMetadata metadata) {
    ...
    if (null == table) throw ire("Base table '%s' doesn't exist", tableName);
    ...
    if (table.isCounter()) throw ire("Materialized views are not supported on counter tables");
    ...
    if (partitionKeyColumns.isEmpty())
      throw ire(
          "Must provide at least one partition key column for materialized view '%s'", viewName);
    ...
    if (!clusteringOrder.isEmpty()
        && !clusteringColumns.equals(new ArrayList<>(clusteringOrder.keySet())))
      throw ire(
    ...
    List<ColumnIdentifier> missingPrimaryKeyColumns =
        Lists.newArrayList(
            filter(
                transform(table.primaryKeyColumns(), c -> c.name),
                c -> !primaryKeyColumns.contains(c)));

    if (!missingPrimaryKeyColumns.isEmpty()) {
      throw ire(
          "Cannot create materialized view '%s' without primary key columns %s from base table"
              + " '%s'",
          viewName,
          join(", ", transform(missingPrimaryKeyColumns, ColumnIdentifier::toString)),
          tableName);
    }

    Set<ColumnIdentifier> regularBaseTableColumnsInViewPrimaryKey =
        new HashSet<>(primaryKeyColumns);
    transform(table.primaryKeyColumns(), c -> c.name)
        .forEach(regularBaseTableColumnsInViewPrimaryKey::remove);
    if (regularBaseTableColumnsInViewPrimaryKey.size() > 1) {
      throw ire(
          "Cannot include more than one non-primary key column in materialized view primary key"
              + " (got %s)",
          join(
              ", ",
              transform(regularBaseTableColumnsInViewPrimaryKey, ColumnIdentifier::toString)));
    }
    ...
    List<ColumnIdentifier> nonRestrictedPrimaryKeyColumns =
        Lists.newArrayList(
            filter(primaryKeyColumns, name -> !restrictions.isRestricted(table.getColumn(name))));
    ...
    if (!restrictedNonPrimaryKeyColumns.isEmpty()
        && !MV_ALLOW_FILTERING_NONKEY_COLUMNS_UNSAFE.getBoolean()) {
      throw ire(
          "Non-primary key columns can only be restricted with 'IS NOT NULL' (got: %s restricted"
              + " illegally)",
    ...
    if (attrs.hasOption(TableParams.Option.DEFAULT_TIME_TO_LIVE)
      ...
      throw ire(
          "Cannot set default_time_to_live for a materialized view. "
    ...
    partitionKeyColumns.stream()
        ...
        .forEach(
            column ->
                builder.addPartitionKeyColumn(column.name, getType(column), column.getMask()));

    clusteringColumns.stream()
        .map(table::getColumn)
        .forEach(
            column -> builder.addClusteringColumn(column.name, getType(column), column.getMask()));

    selectedColumns.stream()
        .filter(name -> !primaryKeyColumns.contains(name))
        .map(table::getColumn)
        .forEach(
            column -> builder.addRegularColumn(column.name, getType(column), column.getMask()));

    ViewMetadata view =
        new ViewMetadata(table.id, table.name, rawColumns.isEmpty(), whereClause, builder.build());
  ...
  private AbstractType<?> getType(ColumnMetadata column) {
    ...
    if (clusteringOrder.containsKey(column.name)) {
      ...
      if (!type.isReversed() && reverse) return ReversedType.getInstance(type);
    }
    return type;
  }
  ...
  public static final class Raw extends CQLStatement.Raw {
    ...
    public void extendClusteringOrder(ColumnIdentifier column, boolean ascending) {
      if (null != clusteringOrder.put(column, ascending))
        throw ire(
            "Duplicate column '%s' in CLUSTERING ORDER BY clause for view '%s'", column, viewName);
    }
  }
}
