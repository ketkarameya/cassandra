...
public final class DropAggregateStatement extends AlterSchemaStatement {
  ...
  public Keyspaces apply(ClusterMetadata metadata) {
    ...
    Collection<UserFunction> aggregates =
        keyspace.userFunctions.get(new FunctionName(keyspaceName, aggregateName));
    if (aggregates.size() > 1 && !argumentsSpeficied) {
      throw ire(
          "'DROP AGGREGATE %s' matches multiple function definitions; "
    ...
    return schema.withAddedOrUpdated(
        keyspace.withSwapped(keyspace.userFunctions.without(aggregate)));
  }
  ...
  public void authorize(ClientState client) {
    ...
    Stream<UserFunction> functions =
        keyspace.userFunctions.get(new FunctionName(keyspaceName, aggregateName)).stream();
