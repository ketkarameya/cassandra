...
public final class CreateFunctionStatement extends AlterSchemaStatement {
  ...
  public Keyspaces apply(ClusterMetadata metadata) {
    ...
    AbstractType<?> returnType =
        rawReturnType.prepare(keyspaceName, keyspace.types).getType().udfType();
    ...
    UserFunction existingFunction =
        keyspace.userFunctions.find(function.name(), argumentTypes).orElse(null);
    if (null != existingFunction) {
      ...
      if (!orReplace) throw ire("Function '%s' already exists", functionName);
      ...
      if (!returnType.isCompatibleWith(existingFunction.returnType())) {
        throw ire(
            "Cannot replace function '%s', the new return type %s is not compatible with the return"
                + " type %s of existing function",
            functionName, returnType.asCQL3Type(), existingFunction.returnType().asCQL3Type());
      }
    ...
    return schema.withAddedOrUpdated(
        keyspace.withSwapped(keyspace.userFunctions.withAddedOrUpdated(function)));
  }
  ...
  public void authorize(ClientState client) {
    ...
    if (Schema.instance
            .findUserFunction(
                name,
                Lists.transform(rawArgumentTypes, t -> t.prepare(keyspaceName).getType().udfType()))
            .isPresent()
        && orReplace)
