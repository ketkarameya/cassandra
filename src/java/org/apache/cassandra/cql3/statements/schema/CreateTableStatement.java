...
public final class CreateTableStatement extends AlterSchemaStatement {
  ...
  @Override
  public String cql() {
    if (expandedCql != null) return expandedCql;
    return super.cql();
  }
  ...
  public Keyspaces apply(ClusterMetadata metadata) {
    ...
    for (KeyspaceMetadata ksm : schema) ufBuilder.add(ksm.userFunctions);

    TableMetadata.Builder builder =
        builder(keyspace.types, ufBuilder.build()).epoch(metadata.nextEpoch());

    // We do not want to set table ID here just yet, since we are using CQL for serialising a fully
    // expanded CREATE TABLE statement.
    this.expandedCql =
        builder.build().toCqlString(false, attrs.hasProperty(TableAttributes.ID), ifNotExists);
  ...
  public void validate(ClientState state) {
    ...
    if (Guardrails.tables.enabled(state)) {
      int totalUserTables =
          Schema.instance.getUserKeyspaces().stream().mapToInt(ksm -> ksm.tables.size()).sum();
    ...
    if (useCompactStorage) Guardrails.compactTablesEnabled.ensureEnabled(state);
  ...
  public TableMetadata.Builder builder(Types types, UserFunctions functions) {
    ...
    // use a TreeMap to preserve ordering across JDK versions (see CASSANDRA-9492) - important for
    // stable unit tests
    Map<ColumnIdentifier, ColumnProperties> columns = new TreeMap<>(comparing(o -> o.bytes));
    rawColumns.forEach(
        (column, properties) ->
            columns.put(
                column, properties.prepare(keyspaceName, tableName, column, types, functions)));
    ...
    concat(partitionKeyColumns, clusteringColumns)
        .forEach(
            column -> {
    ...
    clusteringColumns.forEach(
        column -> {
          ...
          clusteringColumnProperties.add(
              reverse ? columnProperties.withReversedType() : columnProperties);
        });
    ...
    for (ColumnIdentifier id : clusteringOrder.keySet()) {
      ...
      if (!id.equals(c)) {
        if (clusteringOrder.containsKey(c))
          throw ire(
              "The order of columns in the CLUSTERING ORDER directive must match that of the"
                  + " clustering columns (%s must appear before %s)",
              c, id);
    ...
    // For COMPACT STORAGE, we reject any "feature" that we wouldn't be able to translate back to
    // thrift.
    ...
    if (hasCounters) {
      // We've handled anything that is not a PRIMARY KEY so columns only contains NON-PK columns.
      // So
      ...
      if (params.defaultTimeToLive > 0)
        throw ire(
            "Cannot set %s on a table with counters", TableParams.Option.DEFAULT_TIME_TO_LIVE);
    }
    ...
    if (useCompactStorage) {
      ...
      columns.forEach(
          (column, properties) -> {
            ...
            else builder.addRegularColumn(column, properties.type, properties.mask);
          });
    }
    return builder;
  }

  private void validateCompactTable(
      List<ColumnProperties> clusteringColumnProperties,
    ...
    if (isDense) {
      ...
      if (columns.size() > 1)
        throw ire(
            String.format(
                "COMPACT STORAGE with composite PRIMARY KEY allows no more than one column not part"
                    + " of the PRIMARY KEY (got: %s)",
                StringUtils.join(columns.keySet(), ", ")));
    } else {
      // we are in the "static" case, so we need at least one column defined. For non-compact
      // however, having
      // just the PK is fine.
      if (columns.isEmpty())
        throw ire(
            "COMPACT STORAGE with non-composite PRIMARY KEY require one column not part of the"
                + " PRIMARY KEY, none given");
    }
  }

  private void fixupCompactTable(
      List<ColumnProperties> clusteringTypes,
      Map<ColumnIdentifier, ColumnProperties> columns,
      boolean hasCounters,
      TableMetadata.Builder builder) {
    ...
    columns.forEach(
        (name, properties) -> {
          // Note that for "static" no-clustering compact storage we use static for the defined
          // columns
          if (staticColumns.contains(name) || isStaticCompact)
            builder.addStaticColumn(name, properties.type, properties.mask);
          else builder.addRegularColumn(name, properties.type, properties.mask);
        });
    ...
    if (isStaticCompact) {
      ...
      builder.addRegularColumn(
          names.defaultCompactValueName(),
          hasCounters ? CounterColumnType.instance : BytesType.instance);
    } else if (!builder.hasRegularColumns()) {
      // Even for dense, we might not have our regular column if it wasn't part of the declaration.
      // If
      ...
      while (true) {
        String candidate =
            compactIndex == 0
                ...
                : DEFAULT_COMPACT_VALUE_NAME + compactIndex;
        ++compactIndex;
        if (usedNames.add(candidate)) return candidate;
      }
    }
  ...
  public static final class Raw extends CQLStatement.Raw {
    ...
    public void addColumn(
        ColumnIdentifier column, CQL3Type.Raw type, boolean isStatic, ColumnMask.Raw mask) {
      ...
      if (isStatic) staticColumns.add(column);
    }
    ...
    public void extendClusteringOrder(ColumnIdentifier column, boolean ascending) {
      if (null != clusteringOrder.put(column, ascending))
        throw ire(
            "Duplicate column '%s' in CLUSTERING ORDER BY clause for table '%s'", column, name);
    }
  }
  ...
  private static final class ColumnProperties {
    ...
    public final CQL3Type
        cqlType; // we keep the original CQL type for printing fully qualified user type names

    @Nullable public final ColumnMask mask;
    ...
    public ColumnProperties withReversedType() {
      return new ColumnProperties(
          ReversedType.getInstance(type), cqlType, mask == null ? null : mask.withReversedType());
    }

    public static final class Raw {
      public final CQL3Type.Raw rawType;

      @Nullable public final ColumnMask.Raw rawMask;
      ...
      public ColumnProperties prepare(
          String keyspace,
          String table,
        ...
        ColumnMask mask =
            rawMask == null ? null : rawMask.prepare(keyspace, table, column, type, functions);
