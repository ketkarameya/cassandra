...
public enum Operator {
  EQ(0) {
    ...
    public boolean isSatisfiedBy(
        AbstractType<?> type, ByteBuffer leftOperand, ByteBuffer rightOperand) {
  ...
  IN(7) {
    ...
    public boolean canBeUsedWith(ColumnsExpression.Kind kind) {
      return kind == ColumnsExpression.Kind.SINGLE_COLUMN
          || kind == ColumnsExpression.Kind.MULTI_COLUMN;
    }
  },
  CONTAINS(5) {
    @Override
    public boolean isSatisfiedBy(
      ...
      switch (((CollectionType<?>) type).kind) {
        case LIST:
          ...
          return listType
              .compose(leftOperand)
              .contains(listType.getElementsType().compose(rightOperand));
        case SET:
          SetType<?> setType = (SetType<?>) type;
          return setType
              .compose(leftOperand)
              .contains(setType.getElementsType().compose(rightOperand));
        case MAP:
          MapType<?, ?> mapType = (MapType<?, ?>) type;
          return mapType
              .compose(leftOperand)
              .containsValue(mapType.getValuesType().compose(rightOperand));
      }
    ...
    @Override
    public boolean isSatisfiedBy(
        CollectionType<?> type, ComplexColumnData leftOperand, ByteBuffer rightOperand) {
      for (Cell<?> cell : leftOperand) {
        if (type.kind == CollectionType.Kind.SET) {
          if (type.nameComparator().compare(cell.path().get(0), rightOperand) == 0) return true;
        } else {
          if (type.valueComparator().compare(cell.buffer(), rightOperand) == 0) return true;
        }
      }
      return false;
    }
  ...
  ANN(15) {
    ...
    public boolean isSatisfiedBy(
        AbstractType<?> type, ByteBuffer leftOperand, ByteBuffer rightOperand) {
      // The ANN operator is only supported by the vector index so, normally, should never be called
      // directly.
      // In networked queries (non-local) the coordinator will end up calling the row filter
      // directly. So, this
      // needs to return true so that the returned values are allowed through to the
      // VectorTopKProcessor
      throw new UnsupportedOperationException();
    }
  ...
  public static Operator readFrom(DataInput input) throws IOException {
    int b = input.readInt();
    for (Operator operator : values()) if (operator.b == b) return operator;

    throw new IOException(
        String.format("Cannot resolve Relation.Type from binary representation: %s", b));
  }
    ...
    switch (expression.kind()) {
      case SINGLE_COLUMN:
        ...
        if (isSlice()) {
          if (columnType.referencesDuration()) {
            checkFalse(
                columnType.isCollection(),
                "Slice restrictions are not supported on collections containing durations");
            checkFalse(
                columnType.isTuple(),
                "Slice restrictions are not supported on tuples containing durations");
            checkFalse(
                columnType.isUDT(),
                "Slice restrictions are not supported on UDTs containing durations");
            throw invalidRequest("Slice restrictions are not supported on duration columns");
          }
        } else {
          checkFalse(
              appliesToMapKeys() && !(columnType instanceof MapType),
              "Cannot use %s on non-map column %s",
              this,
              firstColumn.name);
          checkFalse(
              appliesToCollectionElements() && !columnType.isCollection(),
              "Cannot use %s on non-collection column %s",
              this,
              firstColumn.name);
        }
      ...
      case MAP_ELEMENT:
        ...
        if (type.isMultiCell()) {
          ...
          // We don't support relations against entire collections (unless they're frozen), like
          // "numbers = {1, 2, 3}"
          checkFalse(
              type.isCollection()
   ...
   * @return {@code true} if the specified expression kind can be used with this operator, {@code
   *     false} otherwise.
   */
  public boolean canBeUsedWith(ColumnsExpression.Kind kind) {
  ...
  /**
   * Checks if this operator applies to collection elements (from frozen and non-frozen
   * collections).
   *
  ...
  /**
   * Checks if this operator <b>requires</b> either filtering or indexing for the specified columns
   * kinds.
   *
   * <p>An operator requires filtering or indexing only if it cannot be executed by other means. An
   * equal operator on a clustering column for example will return {@code false} even if filtering
   * might be used because the previous clustering column is not restricted.
   *
   * @param columnKind the kind of column being restricted by the operator
   * @return {@code true} if this operator requires either filtering or indexing, {@code false}
   *     otherwise.
   */
  public boolean requiresFilteringOrIndexingFor(ColumnMetadata.Kind columnKind) {
    return true;
  ...
  /**
   * Some operators are not supported by the read path because we never fully implemented support
   * for them. It is the case for {@code IS_NOT} and {@code !=}
   *
  ...
  /**
   * The "LIKE_" operators are not real CQL operators and are simply an internal hack that should be
   * removed at some point. Therefore, we want to ignore them in the error messages returned to the
   * users.
   *
   * @return {@code true} for the "LIKE_" operators
   */
  private boolean isLikeVariant() {
    return this == LIKE_CONTAINS
        || this == LIKE_PREFIX
        || this == LIKE_MATCHES
        || this == LIKE_SUFFIX;
  }
   ...
   */
  public static List<Operator> operatorsRequiringFilteringOrIndexingFor(
      ColumnMetadata.Kind columnKind) {
