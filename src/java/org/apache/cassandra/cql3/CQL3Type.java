...
public interface CQL3Type {
   ...
   *
   * @param bytes the value to convert to a CQL literal. This value must be serialized with {@code
  ...
  enum Native implements CQL3Type {
    ...
    /**
     * Delegate to {@link org.apache.cassandra.serializers.TypeSerializer#toCQLLiteral(ByteBuffer)}
     * for native types as most CQL literal representations work fine with the default {@link
     * org.apache.cassandra.serializers.TypeSerializer#toString(Object)} {@link
     * org.apache.cassandra.serializers.TypeSerializer#deserialize(ByteBuffer)} implementations.
     */
  ...
  class Collection implements CQL3Type {
    ...
    private void generateMapCQLLiteral(ByteBuffer buffer, StringBuilder target, int size) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        ByteBuffer element =
            CollectionSerializer.readValue(buffer, ByteBufferAccessor.instance, offset);
    ...
    private static void generateSetOrListCQLLiteral(
        ByteBuffer buffer, StringBuilder target, int size, CQL3Type elements) {
      int offset = 0;
      for (int i = 0; i < size; i++) {
        if (i > 0) target.append(", ");
        ByteBuffer element =
  ...
  class UserDefined implements CQL3Type {
      ...
      for (int i = 0; i < type.size(); i++) {
        ...
        if (buffer.remaining() < 4)
          throw new MarshalException(
              String.format(
                  "Not enough bytes to read size of %dth field %s", i, type.fieldName(i)));
        ...
        if (buffer.remaining() < size)
          throw new MarshalException(
              String.format("Not enough bytes to read %dth field %s", i, type.fieldName(i)));
    ...
    @Override
    public final boolean equals(Object o) {
      if (!(o instanceof UserDefined)) return false;
    ...
    @Override
    public String toString() {
      if (type.isMultiCell()) return ColumnIdentifier.maybeQuote(name);
      else return "frozen<" + ColumnIdentifier.maybeQuote(name) + '>';
    }
  }
  ...
  class Tuple implements CQL3Type {
    ...
    public String toCQLLiteral(ByteBuffer buffer) {
      ...
      for (int i = 0; i < type.size(); i++) {
        ...
        if (buffer.remaining() < 4)
          throw new MarshalException(
              String.format("Not enough bytes to read size of %dth component", i));
    ...
    public String toString(boolean withFrozen) {
      ...
      if (withFrozen) sb.append("frozen<");
      sb.append("tuple<");
      for (int i = 0; i < type.size(); i++) {
        if (i > 0) sb.append(", ");
  ...
  class Vector implements CQL3Type {
    ...
    @Override
    public String toCQLLiteral(ByteBuffer buffer) {
      if (type.isNull(buffer)) return "null";
      buffer = buffer.duplicate();
    ...
    public String toString() {
      StringBuilder sb = new StringBuilder();
      sb.append("vector<")
          .append(type.elementType.asCQL3Type())
          .append(", ")
  ...
  abstract class Raw {
    ...
    private static class RawCollection extends Raw {
      ...
      private RawCollection(
          CollectionType.Kind kind, CQL3Type.Raw keys, CQL3Type.Raw values, boolean frozen) {
        super(frozen);
        this.kind = kind;
        this.keys = keys;
        this.values = values;
      ...
      public RawCollection freeze() {
        ...
        CQL3Type.Raw frozenValues =
            null != values && values.supportsFreezing() ? values.freeze() : values;
      ...
      @Override
      public void validate(ClientState state, String name) {
        if (keys != null) keys.validate(state, name);

        if (values != null) values.validate(state, name);
      }
      ...
      public CQL3Type prepare(String keyspace, Types udts, boolean isInternal)
        ...
        // we represent supercolumns as maps, internally, and we do allow counters in supercolumns.
        // Thus,
        // for internal type parsing (think schema) we have to make an exception and allow counters
        // as (map) values
        ...
        if (keys != null) {
          ...
          if (!frozen && keys.supportsFreezing() && !keys.frozen) throwNestedNonFrozenError(keys);
        }
        ...
        switch (kind) {
          ...
          case MAP:
            ...
            return new Collection(
                MapType.getInstance(keys.prepare(keyspace, udts).getType(), valueType, !frozen));
        }
        throw new AssertionError();
      }
      ...
      private void throwNestedNonFrozenError(Raw innerType) {
        if (innerType instanceof RawCollection)
          ...
          throw new InvalidRequestException(
              "Non-frozen UDTs are not allowed inside collections: " + this);
      }
    ...
    private static class RawUT extends Raw {
      ...
      public CQL3Type prepare(String keyspace, Types udts) throws InvalidRequestException {
        if (name.hasKeyspace()) {
          // The provided keyspace is the one of the current statement this is part of. If it's
          // different from the keyspace of
          // the UTName, we reject since we want to limit user types to their own keyspace (see
          // #6643)
          if (!keyspace.equals(name.getKeyspace()))
            throw new InvalidRequestException(
                String.format(
                    "Statement on keyspace %s cannot refer to a user type in keyspace %s; "
      ...
      @Override
      public String toString() {
        if (frozen) return "frozen<" + name.toString() + '>';
    ...
    private static class RawTuple extends Raw {
      ...
      @Override
      public void validate(ClientState state, String name) {
        for (CQL3Type.Raw t : types) t.validate(state, name);
      }
