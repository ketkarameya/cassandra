...
public abstract class AbstractFunction implements Function {
  ...
  protected AbstractFunction(
      FunctionName name, List<AbstractType<?>> argTypes, AbstractType<?> returnType) {
    this.name = name;
  ...
  public final AssignmentTestable.TestResult testAssignment(
    ...
    // We should ignore the fact that the receiver type is frozen in our comparison as functions do
    // not support
   ...
   *
   * <p>For user function and aggregates tuples need to be handle in a special way as they are
   * frozen by nature but the frozen keyword should not appear in their CQL definition.
   *
   * @param type the type
  ...
  public boolean typesMatch(List<AbstractType<?>> types) {
    if (argTypes().size() != types.size()) return false;

    for (int i = 0; i < argTypes().size(); i++)
      if (!typesMatch(argTypes().get(i), types.get(i))) return false;
