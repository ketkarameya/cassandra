...
public class FunctionCall extends Term.NonTerminal {
  ...
  public ByteBuffer bindAndGet(QueryOptions options) throws InvalidRequestException {
    ...
    for (int i = 0, m = terms.size(); i < m; i++) {
      ...
      RequestValidations.checkBindValueSet(
          argument, "Invalid unset value for argument in call to function %s", fun.name().name);
      arguments.set(i, argument);
    }
    return executeInternal(fun, arguments);
  }

  private static ByteBuffer executeInternal(ScalarFunction fun, Arguments arguments)
    ...
    try {
      ...
      throw new RuntimeException(
          String.format(
              "Return of function %s (%s) is not a valid value for its declared return type %s",
  ...
  public static class Raw extends Term.Raw {
    ...
    public Term prepare(String keyspace, ColumnSpecification receiver)
      ...
      if (fun == null) throw invalidRequest("Unknown function %s called", name);
      if (fun.isAggregate())
      ...
      if (!scalarFun.testAssignment(keyspace, receiver).isAssignable()) {
        if (OperationFcts.isOperation(name))
          throw invalidRequest(
              ...
              OperationFcts.getOperator(scalarFun.name()),
              scalarFun.returnType().asCQL3Type(),
              receiver.name,
      ...
      for (int i = 0; i < terms.size(); i++) {
        Term t =
            terms
                ...
                .prepare(
                    keyspace,
                    FunctionResolver.makeArgSpec(receiver.ksName, receiver.cfName, scalarFun, i));
        parameters.add(t);
      }
    ...
    public AssignmentTestable.TestResult testAssignment(
      ...
      // Note: Functions.get() will return null if the function doesn't exist, or throw is no
      // function matching
      // the arguments can be found. We may get one of those if an undefined/wrong function is used
      // as argument
      // of another, existing, function. In that case, we return true here because we'll throw a
      // proper exception
      ...
      try {
        ...
        // Because the return type of functions built by factories is not fixed but depending on the
        // types of
        // their arguments, we'll always get EXACT_MATCH.  To handle potentially ambiguous function
        // calls with
        // dynamically built functions as an argument, always return WEAKLY_ASSIGNABLE to force the
        // user to
