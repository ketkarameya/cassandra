...
public final class FunctionResolver {
  ...
  public static ColumnSpecification makeArgSpec(
      String receiverKeyspace, String receiverTable, Function fun, int i) {
   ...
   * @param functions a set of user functions that is not yet available in the schema, used during
   *     startup when those functions might not be yet available
   */
  @Nullable
  public static Function get(
    ...
    Collection<Function> candidates =
        collectCandidates(
            keyspace, name, receiverKeyspace, receiverTable, providedArgs, receiverType, functions);
    ...
    return pickBestMatch(
        keyspace, name, providedArgs, receiverKeyspace, receiverTable, receiverType, candidates);
  }

  private static Collection<Function> collectCandidates(
      String keyspace,
      FunctionName name,
      String receiverKeyspace,
      String receiverTable,
    ...
    if (name.hasKeyspace()) {
      ...
      candidates.addAll(
          NativeFunctions.instance.getFactories(nativeName).stream()
              .map(
                  f ->
                      f.getOrCreateFunction(
                          providedArgs, receiverType, receiverKeyspace, receiverTable))
  ...
  private static Function pickBestMatch(
    ...
    for (Function toTest : candidates) {
      if (matchReturnType(toTest, receiverType)) {
        AssignmentTestable.TestResult r =
            matchAguments(keyspace, toTest, providedArgs, receiverKeyspace, receiverTable);
        switch (r) {
          case EXACT_MATCH:
    ...
    if (compatibles == null) {
      ...
      throw invalidRequest(
          "Invalid call to function %s, none of its type signatures match (known type signatures:"
              + " %s)",
    ...
    if (compatibles.size() > 1) {
      if (OperationFcts.isOperation(name)) {
        ...
        throw invalidRequest(
            "Ambiguous '%s' operation with args %s and %s: use type hint to disambiguate, example"
                + " '(int) ?'",
            OperationFcts.getOperator(name), providedArgs.get(0), providedArgs.get(1));
      }
      ...
      throw invalidRequest(
          "Ambiguous call to function %s (can be matched by following signatures: %s): use type"
   ...
   * @return {@code true} if if at least one of the specified arguments is a marker, {@code false}
   *     otherwise
   */
  ...
  /**
   * Checks that the return type of the specified function can be assigned to the specified
   * receiver.
   *
   * @param fun the function to check
   * @param receiverType the receiver type
   * @return {@code true} if the return type of the specified function can be assigned to the
   *     specified receiver, {@code false} otherwise.
   */
  private static boolean matchReturnType(Function fun, AbstractType<?> receiverType) {
    return receiverType == null
        || fun.returnType().testAssignment(receiverType.udfType()).isAssignable();
  }
  ...
  private static AssignmentTestable.TestResult matchAguments(
    ...
    // It's an exact match if all are exact match, but is not assignable as soon as any is not
    // assignable.
