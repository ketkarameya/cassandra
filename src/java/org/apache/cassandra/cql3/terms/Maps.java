...
public final class Maps {
  ...
  public static ColumnSpecification keySpecOf(ColumnSpecification column) {
    return new ColumnSpecification(
        column.ksName,
        column.cfName,
        new ColumnIdentifier("key(" + column.name + ')', true),
        keysType(column.type));
  }
   ...
   */
  public static <T extends AssignmentTestable> AssignmentTestable.TestResult testMapAssignment(
      ColumnSpecification receiver, List<Pair<T, T>> entries) {
    ...
    for (Pair<T, T> entry : entries) {
      ...
      if (t1 == AssignmentTestable.TestResult.NOT_ASSIGNABLE
          || t2 == AssignmentTestable.TestResult.NOT_ASSIGNABLE)
        return AssignmentTestable.TestResult.NOT_ASSIGNABLE;
      if (t1 != AssignmentTestable.TestResult.EXACT_MATCH
          || t2 != AssignmentTestable.TestResult.EXACT_MATCH)
  ...
  public static <T> String mapToString(
      List<Pair<T, T>> items, java.util.function.Function<T, String> mapper) {
    return items.stream()
   ...
   */
  public static <T> MapType<?, ?> getExactMapTypeIfKnown(
      List<Pair<T, T>> entries, java.util.function.Function<T, AbstractType<?>> mapper) {
  ...
  public static <T> MapType<?, ?> getPreferredCompatibleType(
      List<Pair<T, T>> entries, java.util.function.Function<T, AbstractType<?>> mapper) {
  ...
  public static class Literal extends Term.Raw {
    ...
    public Term prepare(String keyspace, ColumnSpecification receiver)
      ...
      // Whereas, internally maps are serialized as a lists where each key is followed by its value
      // (e.g. [k1, v1, k2, v2, ...])
      ...
      for (Pair<Term.Raw, Term.Raw> entry : entries) {
        ...
        if (k.containsBindMarker() || v.containsBindMarker())
          throw new InvalidRequestException(
              String.format(
                  "Invalid map literal for %s: bind variables are not supported inside collection"
                      + " literals",
                  receiver.name));
  ...
  public static class SetterByKey extends Operation {
    ...
    public void execute(DecoratedKey partitionKey, UpdateParameters params)
      ...
      assert column.type.isMultiCell()
          : "Attempted to set a value for a single key on a frozen map";
  ...
  public static class Putter extends Operation {
    ...
    public void execute(DecoratedKey partitionKey, UpdateParameters params)
      ...
      Term.Terminal value = t.bind(params.options);
      if (value != UNSET_VALUE) doPut(value, column, params);
    }

    static void doPut(Term.Terminal value, ColumnMetadata column, UpdateParameters params)
        throws InvalidRequestException {
  ...
  public static class DiscarderByKey extends Operation {
    ...
    public void execute(DecoratedKey partitionKey, UpdateParameters params)
      ...
      if (key == null) throw new InvalidRequestException("Invalid null map key");
      if (key == Constants.UNSET_VALUE) throw new InvalidRequestException("Invalid unset map key");
