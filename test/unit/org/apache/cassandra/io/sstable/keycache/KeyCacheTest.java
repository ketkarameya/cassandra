...
public class KeyCacheTest {
  ...
  public static void defineSchema() throws ConfigurationException {
    ...
    sstableImplCachesKeys =
        KeyCacheSupport.isSupportedBy(DatabaseDescriptor.getSelectedSSTableFormat());
  ...
  private void testKeyCacheLoad(String cf) throws Exception {
    ...
    for (Map.Entry<KeyCacheKey, AbstractRowIndexEntry> entry : savedMap.entrySet()) {
      ...
      for (int i = 0; i < expected.blockCount(); i++) {
        ...
        Assertions.assertThat(actualSstr.descriptor.version.format)
            .isEqualTo(expected.getSSTableFormat());
        if (actual instanceof RowIndexEntry) {
          try (RowIndexEntry.IndexInfoRetriever actualIir =
              ((RowIndexEntry) actual)
                  .openWithIndex(((BigTableReader) actualSstr).getIndexFile())) {
  ...
  public void testKeyCacheLoadZeroCacheLoadTime() throws Exception {
    ...
    // Here max time to load cache is zero which means no time left to load cache. So the keyCache
    // size should
  ...
  public void testKeyCacheLoadTwoTablesTime() throws Exception {
    ...
    // Here max time to load cache is negative which means no time left to load cache. So the
    // keyCache size should
    // be zero after load.
  ...
  public void testKeyCacheLoadCacheLoadTimeExceedingLimit() throws Exception {
    ...
    // Testing cache load. Here using custom built AutoSavingCache instance as simulating delay is
    // not possible with
    // 'CacheService.instance.keyCache'. 'AutoSavingCache.loadSaved()' is returning no.of entries
    // loaded so we don't need
    ...
    AutoSavingCache autoSavingCache =
        new AutoSavingCache(
            mock(ICache.class), CacheService.CacheType.KEY_CACHE, keyCacheSerializerSpy);

    doAnswer(new AnswersWithDelay(delayMillis, InvocationOnMock::callRealMethod))
        .when(keyCacheSerializerSpy)
        .deserialize(any(DataInputPlus.class));

    long maxExpectedKeyCache =
        Math.min(
            numberOfRows,
  ...
  private void createAndInvalidateCache(List<Pair<String, String>> tables, int numberOfRows)
  ...
  private static void readData(
      String keyspace, String columnFamily, int startRow, int numberOfRows) {
    ColumnFamilyStore store = Keyspace.open(keyspace).getColumnFamilyStore(columnFamily);
    for (int i = 0; i < numberOfRows; i++)
      Util.getAll(
          Util.cmd(store, "key" + (i + startRow)).includeRow("col" + (i + startRow)).build());
  }
  ...
  private void assertKeyCacheSize(int expected, String keyspace, String columnFamily) {
    ...
    for (Iterator<KeyCacheKey> iter = CacheService.instance.keyCache.keyIterator();
        iter.hasNext(); ) {
      KeyCacheKey k = iter.next();
      if (k.desc.ksname.equals(keyspace) && k.desc.cfname.equals(columnFamily)) size++;
    }
