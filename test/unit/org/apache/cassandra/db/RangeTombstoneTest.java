...
public class RangeTombstoneTest {
  ...
  public void rangeTombstoneFilteringTest() throws Exception {
    ...
    sb.add(
        ClusteringBound.create(cfs.getComparator(), true, true, 1),
        ClusteringBound.create(cfs.getComparator(), false, true, 10));
    sb.add(
        ClusteringBound.create(cfs.getComparator(), true, true, 16),
        ClusteringBound.create(cfs.getComparator(), false, true, 20));

    partition =
        Util.getOnlyPartitionUnfiltered(
            SinglePartitionReadCommand.create(
                cfs.metadata(), FBUtilities.nowInSeconds(), Util.dk(key), sb.build()));
    rt = rangeTombstones(partition);
    assertEquals(2, rt.size());
  }
  ...
  public void testTrackTimesPartitionTombstone() throws ExecutionException, InterruptedException {
    ...
    long nowInSec = FBUtilities.nowInSeconds();
    new Mutation(PartitionUpdate.fullPartitionDelete(cfs.metadata(), Util.dk(key), 1000, nowInSec))
        .apply();
    Util.flush(cfs);
  ...
  @Test
  public void testTrackTimesPartitionTombstoneWithData()
  ...
  public void testTrackTimesRangeTombstone() throws ExecutionException, InterruptedException {
    ...
    long nowInSec = FBUtilities.nowInSeconds();
    new RowUpdateBuilder(cfs.metadata(), nowInSec, 1000L, key)
        .addRangeTombstone(1, 2)
        .build()
        .apply();
    Util.flush(cfs);
  ...
  @Test
  public void testTrackTimesRangeTombstoneWithData()
      throws ExecutionException, InterruptedException {
    Keyspace ks = Keyspace.open(KSNAME);
    ColumnFamilyStore cfs = ks.getColumnFamilyStore(CFNAME);
    cfs.truncateBlocking();
  ...
  public void overlappingRangeTest() throws Exception {
    ...
    for (int i = 0; i < 5; i++)
      assertTrue(
          "Row " + i + " should be live",
          partition
              .getRow(Clustering.make(bb(i)))
              .hasLiveData(FBUtilities.nowInSeconds(), enforceStrictLiveness));
    for (int i = 16; i < 20; i++)
      assertTrue(
          "Row " + i + " should be live",
          partition
  ...
  public void reverseQueryTest() throws Exception {
    ...
    int last =
        i(
            partition
                .unfilteredIterator(ColumnFilter.all(cfs.metadata()), Slices.ALL, true)
                .next()
                .clustering()
                .bufferAt(0));
  ...
  public void testRowWithRangeTombstonesUpdatesSecondaryIndex() throws Exception {
    ...
    // If rebuild there is, wait for the rebuild to finish so it doesn't race with the following
    // insertions
  ...
  public void testRangeTombstoneCompaction() throws Exception {
    ...
    try (UnfilteredPartitionIterator scanner = sstable.getScanner()) {
      try (UnfilteredRowIterator iter = scanner.next()) {
        ...
        assertTrue(
            "Expecting open marker, got " + u1.toString(cfs.metadata()),
            u1 instanceof RangeTombstoneMarker);
        Unfiltered u2 = iter.next();
        assertTrue(
            "Expecting close marker, got " + u2.toString(cfs.metadata()),
            u2 instanceof RangeTombstoneMarker);
