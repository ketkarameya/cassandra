...
public class GuardrailMinimumReplicationFactorTest extends ThresholdTester {
  ...
  protected long currentValue() {
    return Long.parseLong(
        (Keyspace.open(KS).getReplicationStrategy()).configOptions.get(DATACENTER1));
  }
  ...
  public void testConfigValidation() {
    ...
    assertInvalidPositiveIntProperty(
        (g, a) -> setter.accept(g, DISABLED_GUARDRAIL, a),
        Integer.MIN_VALUE,
        Integer.MAX_VALUE,
        WHAT + "_fail_threshold");
    assertInvalidPositiveIntProperty(
        (g, a) -> setter.accept(g, DISABLED_GUARDRAIL, a),
        -2,
        Integer.MAX_VALUE,
    ...
    assertInvalidPositiveIntProperty(
        (g, w) -> setter.accept(g, w, DISABLED_GUARDRAIL),
        Integer.MIN_VALUE,
        Integer.MAX_VALUE,
        WHAT + "_warn_threshold");
    assertInvalidPositiveIntProperty(
        (g, w) -> setter.accept(g, w, DISABLED_GUARDRAIL),
        -2,
        Integer.MAX_VALUE,
    ...
    Assertions.assertThatThrownBy(() -> setter.accept(guardrails(), 1, 2))
        .hasMessageContaining(
            guardrail.name + "_warn_threshold should be greater than the fail threshold");
  }
  ...
  public void testMinKeyspaceRFDisabled() throws Throwable {
    ...
    assertMinThresholdValid(
        "CREATE KEYSPACE ks WITH replication = { 'class': 'NetworkTopologyStrategy', 'datacenter1':"
            + " 1}");
    assertMinThresholdValid(
        "ALTER KEYSPACE ks WITH replication = { 'class' : 'NetworkTopologyStrategy', 'datacenter1':"
            + " 3}");
  }
  ...
  public void testSimpleStrategyCreate() throws Throwable {
    guardrails()
        .setMinimumReplicationFactorThreshold(
            MINIMUM_REPLICATION_FACTOR_WARN_THRESHOLD, MINIMUM_REPLICATION_FACTOR_FAIL_THRESHOLD);
    assertWarns(
  ...
  public void testSimpleStrategyAlter() throws Throwable {
    ...
    execute(
        "CREATE KEYSPACE ks WITH replication = { 'class': 'SimpleStrategy', 'replication_factor':"
            + " 4}");
    assertWarns(
        "ALTER KEYSPACE ks WITH replication = { 'class': 'SimpleStrategy', 'replication_factor':"
            + " 3}",
        3);
    assertFails(
        "ALTER KEYSPACE ks WITH replication = { 'class': 'SimpleStrategy', 'replication_factor':"
  ...
  public void testMultipleDatacenter() throws Throwable {
    ...
    List<String> twoWarnings =
        Arrays.asList(
            format(
                "The keyspace %s has a replication factor of 2, below the warning threshold of %d.",
                KS, MINIMUM_REPLICATION_FACTOR_WARN_THRESHOLD),
            format(
                "The keyspace %s has a replication factor of 2, below the warning threshold of %d.",
                KS, MINIMUM_REPLICATION_FACTOR_WARN_THRESHOLD));
    ...
    assertValid(
        "CREATE KEYSPACE ks WITH replication = { 'class' : 'NetworkTopologyStrategy',"
            + " 'datacenter1': 4, 'datacenter2' : 4 }");
    execute("DROP KEYSPACE IF EXISTS ks");
    assertWarns(
        "CREATE KEYSPACE ks WITH replication = { 'class' : 'NetworkTopologyStrategy',"
            + " 'datacenter1': 4, 'datacenter2' : 2 }",
        2);
    execute("DROP KEYSPACE IF EXISTS ks");
    assertWarns(
        "CREATE KEYSPACE ks WITH replication = { 'class' : 'NetworkTopologyStrategy',"
  ...
  public void testMinKeyspaceRFOnlyWarnAbove() throws Throwable {
    guardrails()
        .setMinimumReplicationFactorThreshold(
            MINIMUM_REPLICATION_FACTOR_WARN_THRESHOLD, DISABLED_GUARDRAIL);
    assertMinThresholdValid(
        "CREATE KEYSPACE ks WITH replication = { 'class': 'NetworkTopologyStrategy', 'datacenter1':"
            + " 6}");
    assertMinThresholdValid(
  ...
  public void testMinKeyspaceRFOnlyFailAbove() throws Throwable {
    guardrails()
        .setMinimumReplicationFactorThreshold(
            DISABLED_GUARDRAIL, MINIMUM_REPLICATION_FACTOR_FAIL_THRESHOLD);
    assertMinThresholdValid(
        "CREATE KEYSPACE ks WITH replication = { 'class': 'NetworkTopologyStrategy', 'datacenter1':"
            + " 4}");
    assertMinThresholdValid(
  ...
  public void testMinRFGreaterThanDefaultRF() {
    ...
    Assertions.assertThatThrownBy(() -> guardrails().setMinimumReplicationFactorThreshold(5, 4))
        ...
        .hasMessageContaining(
            "minimum_replication_factor_fail_threshold to be set (4) cannot be greater than"
    ...
    Assertions.assertThatThrownBy(() -> DatabaseDescriptor.setDefaultKeyspaceRF(3))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining(
            "default_keyspace_rf to be set (3) cannot be less than"
  ...
  private void assertWarns(String query, int rf) throws Throwable {
    assertWarns(
        query,
        format(
            "The keyspace ks has a replication factor of %d, below the warning threshold of %s.",
  ...
  private void assertFails(String query, int rf) throws Throwable {
    assertFails(
        query,
        format(
            "The keyspace ks has a replication factor of %d, below the failure threshold of %s.",
