...
public class ShardedCompactionWriterTest extends CQLTester {
  ...
  public void before() {
    ...
    schemaChange(
        String.format(
            "CREATE TABLE IF NOT EXISTS %s.%s (k int, t int, v blob, PRIMARY KEY (k, t))",
            KEYSPACE, TABLE));
  }
  ...
  public void testOneSSTablePerShard() throws Throwable {
    // If we set the minSSTableSize ratio to 0.5, because this gets multiplied by the shard size to
    // give the min sstable size,
    // assuming evenly distributed data, it should split at each boundary and so we should end up
    // with numShards sstables
    int numShards = 5;
    int rowCount = 5000;
  ...
  private void testShardedCompactionWriter(
      int numShards, int rowCount, int numOutputSSTables, boolean majorCompaction)
      throws Throwable {
    ...
    LifecycleTransaction txn =
        cfs.getTracker().tryModify(cfs.getLiveSSTables(), OperationType.COMPACTION);

    ShardManager boundaries =
        new ShardManagerNoDisks(
            ColumnFamilyStore.fullWeightedRange(RING_VERSION_IRRELEVANT, cfs.getPartitioner()));
    ShardedCompactionWriter writer =
        new ShardedCompactionWriter(
            cfs,
            cfs.getDirectories(),
            txn,
            txn.originals(),
            false,
            boundaries.boundaries(numShards));

    int rows = compact(cfs, txn, writer);
    assertEquals(numOutputSSTables, cfs.getLiveSSTables().size());
    assertEquals(rowCount, rows);

    long totalOnDiskLength =
        cfs.getLiveSSTables().stream().mapToLong(SSTableReader::onDiskLength).sum();
    long totalBFSize =
        cfs.getLiveSSTables().stream().mapToLong(ShardedCompactionWriterTest::getFilterSize).sum();
    assert totalBFSize > 16 * numOutputSSTables
        : "Bloom Filter is empty"; // 16 is the size of empty bloom filter
    for (SSTableReader rdr : cfs.getLiveSSTables()) {
      assertEquals(
          (double) rdr.onDiskLength() / totalOnDiskLength,
          (double) getFilterSize(rdr) / totalBFSize,
          0.1);
  ...
  static long getFilterSize(SSTableReader rdr) {
    if (!(rdr instanceof SSTableReaderWithFilter)) return 0;
  ...
  public void testDiskAdvance() throws Throwable {
    ...
    final List<Token> diskBoundaries =
        cfs.getPartitioner().splitter().get().splitOwnedRanges(numDisks, localRanges, false);
    ...
    for (SSTableReader rdr : cfs.getLiveSSTables()) verifyNoSpannedBoundaries(diskBoundaries, rdr);

    Token selectionStart = diskBoundaries.get(0);
    Token selectionEnd = diskBoundaries.get(2);

    // Now compact only a section to trigger disk advance; shard needs to advance with disk, a
    // potential problem
    // is to create on-partition sstables at the start because shard wasn't advanced at the right
    // time.
    Set<SSTableReader> liveSSTables = cfs.getLiveSSTables();
    List<SSTableReader> selection =
        liveSSTables.stream()
            .filter(
                rdr ->
                    rdr.getFirst().getToken().compareTo(selectionStart) > 0
                        && rdr.getLast().getToken().compareTo(selectionEnd) <= 0)
            .collect(Collectors.toList());
    List<SSTableReader> remainder =
        liveSSTables.stream().filter(rdr -> !selection.contains(rdr)).collect(Collectors.toList());
    ...
    assertEquals(
        rowCount * 2.0 / numDisks,
        rows * 1.0,
        rowCount / 20.0); // should end up with roughly this many rows

    long totalOnDiskLength =
        compactedSelection.stream().mapToLong(SSTableReader::onDiskLength).sum();
    long totalBFSize =
        compactedSelection.stream().mapToLong(ShardedCompactionWriterTest::getFilterSize).sum();
  ...
  private int compact(
      ...
      Collection<SSTableReader> selection) {
    int rows;
  ...
  private static void verifyNoSpannedBoundaries(List<Token> diskBoundaries, SSTableReader rdr) {
    for (int i = 0; i < diskBoundaries.size(); ++i) {
      ...
      // rdr cannot span a boundary. I.e. it must be either fully before (last <= boundary) or fully
      // after
      // (first > boundary).
      assertTrue(
          rdr.getFirst().getToken().compareTo(boundary) > 0
              || rdr.getLast().getToken().compareTo(boundary) <= 0);
    }
  }

  private int compact(
      ColumnFamilyStore cfs, LifecycleTransaction txn, CompactionAwareWriter writer) {
    // assert txn.originals().size() == 1;
    int rowsWritten = 0;
    long nowInSec = FBUtilities.nowInSeconds();
    try (AbstractCompactionStrategy.ScannerList scanners =
            cfs.getCompactionStrategyManager().getScanners(txn.originals());
        CompactionController controller =
            new CompactionController(cfs, txn.originals(), cfs.gcBefore(nowInSec));
        CompactionIterator ci =
            new CompactionIterator(
                ...
                controller,
                nowInSec,
                TimeUUID.minAtUnixMillis(System.currentTimeMillis()))) {
      while (ci.hasNext()) {
        if (writer.append(ci.next())) rowsWritten++;
      }
    }
    writer.finish();
    return rowsWritten;
  ...
  private void populate(int count, boolean compact) throws Throwable {
    ...
    for (int i = 0; i < count; i++) {
      for (int j = 0; j < ROW_PER_PARTITION; j++)
        execute(
            String.format("INSERT INTO %s.%s(k, t, v) VALUES (?, ?, ?)", KEYSPACE, TABLE), i, j, b);

      if (i % (count / 4) == 0) cfs.forceBlockingFlush(ColumnFamilyStore.FlushReason.UNIT_TESTS);
    }
  ...
  private void validateData(ColumnFamilyStore cfs, int rowCount) throws Throwable {
    for (int i = 0; i < rowCount; i++) {
      ...
      assertRows(
          execute(String.format("SELECT k, t FROM %s.%s WHERE k = :i", KEYSPACE, TABLE), i),
          expected);
    }
  }
}
