...
public class ByteSourceComparisonTest extends ByteSourceTestBase {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ByteSourceComparisonTest.class);
  ...
  public void testStringsUTF8() {
    ...
    testDirect(
        x -> ByteSource.of(x, Version.OSS50), Ordering.<String>natural()::compare, testStrings);
  }
  ...
  public void testTimeType() {
    testType(
        TimeType.instance,
        Arrays.stream(testLongs)
            .filter(x -> x != null && x >= 0 && x <= 24L * 60 * 60 * 1000 * 1000 * 1000)
            .toArray());
  }
  ...
  public void testBytesType() {
    ...
    for (int i = 0; i < testValues.length; ++i)
      for (Object o : testValues[i]) values.add(testTypes[i].decompose(o));
  ...
  public void testLocalPatitionerOrder() {
    for (int i = 0; i < testValues.length; ++i) {
      ...
      testDecoratedKeys(
          new LocalPartitioner(testType),
          Lists.transform(Arrays.asList(testValues[i]), v -> testType.decompose(v)));
    }
  }
  ...
  public void testNullsInClustering() {
    ByteBuffer[][] inputs =
        new ByteBuffer[][] {
          ...
          new ByteBuffer[] {decomposeAndRandomPad(UTF8Type.instance, "a"), null},
          new ByteBuffer[] {
          ...
          new ByteBuffer[] {decomposeAndRandomPad(UTF8Type.instance, ""), null},
          new ByteBuffer[] {null, decomposeAndRandomPad(Int32Type.instance, 0)},
          new ByteBuffer[] {null, decomposeAndRandomPad(Int32Type.instance, null)},
          new ByteBuffer[] {null, null}
        };
    for (ByteBuffer[] input1 : inputs)
      for (ByteBuffer[] input2 : inputs) {
        assertClusteringPairComparesSame(
            UTF8Type.instance,
            Int32Type.instance,
            input1[0],
            input1[1],
  ...
  public void testNullsInClusteringLegacy() {
    ...
    ClusteringPrefix<ByteBuffer> aNull =
        makeBound(
            ClusteringPrefix.Kind.CLUSTERING,
            decomposeAndRandomPad(UTF8Type.instance, "a"),
            decomposeAndRandomPad(Int32Type.instance, null));
    ClusteringPrefix<ByteBuffer> aEmpty =
        makeBound(
            ClusteringPrefix.Kind.CLUSTERING, decomposeAndRandomPad(UTF8Type.instance, "a"), null);
    ClusteringComparator comp = new ClusteringComparator(UTF8Type.instance, Int32Type.instance);
    assertEquals(
        0,
        ByteComparable.compare(
            comp.asByteComparable(aNull), comp.asByteComparable(aEmpty), Version.LEGACY));
    ClusteringComparator compReversed =
        new ClusteringComparator(UTF8Type.instance, ReversedType.getInstance(Int32Type.instance));
    assertEquals(
        0,
        ByteComparable.compare(
            compReversed.asByteComparable(aNull),
            compReversed.asByteComparable(aEmpty),
            Version.LEGACY));
  }
  ...
  private void assertEmptyComparedToStatic(
      int expected, ClusteringPrefix.Kind kind, Version version) {
    ...
    assertEquals(
        ...
        Integer.signum(
            ByteComparable.compare(
                compEmpty.asByteComparable(empty),
                compEmpty.asByteComparable(Clustering.STATIC_CLUSTERING),
                version)));
  }

  void assertClusteringPairComparesSame(
      AbstractType<?> t1, AbstractType<?> t2, Object o1, Object o2, Object o3, Object o4) {
  ...
  void assertClusteringPairComparesSame(
      ...
      boolean testLegacy) {
    EnumSet<ClusteringPrefix.Kind> skippedKinds =
        EnumSet.of(
            ClusteringPrefix.Kind.SSTABLE_LOWER_BOUND, ClusteringPrefix.Kind.SSTABLE_UPPER_BOUND);
    for (Version v : Version.values())
      for (ClusteringPrefix.Kind k1 : EnumSet.complementOf(skippedKinds))
        for (ClusteringPrefix.Kind k2 : EnumSet.complementOf(skippedKinds)) {
          if (!testLegacy && v == Version.LEGACY) continue;
          ...
          assertEquals(
              String.format(
                  "Failed comparing %s and %s, %s vs %s version %s",
                  safeStr(c.clusteringString(comp.subtypes())),
                  safeStr(e.clusteringString(comp.subtypes())),
                  bsc,
                  bse,
                  v),
              expected,
          ...
          ClusteringComparator compR =
              new ClusteringComparator(ReversedType.getInstance(t1), ReversedType.getInstance(t2));
          ...
          assertEquals(
              String.format(
                  "Failed comparing reversed %s and %s, %s vs %s version %s",
                  safeStr(c.clusteringString(comp.subtypes())),
                  safeStr(e.clusteringString(comp.subtypes())),
                  bsrc,
                  bsre,
  ...
  static <T> ClusteringPrefix<T> makeBound(
      ValueAccessor.ObjectFactory<T> factory, ClusteringPrefix.Kind k1, T[] b) {
    switch (k1) {
  ...
  public void testTupleTypeNonFull() {
    ...
    List<ByteBuffer> tests =
        ImmutableList.of(
            tt.pack(
                decomposeAndRandomPad(UTF8Type.instance, ""),
                decomposeAndRandomPad(Int32Type.instance, 0)),
            // Note: a decomposed null (e.g. decomposeAndRandomPad(Int32Type.instance, null)) should
            // not reach a tuple
  ...
  public void testTupleNewField() {
    ...
    assertEquals(
        "The byte-comparable version of a one-field tuple must be the same as a two-field tuple"
            + " with non-present second component.",
        bOne1.byteComparableAsString(Version.OSS50),
        bOne2.byteComparableAsString(Version.OSS50));
    assertEquals(
        "The byte-comparable version of a one-field tuple must be the same as a two-field tuple"
            + " with null as second component.",
        bOne1.byteComparableAsString(Version.OSS50),
        bOneAndNull2.byteComparableAsString(Version.OSS50));
  }

  void assertTupleComparesSame(
      AbstractType t1, AbstractType t2, Object o1, Object o2, Object o3, Object o4) {
    ...
    List<ByteBuffer> tests =
        ImmutableList.of(
            ...
            CompositeType.build(
                ...
                decomposeAndRandomPad(Int32Type.instance, null)),
            CompositeType.build(
                ByteBufferAccessor.instance, decomposeAndRandomPad(UTF8Type.instance, "")),
            CompositeType.build(ByteBufferAccessor.instance),
            CompositeType.build(
                ByteBufferAccessor.instance, true, decomposeAndRandomPad(UTF8Type.instance, "")),
            CompositeType.build(ByteBufferAccessor.instance, true));
    for (ByteBuffer b : tests) tt.validate(b);
    testBuffers(tt, tests);
  }

  void assertCompositeComparesSame(
      AbstractType t1, AbstractType t2, Object o1, Object o2, Object o3, Object o4) {
    CompositeType tt = CompositeType.getInstance(t1, t2);
    ByteBuffer b1 =
        CompositeType.build(
            ByteBufferAccessor.instance,
            decomposeAndRandomPad(t1, o1),
            decomposeAndRandomPad(t2, o2));
    ByteBuffer b2 =
        CompositeType.build(
            ByteBufferAccessor.instance,
            decomposeAndRandomPad(t1, o3),
            decomposeAndRandomPad(t2, o4));
    assertComparesSameBuffers(tt, b1, b2);
  }
  ...
  public void testListTypeLong() {
    testCollection(
        ListType.getInstance(LongType.instance, true),
        testLongs,
        () -> new ArrayList<>(),
        new Random());
  }
  ...
  <T, CT extends Collection<T>> void testCollection(
      CollectionType<CT> tt, T[] values, Supplier<CT> gen, Random rand) {
    int cnt = 0;
    ...
    for (int c = 1; c <= 3; ++c)
      for (int j = 0; j < 5; ++j) {
        CT l = gen.get();
        for (int i = 0; i < c; ++i) l.add(values[cnt++ % values.length]);
  ...
  public void testMapTypeStringLong() {
    testMap(
        MapType.getInstance(UTF8Type.instance, LongType.instance, true),
        testStrings,
        testLongs,
        () -> new HashMap<>(),
        new Random());
  ...
  public void testSeparatorPrefixViaDiffPoint() {
    testSeparator(
        (x, y) ->
            version ->
                ByteSource.cut(
                    y.asComparableBytes(version), ByteComparable.diffPoint(x, y, version)),
        testLongs,
        LongType.instance);
  }
  ...
  public void testSeparatorNext() {
    ...
    testSeparator(
        (x, y) ->
            version ->
                ByteSource.cutOrRightPad(
                    x.asComparableBytes(version), ByteComparable.length(x, version) + 1, 0),
        testLongs,
        LongType.instance);
  }

  private <T> void testSeparator(
      BiFunction<ByteComparable, ByteComparable, ByteComparable> separatorMethod,
      T[] testValues,
      AbstractType<T> type) {
    for (T v1 : testValues)
      for (T v2 : testValues) {
        if (v1 == null || v2 == null) continue;
        if (type.compare(type.decompose(v1), type.decompose(v2)) >= 0) continue;
        ...
        for (Version version : Version.values()) {
          ...
          Assert.assertTrue(
              String.format(
                  "Separator %s must be greater than left %s (for %s) (version %s)",
          ...
          Assert.assertTrue(
              String.format(
                  "Separator %s must be less than or equal to right %s (for %s) (version %s)",
                  separator.byteComparableAsString(version),
  ...
  public void testDecoratedKeyPrefixes(String key, String append, Version version) {
    ...
    ByteBuffer collision =
        Util.generateMurmurCollision(original, append.getBytes(StandardCharsets.UTF_8));
    ...
    logger.info(
        ...
        kk1,
        kk2,
        kk1.byteComparableAsString(version),
        kk2.byteComparableAsString(version));
  ...
  private void maybeCheck41Properties(
      int expectedComparison, ByteComparable s1, ByteComparable s2, Version version) {
    if (version != Version.OSS50) return;

    if (s1 == null || s2 == null || 0 == expectedComparison) return;
    int b1 = randomTerminator();
    int b2 = randomTerminator();
    assertEquals(
        String.format(
            "Comparison failed for %s(%s + %02x) and %s(%s + %02x)",
            s1, s1.byteComparableAsString(version), b1, s2, s2.byteComparableAsString(version), b2),
        expectedComparison,
        Integer.signum(
            compare(
                ByteSource.withTerminator(b1, s1.asComparableBytes(version)),
                ByteSource.withTerminator(b2, s2.asComparableBytes(version)))));
    assertNotPrefix(
        ByteSource.withTerminator(b1, s1.asComparableBytes(version)),
        ByteSource.withTerminator(b2, s2.asComparableBytes(version)));
  }
    ...
    do {
      term =
          ThreadLocalRandom.current()
              .nextInt(ByteSource.MIN_SEPARATOR, ByteSource.MAX_SEPARATOR + 1);
    } while (term >= ByteSource.MIN_NEXT_COMPONENT && term <= ByteSource.MAX_NEXT_COMPONENT);
    return term;
  }

  <K, V, M extends Map<K, V>> void testMap(
      MapType<K, V> tt, K[] keys, V[] values, Supplier<M> gen, Random rand) {
  ...
  public <T> void testType(AbstractType<? super T> type, Iterable<T> values) {
    ...
    for (T i : values) for (T j : values) assertComparesSame(type, i, j);
    if (!type.isReversed()) testType(ReversedType.getInstance(type), values);
  }
  ...
  public void testBuffers(AbstractType<?> type, List<ByteBuffer> values) {
    ...
    for (ByteBuffer i : values) for (ByteBuffer j : values) assertComparesSameBuffers(type, i, j);
  }
  ...
  public void testDecoratedKeys(IPartitioner type, List<ByteBuffer> values) {
    for (ByteBuffer i : values)
      for (ByteBuffer j : values) assertComparesSameDecoratedKeys(type, i, j);
    for (ByteBuffer i : values) assertDecoratedKeyBounds(type, i);
  }
  ...
  void assertComparesSameDecoratedKeys(IPartitioner type, ByteBuffer b1, ByteBuffer b2) {
    ...
    for (Version version : Version.values()) {
      ...
      assertEquals(
          String.format(
              "Failed comparing %s[%s](%s) and %s[%s](%s)\npartitioner %s version %s",
              ByteBufferUtil.bytesToHex(b1),
              k1,
  ...
  void assertDecoratedKeyBounds(IPartitioner type, ByteBuffer b) {
    ...
    assertEquals(
        String.format(
            "Failed comparing bound before (%s) for %s[%s](%s)\npartitioner %s version %s",
    ...
    assertEquals(
        String.format(
            "Failed comparing bound after (%s) for %s[%s](%s)\npartitioner %s version %s",
    ...
    assertEquals(
        String.format(
            "Failed comparing bound before (%s) to after (%s) for %s[%s](%s)\n"
                + "partitioner %s version %s",
            before.byteComparableAsString(version),
            after.byteComparableAsString(version),
            ByteBufferUtil.bytesToHex(b),
  ...
  public <T> void testDirect(
      Function<T, ByteSource> convertor, BiFunction<T, T, Integer> comparator, T[] values) {
    for (T i : values) {
      if (i == null) continue;

      logger.info("Value {} ByteSource {}\n", safeStr(i), convertor.apply(i));
    }
    for (T i : values)
      if (i != null)
        for (T j : values) if (j != null) assertComparesSame(convertor, comparator, i, j);
  }

  <T> void assertComparesSame(
      Function<T, ByteSource> convertor, BiFunction<T, T, Integer> comparator, T v1, T v2) {
  ...
  <T> void assertComparesSame(AbstractType<T> type, T v1, T v2) {
    ...
    for (Version version : Version.values()) {
      ...
      if (expected != actual) {
        if (type.isReversed()) {
          // This can happen for reverse of nulls and prefixes. Check that it's ok within
          // multi-component
          ...
          if (actualcc == expected) return;
          assertEquals(
              String.format(
                  "Failed comparing reversed %s(%s, %s) and %s(%s, %s) direct (%d) and as"
                      + " clustering",
                  safeStr(v1),
                  ByteBufferUtil.bytesToHex(b1),
                  c1,
          ...
          assertEquals(
              String.format(
                  "Failed comparing %s(%s BC %s) and %s(%s BC %s) version %s",
                  safeStr(v1),
                  ByteBufferUtil.bytesToHex(b1),
                  bc1.byteComparableAsString(version),
                  safeStr(v2),
