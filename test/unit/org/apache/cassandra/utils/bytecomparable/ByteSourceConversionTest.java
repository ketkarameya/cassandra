...
public class ByteSourceConversionTest extends ByteSourceTestBase {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ByteSourceConversionTest.class);
        ...
        Arrays.stream(testStrings)
            .filter(
                s ->
                    s.equals(
                        new String(
                            s.getBytes(StandardCharsets.US_ASCII), StandardCharsets.US_ASCII)))
            .toArray());
  }
  ...
  public void testNullsInClustering() {
    ...
    for (ByteBuffer[] input : inputs) {
      assertClusteringPairConvertsSame(
  ...
  public void testEmptyClustering() {
    ...
    for (ClusteringPrefix.Kind kind : EnumSet.complementOf(skippedKinds)) {
      ...
      ClusteringPrefix<ByteBuffer> converted =
          getClusteringPrefix(accessor, kind, comp, comp.asByteComparable(empty));
  ...
  <V> void assertClusteringPairConvertsSame(
    ...
    for (ClusteringPrefix.Kind k1 : EnumSet.complementOf(skippedKinds)) {
      ...
      assertEquals(
          String.format(
              "Failed reverse compare(%s, converted %s ByteSource %s) == 0\ntype %s",
              ...
              compR),
          0,
          compR.compare(c, converted));
      if (checkEquals)
        assertEquals(
            String.format(
                "Failed reverse equals %s, got %s ByteSource %s\ntype %s",
                safeStr(c.clusteringString(compR.subtypes())),
                safeStr(converted.clusteringString(compR.subtypes())),
                bsrc.byteComparableAsString(VERSION),
                compR),
  ...
  private static <V> ClusteringPrefix<V> getClusteringPrefix(
      ValueAccessor<V> accessor,
    ...
    for (int c = 1; c <= 3; ++c)
      for (int j = 0; j < 5; ++j) {
        ...
        for (int i = 0; i < c; ++i) {
          ...
          if (value != null) l.put(keys[rand.nextInt(keys.length)], value);
        }
  ...
  public void testBuffers(AbstractType<?> type, List<ByteBuffer> values) {
    ...
    for (ByteBuffer i : values) assertConvertsSameBuffers(type, i);
  }
  ...
  void assertConvertsSameBuffers(AbstractType<?> type, ByteBuffer b1) {
    ...
    assertEquals(
        String.format(
            "Failed compare(%s, converted %s (bytesource %s))",
  ...
  public void testDecoratedKeys(IPartitioner type, List<ByteBuffer> values) {
    for (ByteBuffer i : values) assertConvertsSameDecoratedKeys(type, i);
  }
  ...
  void assertConvertsSameDecoratedKeys(IPartitioner type, ByteBuffer b1) {
    ...
    assertEquals(
        String.format(
            "Failed compare(%s[%s bs %s], %s[%s bs %s])\npartitioner %s",
            k1,
            ByteBufferUtil.bytesToHex(b1),
    ...
    assertEquals(
        String.format(
            "Failed equals(%s[%s bs %s], %s[%s bs %s])\npartitioner %s",
            k1,
            ByteBufferUtil.bytesToHex(b1),
            k1.byteComparableAsString(VERSION),
            actual,
  ...
  public <T> void testDirect(
      Function<T, ByteSource> convertor, Function<ByteSource.Peekable, T> inverse, T[] values) {
    for (T i : values) {
      if (i == null) continue;

      logger.info("Value {} ByteSource {}\n", safeStr(i), convertor.apply(i));
    }
    for (T i : values) if (i != null) assertConvertsSame(convertor, inverse, i);
  }

  <T> void assertConvertsSame(
      Function<T, ByteSource> convertor, Function<ByteSource.Peekable, T> inverse, T v1) {
