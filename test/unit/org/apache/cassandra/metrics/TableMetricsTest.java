...
public class TableMetricsTest {
  ...
  public static void setup() throws ConfigurationException, IOException {
    ...
    session.execute(
        String.format(
            "CREATE TABLE IF NOT EXISTS %s.%s (id_c counter, id int, val text, PRIMARY KEY(id,"
                + " val));",
            KEYSPACE, COUNTER_TABLE));
  }
  ...
  private ColumnFamilyStore recreateTable(String table) {
    ...
    session.execute(
        String.format(
            "CREATE TABLE IF NOT EXISTS %s.%s (id int, val1 text, val2 text, PRIMARY KEY(id,"
                + " val1));",
            KEYSPACE, table));
  ...
  private void executeBatch(
      boolean isLogged, int distinctPartitions, int statementsPerPartition, String... tables) {
  ...
  private static void populateBatch(
      BatchStatement batch, String table, int distinctPartitions, int statementsPerPartition) {
    PreparedStatement ps =
        session.prepare(
            String.format("INSERT INTO %s.%s (id, val1, val2) VALUES (?, ?, ?);", KEYSPACE, table));
  ...
  public void testRegularStatementsExecuted() {
    ...
    assertRowsContains(
        cluster,
        session.execute("SELECT * FROM system_metrics.table_group"),
        row(
            "org.apache.cassandra.metrics.Table.CoordinatorWriteLatency.junit.tablemetricstest",
            "junit.tablemetricstest",
            "timer",
            String.valueOf(cfs.metric.coordinatorWriteLatency.getCount())));
    assertRowsContains(
        cluster,
        session.execute("SELECT * FROM system_metrics.column_family_group"),
        row(
            "org.apache.cassandra.metrics.ColumnFamily.CoordinatorWriteLatency.junit.tablemetricstest",
            "junit.tablemetricstest",
            "timer",
            String.valueOf(cfs.metric.coordinatorWriteLatency.getCount())));
  }
  ...
  public void testMaxSSTableSize() throws Exception {
    ...
    assertRowsContains(
        ...
        session.execute("SELECT * FROM system_metrics.table_group"),
        row(
            "org.apache.cassandra.metrics.Table.MaxSSTableSize.junit.tablemetricstest",
            "junit.tablemetricstest",
            "gauge",
    ...
    assertRowsContains(
        ...
        session.execute("SELECT * FROM system_metrics.column_family_group"),
        row(
            "org.apache.cassandra.metrics.ColumnFamily.MaxSSTableSize.junit.tablemetricstest",
            "junit.tablemetricstest",
            "gauge",
            String.valueOf(cfs.metric.maxSSTableSize.getValue())));
  }
  ...
  public void testMaxSSTableDuration() throws Exception {
    ...
    session.execute(
        String.format(
            "INSERT INTO %s.%s (id, val1, val2) VALUES (%d, '%s', '%s')",
            KEYSPACE, TWCS_TABLE, 1, "val1", "val1"));
    Uninterruptibles.sleepUninterruptibly(10, TimeUnit.SECONDS);
    session.execute(
        String.format(
            "INSERT INTO %s.%s (id, val1, val2) VALUES (%d, '%s', '%s')",
            KEYSPACE, TWCS_TABLE, 2, "val2", "val2"));
    ...
    assertRowsContains(
        ...
        session.execute("SELECT * FROM system_metrics.table_group"),
        row(
            "org.apache.cassandra.metrics.Table.MaxSSTableDuration.junit.tablemetricstesttwcs",
            "junit.tablemetricstesttwcs",
            "gauge",
    ...
    assertRowsContains(
        ...
        session.execute("SELECT * FROM system_metrics.column_family_group"),
        row(
            "org.apache.cassandra.metrics.ColumnFamily.MaxSSTableDuration.junit.tablemetricstesttwcs",
            "junit.tablemetricstesttwcs",
            "gauge",
            String.valueOf(cfs.metric.maxSSTableDuration.getValue())));
  }
  ...
  public void testLoggedPartitionsPerBatch() {
    ...
    assertEquals(
        2, cfs.metric.coordinatorWriteLatency.getCount()); // 2 for previous batch and this batch
    assertGreaterThan(cfs.metric.coordinatorWriteLatency.getMeanRate(), 0);
  }
  ...
  public void testLoggedPartitionsPerBatchMultiTable() {
    ...
    assertEquals(
        2, first.metric.coordinatorWriteLatency.getCount()); // 2 for previous batch and this batch
    assertEquals(
        2, second.metric.coordinatorWriteLatency.getCount()); // 2 for previous batch and this batch
  ...
  public void testCounterStatement() {
    ...
    session.execute(
        String.format(
            "UPDATE %s.%s SET id_c = id_c + 1 WHERE id = 1 AND val = 'val1'",
            KEYSPACE, COUNTER_TABLE));
    assertEquals(1, cfs.metric.coordinatorWriteLatency.getCount());
    assertGreaterThan(cfs.metric.coordinatorWriteLatency.getMeanRate(), 0);
  }
  ...
  public void testViewMetricsCleanupOnDrop() {
    ...
    session.execute(
        String.format(
            "CREATE MATERIALIZED VIEW %s.%s AS SELECT id,val1 FROM %s.%s WHERE id IS NOT NULL AND"
                + " val1 IS NOT NULL PRIMARY KEY (id,val1);",
            KEYSPACE, viewName, KEYSPACE, tableName));
    // some metrics
    assertTrue(metrics.get().count() > 0);
