...
public class AuthorizationProxyTest {
  ...
  Set<ObjectName> allBeans =
      objectNames(osBean, runtimeBean, threadingBean, hintsBean, batchlogBean, customBean);
  ...
  public void roleDoesNotHaveRequiredPermission() throws Throwable {
    Map<RoleResource, Set<PermissionDetails>> permissions =
        ImmutableMap.of(
            role1, Collections.singleton(permission(role1, osBean, Permission.AUTHORIZE)));
  ...
  public void roleHasRequiredPermissionOnRootResource() throws Throwable {
    Map<RoleResource, Set<PermissionDetails>> permissions =
        ImmutableMap.of(
            role1, Collections.singleton(permission(role1, JMXResource.root(), Permission.SELECT)));
  ...
  public void roleHasOtherPermissionOnRootResource() throws Throwable {
    Map<RoleResource, Set<PermissionDetails>> permissions =
        ImmutableMap.of(
            role1,
            Collections.singleton(permission(role1, JMXResource.root(), Permission.AUTHORIZE)));
  ...
  public void rejectWhenSubjectNotAuthenticated() throws Throwable {
    ...
    AuthorizationProxy proxy =
        new ProxyBuilder()
            ...
            .build();
    assertFalse(
        proxy.authorize(new Subject(), "getAttribute", new Object[] {objectName(osBean), "arch"}));
    assertFalse(suStatusChecked.get());
  }
  ...
  public void authorizeWhenWildcardGrantCoversExactTarget() throws Throwable {
    Map<RoleResource, Set<PermissionDetails>> permissions =
        ImmutableMap.of(
            role1, Collections.singleton(permission(role1, javaLangWildcard, Permission.SELECT)));
  ...
  public void authorizeWhenWildcardGrantCoversWildcardTarget() throws Throwable {
    Map<RoleResource, Set<PermissionDetails>> permissions =
        ImmutableMap.of(
            role1, Collections.singleton(permission(role1, javaLangWildcard, Permission.DESCRIBE)));
  ...
  public void rejectWhenWildcardGrantIsDisjointWithWildcardTarget() throws Throwable {
    ...
    Map<RoleResource, Set<PermissionDetails>> permissions =
        ImmutableMap.of(
            role1, Collections.singleton(permission(role1, customWildcard, Permission.DESCRIBE)));
  ...
  public void rejectWhenWildcardGrantIntersectsWithWildcardTarget() throws Throwable {
    ...
    Map<RoleResource, Set<PermissionDetails>> permissions =
        ImmutableMap.of(
            role1,
            ImmutableSet.of(
                permission(role1, allCustomBeans, Permission.DESCRIBE),
    ...
    assertFalse(
        proxy.authorize(
            subject(role1.getRoleName()), "queryNames", new Object[] {allCassandraBeans, null}));
  }
  ...
  public void rejectInvocationOfUnknownMethod() throws Throwable {
    ...
    Set<PermissionDetails> allPerms =
  ...
  public void rejectInvocationOfRestrictedMethods() throws Throwable {
    ...
    AuthorizationProxy proxy =
        new ProxyBuilder().isAuthzRequired(() -> true).isSuperuser((role) -> true).build();
  ...
  private void testNonMbeanMethods(boolean withPermission) {
    ...
    ProxyBuilder builder =
        new ProxyBuilder().isAuthzRequired(() -> true).isSuperuser((role) -> false);
    if (withPermission) {
      Map<RoleResource, Set<PermissionDetails>> permissions =
          ImmutableMap.of(
              role1, ImmutableSet.of(permission(role1, JMXResource.root(), Permission.DESCRIBE)));
  ...
  private static PermissionDetails permission(
      RoleResource grantee, IResource resource, Permission permission) {
  ...
  private static Set<ObjectName> objectNames(JMXResource... resource) {
    ...
    try {
      for (JMXResource r : resource) names.add(objectName(r));
  ...
  public static class ProxyBuilder {
    ...
    AuthorizationProxy build() {
      ...
      if (getPermissions != null) proxy.setGetPermissions(getPermissions);

      if (queryNames != null) proxy.setQueryNames(queryNames);

      if (isSuperuser != null) proxy.setIsSuperuser(isSuperuser);

      if (isAuthzRequired != null) proxy.setIsAuthzRequired(isAuthzRequired);
