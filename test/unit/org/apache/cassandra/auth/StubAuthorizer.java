...
public class StubAuthorizer implements IAuthorizer {
  ...
  public Set<Permission> grant(
    ...
    Set<Permission> oldPermissions =
        userPermissions.computeIfAbsent(key, k -> Collections.emptySet());
    Set<Permission> nonExisting = Sets.difference(permissions, oldPermissions);

    if (!nonExisting.isEmpty()) userPermissions.put(key, Sets.union(oldPermissions, nonExisting));
  ...
  public Set<Permission> revoke(
    ...
    if (existing.isEmpty()) userPermissions.remove(key);
    else userPermissions.put(key, Sets.difference(oldPermissions, existing));
  ...
  public void revokeAllFrom(RoleResource revokee) {
    for (Pair<String, IResource> key : userPermissions.keySet())
      if (key.left.equals(revokee.getRoleName())) userPermissions.remove(key);
  }

  public void revokeAllOn(IResource droppedResource) {
    for (Pair<String, IResource> key : userPermissions.keySet())
      if (key.right.equals(droppedResource)) userPermissions.remove(key);
  }
