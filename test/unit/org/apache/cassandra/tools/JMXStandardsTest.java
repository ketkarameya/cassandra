...
public class JMXStandardsTest {
  ...
  /**
   * JMX typlically works well with java.* and javax.*, but not all types are serializable and will
   * work, so this class goes with a explicit approval list, new APIs may fail if a java.* or
   * javax.* is used not in this allow list, if that is the case it is fine to add here.
   *
   * <p>It is never fine to allow non java.* and javax.* types, they can not be handled by clients,
  ...
  private static final Set<Class<?>> ALLOWED_TYPES =
      ImmutableSet.<Class<?>>builder()
          ...
          .add(ObjectName.class)
          .add(TabularData.class)
          .add(CompositeData.class)
          // Exceptions
  ...
  /**
   * This list is a set of types under java.* and javax.*, but are too vague that could cause
   * issues; this does not mean issues will happen with JMX, only that issues may happen only after
   * running and can not be detected at compile time.
   */
  ...
  public void interfaces() throws ClassNotFoundException {
    ...
    for (String className : matches) {
      for (Class<?> klass = Class.forName(className);
          klass != null && !Object.class.equals(klass);
        ...
        for (int i = 0; i < methods.length; i++) {
          ...
          Stream.of(method.getGenericParameterTypes())
              .forEach(t -> checkType(method, "parameter", t, warnings, errors));
          Stream.of(method.getGenericExceptionTypes())
              .forEach(t -> checkType(method, "throws", t, warnings, errors));
        }
      }
    }
    if (!warnings.isEmpty()) warnings.forEach(logger::warn);
    if (!errors.isEmpty())
      throw new AssertionError(
          "Errors detected while validating MBeans\n" + String.join("\n", errors));
  }
  ...
  private static void checkType(
    ...
    if (type instanceof Class<?>) {
      ...
      if (!ALLOWED_TYPES.contains(klass)) {
        ...
        for (int i = 0; i < numArrays; i++) typeName.append("[]");
        if (DANGEROUS_TYPES.contains(klass)) {
          warnings.add(
              String.format(
                  "Dangerous type used at signature %s, type %s; method '%s'",
                  sig, typeName, method));
        } else {
          String msg =
              String.format(
                  "Error at signature %s; type %s is not in the supported set of types, method"
                      + " method '%s'",
                  sig, typeName, method);
