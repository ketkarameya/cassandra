...
public class ProtocolVersionTest {
  ...
  public void testDecode() {
    for (ProtocolVersion version : ProtocolVersion.SUPPORTED)
      Assert.assertEquals(
          ...
          ProtocolVersion.decode(
              version.asInt(), DatabaseDescriptor.getNativeTransportAllowOlderProtocols()));

    for (ProtocolVersion version : ProtocolVersion.UNSUPPORTED) { // unsupported old versions
      try {
        Assert.assertEquals(
            version,
            ProtocolVersion.decode(
    ...
    try { // unsupported newer version
      Assert.assertEquals(
          null,
          ProtocolVersion.decode(63, DatabaseDescriptor.getNativeTransportAllowOlderProtocols()));
      Assert.fail("Expected invalid protocol exception");
    } catch (ProtocolException ex) {
      // behavior changed in CASSANDRA-16581
      // When an unknown version is seen which is larger than any we know about, we can't try to
      // fall back to the
      // max version as this can cause issues as the channel protocol doesn't match (which gets
      // rejected), so
  ...
  public void testSupportedVersions() {
    Assert.assertTrue(
        ProtocolVersion.supportedVersions().size() >= 2); // at least one OS and one DSE
