...
public class CQLUserAuditTest {
  ...
  public static void setup() throws Exception {
    OverrideConfigurationLoader.override(
        (config) -> {
          ...
          config.audit_logging_options.logger =
              new ParameterizedClass("DiagnosticEventAuditLogger", null);
        });
    ...
    executeAs(
        Arrays.asList(
            "CREATE ROLE testuser WITH LOGIN = true AND SUPERUSER = false AND PASSWORD = 'foo'",
            "CREATE ROLE testuser_nologin WITH LOGIN = false AND SUPERUSER = false AND PASSWORD ="
                + " 'foo'",
  ...
  public void querySimpleSelect() throws Throwable {
    ArrayList<AuditEvent> events =
        executeAs(
            Arrays.asList("SELECT * FROM testks.table1"),
            "testuser",
            "foo",
  ...
  public void queryInsert() throws Throwable {
    ArrayList<AuditEvent> events =
        executeAs(
            Arrays.asList("INSERT INTO testks.table1 (a, b, c) VALUES ('a', 1, 1)"),
            "testuser",
            "foo",
            AuditLogEntryType.LOGIN_SUCCESS);
    assertEquals(1, events.size());
  ...
  public void queryBatch() throws Throwable {
    String query =
        "BEGIN BATCH "
            ...
            + "APPLY BATCH;";
    ArrayList<AuditEvent> events =
        executeAs(Arrays.asList(query), "testuser", "foo", AuditLogEntryType.LOGIN_SUCCESS);
    assertEquals(1, events.size());
    AuditEvent e = events.get(0);
    Map<String, Serializable> m = e.toMap();
    assertEquals("testuser", m.get("user"));
  ...
  private static ArrayList<AuditEvent> executeAs(
      List<String> queries, String username, String password, AuditLogEntryType expectedAuthType)
      throws Exception {
    ...
    assertEquals(
        InetAddressAndPort.getLoopbackAddress().getAddress(),
    ...
    // drain all remaining login related events, as there's no specification how connections and
    // login attempts
    // should be handled by the driver, so we can't assert a fixed number of login events
    for (AuditEvent e = auditEvents.peek();
        e != null
            && (e.getType() == AuditLogEntryType.LOGIN_ERROR
