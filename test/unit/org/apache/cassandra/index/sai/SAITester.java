...
public abstract class SAITester extends CQLTester {
  ...
  protected static final String CREATE_KEYSPACE_TEMPLATE =
      "CREATE KEYSPACE IF NOT EXISTS %s WITH replication = "
          + "{'class': 'SimpleStrategy', 'replication_factor': '1'}";

  protected static final String CREATE_TABLE_TEMPLATE =
      "CREATE TABLE %s (id1 TEXT PRIMARY KEY, v1 INT, v2 TEXT) WITH compaction = "
          + "{'class' : 'SizeTieredCompactionStrategy', 'enabled' : false }";
  protected static final String CREATE_INDEX_TEMPLATE =
      "CREATE INDEX IF NOT EXISTS ON %%s(%s) USING 'sai'";
  ...
  protected static final Injections.Counter indexBuildCounter =
      Injections.newCounter("IndexBuildCounter")
          .add(
              newInvokePoint()
                  .onClass(CompactionManager.class)
                  .onMethod(
                      "submitIndexBuild", "SecondaryIndexBuilder", "ActiveCompactionsTracker"))
          .build();
  ...
  public static final PrimaryKey.Factory TEST_FACTORY =
      new PrimaryKey.Factory(Murmur3Partitioner.instance, EMPTY_COMPARATOR);
  ...
  @Rule public TestRule testRules = new ResourceLeakDetector();

  @Rule public FailureWatcher failureRule = new FailureWatcher();
  ...
  public void removeAllInjections() {
    ...
    V1OnDiskFormat.SEGMENT_BUILD_MEMORY_LIMITER.setLimitBytes(
        V1OnDiskFormat.SEGMENT_BUILD_MEMORY_LIMIT);
  }

  public static Randomization getRandom() {
    if (random == null) random = new Randomization();
    return random;
  }
  ...
  public enum CorruptionType {
    REMOVED {
      ...
      public void corrupt(File file) throws IOException {
        if (!file.tryDelete()) throw new IOException("Unable to delete file: " + file);
      }
    },
    EMPTY_FILE {
      @Override
  ...
  public static StorageAttachedIndex createMockIndex(ColumnMetadata column) {
    TableMetadata table =
        TableMetadata.builder(column.ksName, column.cfName)
    ...
    options.put(
        IndexTarget.CUSTOM_INDEX_OPTION_NAME, StorageAttachedIndex.class.getCanonicalName());
    options.put("target", column.name.toString());

    IndexMetadata indexMetadata =
        IndexMetadata.fromSchemaMetadata(
            column.name.toString(), IndexMetadata.Kind.CUSTOM, options);
  ...
  public static StorageAttachedIndex createMockIndex(AbstractType<?> cellType) {
    ...
    IndexMetadata indexMetadata =
        IndexMetadata.fromSchemaMetadata("val", IndexMetadata.Kind.CUSTOM, options);
  ...
  public static IndexTermType createIndexTermType(AbstractType<?> cellType) {
    return IndexTermType.create(
        ColumnMetadata.regularColumn("sai", "internal", "val", cellType),
  ...
  public static IndexIdentifier createIndexIdentifier(
      String keyspaceName, String tableName, String indexName) {
  ...
  protected void simulateNodeRestart(boolean wait) {
    ...
    cfs.indexManager
        .listIndexes()
        .forEach(
            index -> ((StorageAttachedIndexGroup) cfs.indexManager.getIndexGroup(index)).reset());
  ...
  protected void corruptIndexComponent(IndexComponent indexComponent, CorruptionType corruptionType)
      throws Exception {
    ColumnFamilyStore cfs = getCurrentColumnFamilyStore();
  ...
  protected void corruptIndexComponent(
      IndexComponent indexComponent, IndexIdentifier indexIdentifier, CorruptionType corruptionType)
      throws Exception {
    ColumnFamilyStore cfs = getCurrentColumnFamilyStore();
    for (SSTableReader sstable : cfs.getLiveSSTables()) {
  ...
  protected boolean verifyChecksum(IndexTermType indexContext, IndexIdentifier indexIdentifier) {
    ...
    for (SSTableReader sstable : cfs.getLiveSSTables()) {
      ...
      if (!indexDescriptor.validatePerSSTableComponents(IndexValidation.CHECKSUM, true, false)
          || !indexDescriptor.validatePerIndexComponents(
              indexContext, indexIdentifier, IndexValidation.CHECKSUM, true, false)) return false;
    }
    return true;
  }
        ...
        () ->
            assertFalse(
                CompactionManager.instance.isCompacting(
                    ColumnFamilyStore.all(), ssTableReader -> true)),
        10,
        TimeUnit.SECONDS);
  }
  ...
  protected void waitForEquals(ObjectName name, long value) {
    waitForAssert(
        () -> assertEquals(value, ((Number) getMetricValue(name)).longValue()),
        10,
        TimeUnit.SECONDS);
  }

  protected ObjectName objectName(
      String name, String keyspace, String table, String index, String type) {
    try {
  ...
  protected void verifyIndexFiles(
      IndexTermType indexTermType, IndexIdentifier indexIdentifier, int indexFiles) {
  ...
  protected void verifyIndexFiles(
    ...
    for (IndexComponent indexComponent :
        Version.LATEST.onDiskFormat().perSSTableIndexComponents(false)) {
      Component component =
          SSTableFormat.Components.Types.CUSTOM.createComponent(
              Version.LATEST.fileNameFormatter().format(indexComponent, null));
      Set<File> tableFiles = componentFiles(indexFiles, component);
      assertEquals(tableFiles.toString(), perSSTableFiles, tableFiles.size());
    }

    for (IndexComponent indexComponent :
        Version.LATEST.onDiskFormat().perColumnIndexComponents(indexTermType)) {
      String componentName =
          Version.LATEST.fileNameFormatter().format(indexComponent, indexIdentifier);
      ...
      else assertEquals(stringIndexFiles.toString(), perColumnFiles, stringIndexFiles.size());
    }
  }
  ...
  protected void verifySSTableIndexes(
      IndexIdentifier indexIdentifier, int sstableContextCount, int sstableIndexCount) {
    ColumnFamilyStore cfs = getCurrentColumnFamilyStore();
    StorageAttachedIndexGroup indexGroup = getCurrentIndexGroup();
    int contextCount = indexGroup.sstableContextManager().size();
    assertEquals(
        "Expected " + sstableContextCount + " SSTableContexts, but got " + contextCount,
        sstableContextCount,
        contextCount);

    StorageAttachedIndex sai =
        (StorageAttachedIndex) cfs.indexManager.getIndexByName(indexIdentifier.indexName);
    Collection<SSTableIndex> sstableIndexes =
        sai == null ? Collections.emptyList() : sai.view().getIndexes();
    assertEquals(
        "Expected " + sstableIndexCount + " SSTableIndexes, but got " + sstableIndexes.toString(),
        sstableIndexCount,
        sstableIndexes.size());
  }
  ...
  protected Set<File> indexFiles() {
    ...
    Set<Component> components =
        cfs.indexManager.listIndexGroups().stream()
    ...
    for (Component component : components) {
      List<File> files =
          cfs.getDirectories().getCFDirectories().stream()
  ...
  public void flush(String keyspace, String table) {
    ...
    if (store != null) store.forceBlockingFlush(ColumnFamilyStore.FlushReason.UNIT_TESTS);
  }
  ...
  protected void truncate(boolean snapshot) {
    ColumnFamilyStore cfs = getCurrentColumnFamilyStore();
    if (snapshot) cfs.truncateBlocking();
    else cfs.truncateBlockingWithoutSnapshot();
  }
  ...
  protected void restoreSnapshot(String snapshot) {
    ...
    Directories.SSTableLister lister =
        cfs.getDirectories().sstableLister(Directories.OnTxnErr.IGNORE).snapshots(snapshot);
    restore(cfs, lister);
  }
  ...
  protected static Injection newFailureOnEntry(
      String name, Class<?> invokeClass, String method, Class<? extends Throwable> exception) {
  ...
  private void verifySSTableComponents(String table, boolean indexComponentsExist)
      throws Exception {
    ColumnFamilyStore cfs =
        Objects.requireNonNull(Schema.instance.getKeyspaceInstance(KEYSPACE))
    ...
    for (SSTable sstable : cfs.getLiveSSTables()) {
      ...
      if (indexComponentsExist)
        assertTrue(
            "Expect all index components are tracked by SSTable, but "
                + diff
                + " are not included.",
            !ndiComponents.isEmpty() && diff.isEmpty());
      else
        assertFalse(
            "Expect no index components, but got " + components,
            components.toString().contains("SAI"));
  ...
  protected static void setBDKPostingsWriterSizing(int minimumPostingsLeaves, int postingsSkip) {
    CassandraRelevantProperties.SAI_MINIMUM_POSTINGS_LEAVES.setString(
        Integer.toString(minimumPostingsLeaves));
  ...
  protected String getSingleTraceStatement(Session session, String query, String contains) {
    ...
    QueryTrace trace =
        session
            .execute(session.prepare(query).bind().enableTracing())
            .getExecutionInfo()
            .getQueryTrace();
    waitForTracingEvents();

    for (QueryTrace.Event event : trace.getEvents()) {
      if (event.getDescription().contains(contains)) return event.getDescription();
    }
    return null;
  }
  ...
  /**
   * Because the tracing executor is single threaded, submitting an empty event should ensure that
   * all tracing events mutations have been applied.
   */
  ...
  public static class Randomization {
    ...
    public void printSeedOnFailure() {
      logger.error(
          "Randomized test failed. To rerun test use -D{}={}", TEST_RANDOM_SEED.getKey(), seed);
    }
    ...
    public BigDecimal nextBigDecimal(
        int minUnscaledValue, int maxUnscaledValue, int minScale, int maxScale) {
      return BigDecimal.valueOf(
          RandomInts.randomIntBetween(random, minUnscaledValue, maxUnscaledValue),
  ...
  public static class FailureWatcher extends TestWatcher {
    ...
    protected void failed(Throwable e, Description description) {
      if (random != null) random.printSeedOnFailure();
    }
  }
  ...
  protected static class TestWithConcurrentVerification {
     ...
     * @param verificationIntervalInMs interval between each verification task, -1 to run
     *     verification task once
     */
    public TestWithConcurrentVerification(
        Runnable verificationTask, Runnable targetTask, int verificationIntervalInMs) {
              ...
              () -> {
                ...
                while (true) {
                  try {
                    ...
                    if (verificationIntervalInMs < 0
                        || taskCompleted.await(verificationIntervalInMs, TimeUnit.MILLISECONDS))
                      break;
                  } catch (Throwable e) {
                    throw Throwables.unchecked(e);
                  }
