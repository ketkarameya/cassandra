...
/**
 * Clone of KeysIndex used in CassandraIndexTest#testCustomIndexWithCFS to verify behaviour of
 * flushing CFS backed CUSTOM indexes
 */
public class CustomCassandraIndex implements Index {
  ...
  public Callable<?> getInitializationTask() {
    ...
    // or if the table is empty we've nothing to do. Otherwise, submit for building via
  ...
  private void setMetadata(IndexMetadata indexDef) {
    ...
    indexCfs =
        ColumnFamilyStore.createColumnFamilyStore(
            baseCfs.keyspace, cfm.name, cfm.ref.get(), baseCfs.getTracker().loadsstables);
  ...
  protected CBuilder buildIndexClusteringPrefix(
      ByteBuffer partitionKey, ClusteringPrefix<?> prefix, CellPath path) {
    CBuilder builder = CBuilder.create(getIndexComparator());
    builder.add(partitionKey);
    return builder;
  }

  protected ByteBuffer getIndexedValue(
  ...
  public IndexEntry decodeEntry(DecoratedKey indexedValue, Row indexEntry) {
    throw new UnsupportedOperationException(
        "KEYS indexes do not use a specialized index entry format");
  }
