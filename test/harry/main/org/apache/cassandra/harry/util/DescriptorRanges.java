...
// TODO: this is not really an interval tree, just two sorted arrays. However, given that
// ExhaustiveChecker has
//       to inflate a partition every time we execute the query, we always know all boundaries at
...
public class DescriptorRanges {
  ...
  public List<DescriptorRange> shadowedBy(long cd, long lts) {
    ...
    for (DescriptorRange range : sortedByMin) {
      if (range.minBound > cd) break;

      if (range.contains(cd, lts)) shadowedBy.add(range);
    }
  ...
  public static class DescriptorRange {
    ...
    public boolean contains(long descriptor) {
      if (minInclusive && maxInclusive) return descriptor >= minBound && descriptor <= maxBound;
  ...
  public String toString() {
    return "Ranges{" + "sortedByMin=" + sortedByMin + '}';
  }
}
