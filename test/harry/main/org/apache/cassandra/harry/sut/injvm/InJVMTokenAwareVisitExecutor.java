...
public class InJVMTokenAwareVisitExecutor extends LoggingVisitor.LoggingVisitorExecutor {
  ...
  public static Function<Run, VisitExecutor> factory(
  ...
  public InJVMTokenAwareVisitExecutor(
      Run run,
      OperationExecutor.RowVisitorFactory rowVisitorFactory,
      SystemUnderTest.ConsistencyLevel cl,
      TokenPlacementModel.ReplicationFactor rf) {
  ...
  protected Object[][] executeWithRetries(long lts, long pd, CompiledStatement statement) {
    if (sut.isShutdown()) throw new IllegalStateException("System under test is shut down");

    int retries = 0;

    Object[] pk = schema.inflatePartitionKey(pd);
    List<TokenPlacementModel.Replica> replicas =
    ...
    while (retries++ < MAX_RETRIES) {
      try {
        ...
        logger.error(
            String.format(
                "Caught message while trying to execute: %s. Scheduled to retry in %s seconds",
                statement, delaySecs),
            t);
    ...
    throw new IllegalStateException(
        String.format("Can not execute statement %s after %d retries", statement, retries));
  }
    ...
    List<TokenPlacementModel.Node> other =
        peerStateToNodes(
            sut.cluster
                .coordinator(1)
                .execute(
                    "select peer, tokens, data_center, rack from system.peers",
    ...
    List<TokenPlacementModel.Node> self =
        peerStateToNodes(
            sut.cluster
                .coordinator(1)
                .execute(
                    "select broadcast_address, tokens, data_center, rack from system.local",
                    ConsistencyLevel.ONE));
  ...
  public static class Configuration
    ...
    public Configuration(
        @JsonProperty("row_visitor")
            org.apache.cassandra.harry.core.Configuration.RowVisitorConfiguration rowVisitor,
    ...
    public Visitor make(Run run) {
      return new GeneratingVisitor(
          run,
          new InJVMTokenAwareVisitExecutor(
              run,
              row_visitor,
              consistency_level,
              new TokenPlacementModel.SimpleReplicationFactor(rf)));
    }
  }
}
