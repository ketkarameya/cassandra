 ...
 *
 * <p>This class coordinates the running of actions that have been planned by an ActionPlan, or are
 * the consequences of actions that have been executed by such a plan. This coordination includes
 * enforcing all {@link OrderOn} criteria, and running DAEMON (recurring scheduled) tasks.
 *
 * <p>Note there is a distinct scheduling mechanism {@link
 * org.apache.cassandra.simulator.Action.Modifier#WITHHOLD} that is coordinated by an Action and its
 * parent, that is used to prevent certain actions from running unless all descendants have executed
 * (with the aim of it ordinarily being invalidated before this happens), and this is not imposed
 * here because it would be more complicated to manage.
 */
public class ActionSchedule implements CloseableIterator<Object>, LongConsumer {
  ...
  public static class ReconcileItem {
    ...
    public String toString() {
      return "run:"
          + performed.toReconcileString()
          + "; next:"
          + result.toReconcileString()
          + "; between ["
          + start
          + ','
          + end
  ...
  final LongSupplier
      schedulerJitter; // we will prioritise all actions scheduled to run within this period of the
                       // current oldest action
  ...
  final PriorityQueue<Action> scheduled =
      new DefaultPriorityQueue<>(Action::compareByDeadline, 128);
  // queue of items that are runnable (i.e. within scheduler jitter of min deadline) sorted by their
  // execution order (i.e. priority)
  final PriorityQueue<Action> runnable = new DefaultPriorityQueue<>(Action::compareByPriority, 128);
  // auxillary queue of items that are runnable so that we may track the time span covered by
  // runnable items we are randomising execution of
  final PriorityQueue<Action> runnableByDeadline =
      new DefaultPriorityQueue<>(Action::compareByDeadline, 128);

  private Mode mode;

  // if running in TIME_LIMITED mode, stop ALL streams (finite or infinite) and daemon tasks once we
  // pass this point
  private long runUntilNanos;

  // if running in STREAM_LIMITED mode, stop infinite streams once we have no more finite streams to
  // process
  ...
  // TODO (cleanup): we can do better than this, probably most straightforwardly by ensuring daemon
  // actions
  ...
  public ActionSchedule(
      SimulatedTime time,
      FutureActionScheduler futureScheduler,
      LongSupplier schedulerJitter,
      RunnableActionScheduler runnableScheduler,
      Work... moreWork) {
    this(
        time,
        futureScheduler,
        runnableScheduler,
        schedulerJitter,
        Arrays.asList(moreWork).iterator());
  }
  ...
  void add(Action action) {
    ...
    if (action.is(STREAM) && !action.is(DAEMON)) ++activeFiniteStreamCount;
  ...
  public boolean hasNext() {
    if (!runnable.isEmpty() || !scheduled.isEmpty()) return true;
    ...
    if (!sequences.isEmpty()) {
      // TODO (feature): detection of which action is blocking progress, and logging of its stack
      // trace only
      ...
      if (Ordered.DEBUG) {
        ...
        logger.error(
            "Simulation failed to make progress. Run with -D{}=true to see the blocked task graph."
                + " Blocked tasks:",
            TEST_SIMULATOR_DEBUG.getKey());
      ...
      actions
          ...
          .sorted(
              Comparator.comparingLong(
                  a ->
                      ((long) ((a.isStarted() ? 1 : 0) + (a.isFinished() ? 2 : 0)) << 32)
                          | a.childCount()))
  ...
  private boolean moreWork() {
    if (!moreWork.hasNext()) return false;
    ...
    if (oldMode != work.mode) {
      if (work.mode == UNLIMITED) {
        ...
        while (!pendingDaemonWave.isEmpty()) advance(pendingDaemonWave.poll());
  ...
  public Object next() {
    ...
    if (!scheduled.isEmpty()) {
      long scheduleUntil =
          Math.min(
              (runnableByDeadline.isEmpty() ? now : runnableByDeadline.peek().deadline())
                  + currentJitter,
              currentJitterUntil);

      while (!scheduled.isEmpty()
          && (runnable.isEmpty() || scheduled.peek().deadline() <= scheduleUntil))
        advance(scheduled.poll());
    }

    Action perform = runnable.poll();
    if (perform == null) throw new NoSuchElementException();
    ...
    add(consequences);
    if (perform.is(STREAM) && !perform.is(DAEMON)) --activeFiniteStreamCount;
  ...
  private void maybeScheduleDaemons(Action perform) {
    if (pendingDaemonWave != null) {
      if (perform.is(DAEMON) && --activeDaemonWaveCount == 0) {
        pendingDaemonWaveCountDown =
            Math.max(128, 16 * (scheduled.size() + pendingDaemonWave.size()));
        ...
        if (activeDaemonWaveCount == 0)
          pendingDaemonWaveCountDown = Math.max(128, 16 * scheduled.size());
      }
    }
  }
  ...
  public void close() {
    if (sequences.isEmpty()
        && scheduled.isEmpty()
        && runnable.isEmpty()
        && (pendingDaemonWave == null || pendingDaemonWave.isEmpty())
        && !moreWork.hasNext()) return;

    List<Sequence> invalidateSequences = new ArrayList<>(this.sequences.values());
    List<Action> invalidateActions =
        new ArrayList<>(
            scheduled.size()
                + runnable.size()
                + (pendingDaemonWave == null ? 0 : pendingDaemonWave.size()));
    invalidateActions.addAll(scheduled);
    invalidateActions.addAll(runnable);
    if (pendingDaemonWave != null) invalidateActions.addAll(pendingDaemonWave);
    while (moreWork.hasNext()) moreWork.next().actors.forEach(invalidateActions::addAll);

    Throwable fail = SafeCollections.safeForEach(invalidateSequences, Sequence::invalidatePending);
    fail =
        Throwables.merge(fail, SafeCollections.safeForEach(invalidateActions, Action::invalidate));
    scheduled.clear();
    runnable.clear();
    runnableByDeadline.clear();
    if (pendingDaemonWave != null) pendingDaemonWave.clear();
    sequences.clear();
  ...
  public void accept(long discontinuity) {
    if (runUntilNanos > 0) runUntilNanos += discontinuity;
  }
}
