...
public class HarryValidatingQuery extends SimulatedAction {
  ...
  protected InterceptedExecution task() {
    return new InterceptedExecution.InterceptedTaskExecution(on) {
      public void run() {
        try {
          QuiescentLocalStateChecker model =
              new QuiescentLocalStateChecker(run, rf) {
                @Override
                ...
                protected void validate(Query query, TokenPlacementModel.ReplicatedRanges ring) {
                  ...
                  List<TokenPlacementModel.Replica> replicas =
                      ring.replicasFor(this.token(query.pd));
                  logger.trace(
                      "Predicted {} as replicas for {}. Ring: {}",
                      new Object[] {replicas, query.pd, ring});
                  ...
                  for (TokenPlacementModel.Replica replica : replicas) {
                    try {
                      validate(
                          () -> {
                            Object[][] objects =
                                this.executeNodeLocal(
                                    compiled.cql(), replica.node(), compiled.bindings());
                            List<ResultSetRow> result = new ArrayList();
                            int length = objects.length;
                            for (int i = 0; i < length; ++i) {
                              Object[] res = objects[i];
                              result.add(
                                  SelectHelper.resultSetToRow(query.schemaSpec, this.clock, res));
                            }
                      ...
                      throwables.add(
                          new AssertionError(
                              String.format(
                                  "Caught an exception while validating %s on %s", query, replica),
                              t));
                    }
                  }
                  ...
                  if (!throwables.isEmpty()) {
                    logger.error(
                        String.format(
                            "Could not validate %d out of %d replicas %s",
