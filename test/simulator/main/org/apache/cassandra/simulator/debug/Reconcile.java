...
public class Reconcile {
  ...
  private static final Pattern STRIP_TRACES =
      Pattern.compile(
          "(Wakeup|Continue|Timeout|Waiting)\\[(((([a-zA-Z]\\.)*[a-zA-Z0-9_$]+\\.[a-zA-Z0-9_<>$]+:[\\-0-9]+;"
              + " )*(([a-zA-Z]\\.)*[a-zA-Z0-9_$]+\\.[a-zA-Z0-9_<>$]+:[\\-0-9]+))( #\\[.*?]#)?)"
              + " ?(by\\[.*?])?]");
  private static final Pattern STRIP_NOW_TRACES = Pattern.compile("( #\\[.*?]#)");
  private static final Pattern NORMALISE_THREAD_RECORDING_IN =
      Pattern.compile("(Thread\\[[^]]+:[0-9]+),?[0-9]+(,node[0-9]+)]");
  static final Pattern NORMALISE_LAMBDA =
      Pattern.compile("((\\$\\$Lambda\\$[0-9]+/[0-9]+)?(@[0-9a-f]+)?)");
  static final Pattern NORMALISE_THREAD =
      Pattern.compile("(Thread\\[[^]]+:[0-9]+),[0-9](,node[0-9]+)(_[0-9]+)?]");
  ...
  public static class TimeReconciler extends AbstractReconciler
      implements SimulatedTime.Listener, Closeable {
    boolean disabled;
  ...
  public static class RandomSourceReconciler extends RandomSource.Abstract
      implements Supplier<RandomSource>, Closeable {
    private static final Logger logger = LoggerFactory.getLogger(RandomSourceReconciler.class);
    private static final AtomicReferenceFieldUpdater<Reconcile.RandomSourceReconciler, Thread>
        lockedUpdater =
            AtomicReferenceFieldUpdater.newUpdater(
                Reconcile.RandomSourceReconciler.class, Thread.class, "locked");
    final DataInputPlus in;
    ...
    public RandomSourceReconciler(
        DataInputPlus in, RandomSource wrapped, boolean inputHasCallSites, RecordOption reconcile) {
      this.in = in;
    ...
    private void enter() {
      if (!lockedUpdater.compareAndSet(this, null, Thread.currentThread())) {
        ...
        disabled = true;
        logger.error(
            "Race within RandomSourceReconciler - means we have a Simulator bug permitting two"
      ...
      try {
        ...
        if (type != 1 || min != min1 || max != max1 || v != v1 || c != count) {
          logger.error(
              String.format(
                  "(%d,%d,%d[%d,%d]) != (%d,%d,%d[%d,%d])",
                  1, count, v, min, max, type, c, v1, min1, max1));
          throw failWithOOM();
        }
        ++count;
      } catch (IOException e) {
      ...
      try {
        ...
        if (type != 2 || min != min1 || max != max1 || v != v1 || c != count) {
          logger.error(
              String.format(
                  "(%d,%d,%d[%d,%d]) != (%d,%d,%d[%d,%d])",
                  2, count, v, min, max, type, c, v1, min1, max1));
          throw failWithOOM();
        }
        ++count;
      } catch (IOException e) {
    ...
    public synchronized void reset(long seed) {
      ...
      try {
        ...
        int c = in.readVInt32();
        long v1 = in.readVInt();
        if (type != 4 || seed != v1 || c != count) throw failWithOOM();
        ++count;
      } catch (IOException e) {
        throw new RuntimeException(e);
      } finally {
    ...
    public synchronized long reset() {
      ...
      try {
        ...
        int c = in.readVInt32();
        long v1 = in.readVInt();
        if (type != 5 || v != v1 || c != count) throw failWithOOM();
        ++count;
      } catch (IOException e) {
        throw new RuntimeException(e);
      } finally {
  ...
  public static void reconcileWith(
      String loadFromDir,
      long seed,
      RecordOption withRng,
      RecordOption withTime,
      ClusterSimulation.Builder<?> builder) {
    ...
    try (BufferedReader eventIn =
            new BufferedReader(
                new InputStreamReader(new GZIPInputStream(eventFile.newInputStream())));
        DataInputStreamPlus rngIn =
            Util.DataInputStreamPlusImpl.wrap(
                rngFile.exists() && withRng != NONE
                    ? new GZIPInputStream(rngFile.newInputStream())
                    : new ByteArrayInputStream(new byte[0]));
        DataInputStreamPlus timeIn =
            Util.DataInputStreamPlusImpl.wrap(
                timeFile.exists() && withTime != NONE
                    ? new GZIPInputStream(timeFile.newInputStream())
                    : new ByteArrayInputStream(new byte[0]))) {
      boolean inputHasWaitSites, inputHasWakeSites, inputHasRngCallSites, inputHasTimeCallSites;
      {
        String modifiers = eventIn.readLine();
        if (!modifiers.startsWith("modifiers:")) throw new IllegalStateException();
      ...
      if (withRng != NONE && !rngFile.exists()) throw new IllegalStateException();
      if (withTime != NONE && !timeFile.exists()) throw new IllegalStateException();

      {
        Set<String> modifiers = new LinkedHashSet<>();
        if (withRng == WITH_CALLSITES) modifiers.add("rngCallSites");
        else if (withRng == VALUE) modifiers.add("rng");
        if (withTime == WITH_CALLSITES) modifiers.add("timeCallSites");
        else if (withTime == VALUE) modifiers.add("time");
        if (builder.capture().waitSites) modifiers.add("WaitSites");
        if (builder.capture().wakeSites) modifiers.add("WakeSites");
      ...
      if (withRng != NONE) {
        builder.random(
            random =
                new RandomSourceReconciler(
                    rngIn, new RandomSource.Default(), inputHasRngCallSites, withRng));
      ...
      Line line = new Line(); // box for heap dump analysis
      try (ClusterSimulation<?> cluster = builder.create(seed);
        ...
        try {
          while (iter.hasNext()) {
            ...
            String input =
                (inputHasWaitSites != builder.capture().waitSites
                        || inputHasWakeSites != builder.capture().wakeSites)
                    ? normaliseRecordingInWithoutWaitOrWakeSites(
                        rawInput,
                        inputHasWaitSites && !builder.capture().waitSites,
                        inputHasWakeSites && !builder.capture().wakeSites)
            ...
            if (!input.equals(output)) failWithHeapDump(line.line, input, output);
          }
  ...
  private static String normaliseRecordingIn(String input) {
    return STRIP_NOW_TRACES
        .matcher(
            NORMALISE_THREAD_RECORDING_IN
                .matcher(NORMALISE_LAMBDA.matcher(input).replaceAll(""))
                .replaceAll("$1$2]"))
        .replaceAll("");
  }

  private static String normaliseRecordingInWithoutWaitOrWakeSites(
      String input, boolean stripWaitSites, boolean stripWakeSites) {
    return STRIP_TRACES
        .matcher(
            NORMALISE_THREAD_RECORDING_IN
                .matcher(NORMALISE_LAMBDA.matcher(input).replaceAll(""))
                .replaceAll("$1$2]"))
        .replaceAll(
            stripWaitSites && stripWakeSites ? "$1[]" : stripWaitSites ? "$1[$9]" : "$1[$3]");
  }
