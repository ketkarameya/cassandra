...
public class MutualTlsCertificateValidityPeriodTest extends TestBaseImpl {
  ...
  public static void setupClass() throws Exception {
    ...
    CA =
        new CertificateBuilder()
            .subject(
                "CN=Apache Cassandra Root CA, OU=Certification Authority, O=Unknown, C=Unknown")
            .alias("fakerootca")
            .isCertificateAuthority(true)
            .buildSelfSigned();

    truststorePath =
        CA.toTempKeyStorePath(
    ...
    CertificateBundle keystore =
        new CertificateBuilder()
            .subject(
                "CN=Apache Cassandra, OU=ssl_test, O=Unknown, L=Unknown, ST=Unknown, C=Unknown")
  ...
  public void testExpiringCertificate() throws Exception {
    ...
    com.datastax.driver.core.Cluster driver =
        JavaDriverUtils.create(
            CLUSTER, null, b -> b.withSSL(getSSLOptions(clientKeystorePath, truststorePath)));

    testWithDriver(
        driver,
        (Session session) -> {
          ResultSet clientView =
              session.execute(new SimpleStatement("SELECT * FROM system_views.clients"));
          ...
          Assertions.assertThat(authenticationMetadata)
              .isNotNull()
              .hasSize(1)
              .containsKey("identity")
              .extractingByKey("identity", as(InstanceOfAssertFactories.STRING))
              .isEqualTo(CLIENT_SPIFFE_IDENTITY);
          ...
          Assertions.assertThat(
                  CLUSTER
                      ...
                      .logs()
                      .grep(
                          "Certificate with identity '" + CLIENT_SPIFFE_IDENTITY + "' will expire")
                      .getResult())
              .isNotEmpty();
          CLUSTER
              .get(1)
              .runOnInstance(
                  () ->
                      Assertions.assertThat(
                              MutualTlsMetrics.instance.clientCertificateExpirationDays.getCount())
                          .isEqualTo(2));
        });
  }
  ...
  public void testFailsWhenCertificateExceedsMaxAllowedValidityPeriod() throws Exception {
    Path clientKeystorePath =
        generateClientCertificate(
            b -> b.notAfter(Instant.now().plus(365, ChronoUnit.DAYS)), tempFolder.getRoot(), CA);
    ...
    try {
      ...
      fail(
          "Should not be able to connect when the certificate exceeds the maximum allowed validity"
              + " period");
    } catch (com.datastax.driver.core.exceptions.NoHostAvailableException exception) {
      Assertions.assertThat(exception)
          .hasMessageContaining(
              "The validity period of the provided certificate (366 days) exceeds the maximum"
                  + " allowed validity period of 30 days");
    }
  }
  ...
  private void testWithDriver(
      com.datastax.driver.core.Cluster providedDriver, Consumer<Session> consumer) {
