...
public class CoordinatorPathTest extends CoordinatorPathTestBase {
  ...
  private static final TokenPlacementModel.SimpleReplicationFactor RF =
    ...
    coordinatorPathTest(
        ...
        (cluster, simulatedCluster) -> {
          Configuration.ConfigurationBuilder configBuilder =
              HarryHelper.defaultConfiguration().setSUT(() -> new InJvmSut(cluster));
          Run run = configBuilder.build().createRun();

          for (int ignored : new int[] {2, 3, 4, 5}) simulatedCluster.createNode().register();

          for (int idx : new int[] {2, 3, 4, 5}) simulatedCluster.node(idx).join();

          VirtualSimulatedCluster prediction = simulatedCluster.asVirtual();
          prediction.createNode();
          prediction.node(6).register();
          prediction.node(6).lazyJoin().prepareJoin().startJoin();
          ...
          while (true) {
            ...
            // At most 2 replicas should respond, so that when the pending node is added, results
            // would be insufficient for recomputed blockFor
            BooleanSupplier shouldRespond =
                atMostResponses(
                    simulatedCluster
                            .state
                            .get()
                            .isWriteTargetFor(token, simulatedCluster.node(1).matcher)
                        ? 1
                        : 2);
            List<WaitingAction<?, ?>> waiting =
                simulatedCluster
                    .filter(
                        (n) ->
                            replicas.stream().map(Replica::node).anyMatch(n.matcher)
                                && n.node.idx() != 1)
                    .map(
                        (nodeToBlockOn) ->
                            nodeToBlockOn.blockOnReplica(
                                (node) -> new MutationAction(node, shouldRespond)))
            ...
            Future<?> writeQuery =
                async(
                    () -> {
                      ...
                      CompiledStatement s =
                          WriteHelper.inflateInsert(
                              run.schemaSpec,
                              pd,
                              cd,
                              run.descriptorSelector.vds(
                                  pd,
                                  cd,
                                  lts,
                                  0,
                      ...
                      cluster
                          .coordinator(1)
                          .execute(s.cql(), ConsistencyLevel.QUORUM, s.bindings());
                      return null;
                    });
            ...
            simulatedCluster.node(6).lazyJoin().prepareJoin().startJoin();
            ...
            try {
              ...
              if (t.getMessage() == null) throw t;
              Assert.assertTrue(
                  "Expected a different error message, but got " + t.getMessage(),
        ...
        (cluster, simulatedCluster) -> {
          ...
          while (true) {
            ...
            if (!simulatedCluster
                    .state
                    .get()
                    .isReadReplicaFor(token(pk), simulatedCluster.node(4).matcher)
                || !simulatedCluster
                    .state
                    .get()
                    .isReadReplicaFor(token(pk), simulatedCluster.node(1).matcher)) continue;
                    ...
                    .map(
                        (nodeToBlockOn) ->
                            nodeToBlockOn.blockOnReplica(
                                (node) ->
                                    new ReadAction(
                                        node, shouldRespond.apply(nodeToBlockOn.node.idx()))))
                    .collect(Collectors.toList());
                ...
                async(
                    () ->
                        cluster
                            .coordinator(1)
                            .execute(
                                "select * from distributed_test_keyspace.tbl where pk = ?",
            ...
            try {
              ...
              if (t.getMessage() == null) throw t;
              Assert.assertTrue(
                  String.format("Got exception: %s", t),
                  t.getMessage().contains("the ring has changed"));
              return;
            }
          }
    ...
    coordinatorPathTest(
        ...
        (cluster, simulatedCluster) -> {
          ...
          simulatedCluster
              ...
              .on(
                  Verb.READ_REQ,
                  new ReadAction(simulatedCluster.node(6)) {
          ...
          simulatedCluster
              ...
              .on(
                  Verb.MUTATION_REQ,
                  new MutationAction(simulatedCluster.node(6)) {
          ...
          for (int i = 0; i < 500; i++) {
            if (simulatedCluster
                .state
                .get()
                .isWriteTargetFor(token(i), simulatedCluster.node(6).matcher)) expectedWrites++;
            cluster
                .coordinator(1)
                .execute(
                    "insert into distributed_test_keyspace.tbl (pk, ck) values (" + i + ", 1)",
                    ConsistencyLevel.ALL);
            cluster
                .coordinator(1)
                .execute(
                    "select * from distributed_test_keyspace.tbl where pk = " + i,
                    ConsistencyLevel.ALL);
          }
