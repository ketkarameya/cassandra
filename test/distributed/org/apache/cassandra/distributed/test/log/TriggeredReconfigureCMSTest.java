...
public class TriggeredReconfigureCMSTest extends FuzzTestBase {
  ...
  public void testRemoveCMSMember() throws IOException, ExecutionException, InterruptedException {
    try (Cluster cluster =
      ...
      IInvokableInstance nodeToRemove =
          cluster.stream()
              .filter(i -> i.config().broadcastAddress().toString().contains(instanceToRemove))
              .findFirst()
              .get();
      String nodeId =
          nodeToRemove.callOnInstance(
          ...
          .runOnInstance(
              () -> {
                try {
                  ...
                  while (FailureDetector.instance.isAlive(
                          InetAddressAndPort.getByName(instanceToRemove.replace("/", "")))
  ...
  public void testReplaceCMSMember() throws IOException {
    try (Cluster cluster =
      ...
      try (WithProperties replacementProps = new WithProperties()) {
        replacementProps.set(
            CassandraRelevantProperties.REPLACE_ADDRESS_FIRST_BOOT,
