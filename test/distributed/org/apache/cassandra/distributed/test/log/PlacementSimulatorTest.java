...
public class PlacementSimulatorTest {
  ...
  public void testMove(long t1, long t2, long t3, long t4, long newToken, ReplicationFactor rf) {
    ...
    placements = steps.advance(placements);
    assertPlacements(
        placements, rf.replicate(finalState).asMap(), rf.replicate(finalState).asMap());
  }
  ...
  public void testBootstrap(
      long t1, long t2, long t3, long t4, long newToken, ReplicationFactor rf) {
    NodeFactory factory = TokenPlacementModel.nodeFactory();
    List<Node> orig =
        Arrays.asList(
            factory.make(1, 1, 1).overrideToken(t1),
            factory.make(2, 1, 1).overrideToken(t2),
            factory.make(3, 1, 1).overrideToken(t3),
            factory.make(4, 1, 1).overrideToken(t4));
    orig.sort(Node::compareTo);
    ...
    assertPlacements(
        placements,
        rf.replicate(afterSplit).asMap(),
        superset(rf.replicate(afterSplit).asMap(), rf.replicate(finalState).asMap()));
  ...
  public void simulate(ReplicationFactor rf) throws Throwable {
    ...
    modelChecker
        .init(
            ...
            new SUTState())
        .step(
            (state, sut) -> state.nodes.size() < rf.total(),
            (state, sut, rng) ->
                new ModelChecker.Pair<>(
                    PlacementSimulator.joinFully(
                        state, factory.make(addressCounter.incrementAndGet(), 1, 1)),
                    sut))
        .step(
            (state, sut) -> state.nodes.size() >= rf.total() && state.stashedStates.size() < 1,
            (state, sut, rng) -> {
                    ...
                    state.withStashed(
                        rng.nextBoolean()
                            ? replace(
                                ...
                                toRemove,
                                factory
                                    .make(addressCounter.incrementAndGet(), 1, 1)
                                    .overrideToken(toRemove.token()))
                            : leave(state, toRemove));
                return new ModelChecker.Pair<>(state, sut);
              } else {
                // schedule bootstrapping an additional node
                return new ModelChecker.Pair<>(
                    state.withStashed(
                        join(state, factory.make(addressCounter.incrementAndGet(), 1, 1))),
                    sut);
              }
            })
        .step(
            (state, sut) -> !state.stashedStates.isEmpty(),
            (state, sut, rng) -> {
        ...
        .exitCondition(
            (state, sut) -> {
              if (addressCounter.get() >= 100 && state.stashedStates.isEmpty()) {
                ...
                assertRanges(state.writePlacements, rf.replicate(state.nodes).asMap());
  ...
  public void revertPartialBootstrap() throws Throwable {
    for (int n : new int[] {2, 3, 5}) {
      ...
      for (int i = 1; i <= 10; i++) nodes.add(factory.make(i, 1, 1));
      nodes.sort(Comparator.comparing(Node::token));

      SimulatedPlacements sim =
          new SimulatedPlacements(
              rf,
              nodes,
  ...
  private void revertPartiallyCompleteOp(
      SimulatedPlacements startingState,
    ...
    for (int i = 1; i <= maxStepsBeforeRevert; i++) startThenRevertOp(startingState, opProvider, i);
  }

  private void startThenRevertOp(
      SimulatedPlacements sim, Supplier<Transformations> opProvider, int stepsToExecute) {
    ...
    for (int i = 0; i < stepsToExecute; i++) sim = steps.advance(sim);
