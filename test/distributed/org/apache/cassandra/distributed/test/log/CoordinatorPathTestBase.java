...
public abstract class CoordinatorPathTestBase extends FuzzTestBase {
  ...
  public void coordinatorPathTest(
      TokenPlacementModel.ReplicationFactor rf,
      TestRunner.ThrowingBiConsumer<Cluster, SimulatedCluster> test)
      throws Throwable {
   ...
   *
   * <p>CMS node is 127.0.0.10 and is simulated.
   */
  public void coordinatorPathTest(
      TokenPlacementModel.ReplicationFactor rf,
      TestRunner.ThrowingBiConsumer<Cluster, SimulatedCluster> test,
    ...
    try (Cluster cluster =
                    ...
                    cfg ->
                        cfg.set(
                            ...
                            new ParameterizedClass(
                                SimpleSeedProvider.class.getName(),
      ...
      if (startCluster) cluster.startup();
  ...
  public void cmsNodeTest(TestRunner.ThrowingBiConsumer<Cluster, SimulatedCluster> test)
      throws Throwable {
    ServerTestUtils.daemonInitialization();
    DatabaseDescriptor.setPartitionerUnsafe(Murmur3Partitioner.instance);
    ...
    try (Cluster cluster =
        ...
        SimulatedCluster simulatedCluster =
            new SimulatedCluster(
                new TokenPlacementModel.SimpleReplicationFactor(3), cluster, tokenSupplier)) {
      // Note that in case of a CMS node teset we first start a cluster, and then initialize a
      // simulated cluster.
      cluster.startup();
  ...
  public Cluster.Builder builder() {
    return Cluster.build()
        .withConfig(
            cfg ->
                cfg.set("request_timeout", String.format("%dms", TimeUnit.MINUTES.toMillis(10))));
  }
  ...
  public static Function<Integer, BooleanSupplier> respondFrom(int... nodeIds) {
    ...
    for (int idx : nodeIds) nodes.add(idx);
  ...
  protected static class RealSimulatedNode extends VirtualSimulatedNode
    ...
    public String toString() {
      return "RealSimulatedNode{" + "id=" + node.idx() + ", token=" + node.token() + '}';
    }
    ...
    public ClusterMetadataTestHelper.JoinProcess lazyJoin() {
      ...
      ClusterMetadataTestHelper.JoinProcess real =
          ClusterMetadataTestHelper.lazyJoin(node.idx(), node.token());
    ...
    /**
     * Serialize and send a single message from this (simulated) node to the real node in the
     * simulated cluster, and block/wait for its response.
     */
    ...
    /**
     * Deliver a single message from this (simulated) node to the real node in the simulated
     * cluster. Communication between the simulated nodes is transparent for the purposes of this
     * test, so it was not implemented.
     */
    public void sendFrom(int from, Message<?> message) {
      try {
        cluster.realCluster.deliverMessage(
            cluster.realCluster.get(1).broadcastAddress(),
            // todo: use addr from the node!
            Instance.serializeMessage(
                ClusterMetadataTestHelper.addr(from), ClusterMetadataTestHelper.addr(1), message));
      } catch (Throwable t) {
        throw new IllegalStateException(
            String.format("Caught an error while delivering %s to %d", message, from), t);
      }
    }

    /**
     * Serialize and deliver a single payload from this (simulated) node to the the real node in the
     * simulated cluster. Communication between the simulated nodes is transparent for the purposes
     * of this test, so it was not implemented.
     */
    public <T> void sendFrom(int from, T payload) {
      sendFrom(from, Message.out(classToVerb.get(payload.getClass()), payload, 0));
    }
    ...
    public boolean test(Message<?> message) {
      ...
      Assert.assertNotNull(
          String.format("Can't find an action that corresponds to verb %s", message.verb()),
          action);
      action.validate(message);
      Message<?> response = action.respondTo(message);
      if (response != null) sendFrom(response.from().addressBytes[3], response);

      return false;
    }

    /** Executes {@param request}, */
    public <IN, OUT> WaitingAction<IN, OUT> blockOnReplica(
        Function<RealSimulatedNode, SimulatedAction<IN, OUT>> factory) {
  ...
  /**
   * A virtual simulated node, that can only be used to make predictions about the cluster state,
   * and doesn't add any handlers to the cluster state.
   */
  protected static class VirtualSimulatedNode implements ClusterMetadataTestHelper.NodeOperations {
    ...
    @Override
    public void join() {
      lazyJoin().prepareJoin().startJoin().midJoin().finishJoin();
    }

    @Override
    public void leave() {
      lazyLeave().prepareLeave().startLeave().midLeave().finishLeave();
    }
  ...
  public static class SimulatedCluster implements Closeable {
    ...
    public SimulatedCluster(
        TokenPlacementModel.ReplicationFactor rf,
        ICluster<IInvokableInstance> realCluster,
      ...
      this.state =
          new RefSimulatedPlacementHolder(
              new SimulatedPlacements(
                  rf,
      ...
      this.nodes.put(
          nodeUnderTestAddr,
          new RealSimulatedNode(this, nodeUnderTest) {
            @Override
            public boolean test(Message<?> message) {
              realCluster
                  .get(1)
                  .receiveMessage(
                      Instance.serializeMessage(message.from(), nodeUnderTestAddr, message));
              return true;
            }
          });
    }
    ...
    public void initWithFakeCms(Node cms, InetAddressAndPort nodeUnderTest) {
      ...
      LocalLog log =
          LocalLog.logSpec().withInitialState(initial).sync().withStorage(logStorage).createLog();

      // Replicator only replicates to the node under test, as there are no other nodes in reality
      Commit.Replicator replicator =
          (result, source) -> {
            realCluster.deliverMessage(
                realCluster.get(1).broadcastAddress(),
                Instance.serializeMessage(
                    cms.addr(),
                    nodeUnderTest,
      ...
      ClusterMetadataService service =
      ...
      service.commit(
          new Register(
              new NodeAddresses(cms.addr()),
              new Location(cms.dc(), cms.rack()),
              NodeVersion.CURRENT));

      IVerbHandler<Commit> commitRequestHandler =
          Commit.handlerForTests(
              processor,
              replicator,
              (msg, to) ->
              ...
              executor.submit(
                  () -> {
                    try {
                      ...
                      if (target.equals(cms.addr())) {
                        switch (message.verb()) {
                          case TCM_DISCOVER_REQ:
                            Message<?> rsp =
                                message.responseWith(
                                    new Discovery.DiscoveredNodes(
                                        Collections.singleton(cms.addr()),
                                        Discovery.DiscoveredNodes.Kind.CMS_ONLY));
                            realCluster.deliverMessage(
                                message.from(),
                          ...
                          case TCM_FETCH_CMS_LOG_REQ:
                            {
                              ...
                              realCluster.deliverMessage(
                                  message.from(),
                                  Instance.serializeMessage(
                                      cms.addr(), message.from(), message.responseWith(logState)));
                              return;
                            }
                          default:
                            logger.error(
                                "Mocked CMS node has received message with {} verb: {}",
                                msg.verb(),
                                msg);
                        }
                      } else {
                      ...
                      logger.error(
                          String.format(
                              "Caught an exception while trying to deliver %s to %s", msg, target),
                          t);
                    }
                  }));
    }
    ...
    public void init() {
      ...
      LocalLog log = LocalLog.logSpec().sync().createLog();
          ...
          new ClusterMetadataService(
              ...
              new Processor() {
                public Commit.Result commit(
                  ...
                  if (lastKnown == null) lastKnown = log.waitForHighestConsecutive().epoch;
                  Commit.Result result =
                      driver.requestResponse(new Commit(entryId, event, lastKnown));
                  if (result.isSuccess()) {
      ...
      driver.on(
          ...
          new SimulatedAction<LogState, NoPayload>() {
            public Verb verb() {
            ...
            public Message<NoPayload> respondTo(Message<LogState> request) {
              for (Entry entry : request.payload.entries) log.append(entry);
              log.waitForHighestConsecutive();
              return null;
            }
          });
      ...
      // to have a more efficient way of blocking, namely one with a queue and continuation or a
      // singal,
  ...
  public static class VirtualSimulatedCluster {
    ...
    public VirtualSimulatedCluster(SimulatedPlacementHolder state, NodeFactory factory) {
      ...
      for (Node node : state.get().nodes) this.nodes.add(new VirtualSimulatedNode(state, node));
    }
  ...
  public abstract static class AbstractSimulatedAction<IN, OUT>
      implements SimulatedAction<IN, OUT> {
  ...
  public static class GossipSynAction
      extends AbstractSimulatedAction<GossipDigestSyn, GossipDigestAck> {
    ...
    public Message<GossipDigestAck> respondTo(Message<GossipDigestSyn> request) {
      return Message.out(
          GOSSIP_DIGEST_ACK, new GossipDigestAck(Collections.emptyList(), Collections.emptyMap()));
    }
  }
  ...
  public static class ReadAction extends AbstractSimulatedAction<ReadCommand, ReadResponse> {
    ...
    public void validate(Message<ReadCommand> request) {
      ...
      Murmur3Partitioner.LongToken requestToken =
          (Murmur3Partitioner.LongToken) sprc.partitionKey().getToken();
      assert node.cluster.state.get().isReadReplicaFor(requestToken.token, node.matcher)
          : String.format(
              "Node %s is not a read replica for %s. Replicas: %s",
              node,
              requestToken.token,
              node.cluster.state.get().readReplicasFor(requestToken.token));
    }
    ...
    public Message<ReadResponse> respondTo(Message<ReadCommand> request) {
      if (shouldRespond.getAsBoolean()) {
        ReadCommand command = request.payload;
        return Message.remoteResponseForTests(
            request.id(),
            node.node.addr(),
            request.verb().responseVerb,
            ReadResponse.createDataResponse(
                EmptyIterators.unfilteredPartition(command.metadata()), command));
      }
      return null;
    }
  }
  ...
  public static class RangeReadAction extends AbstractSimulatedAction<ReadCommand, ReadResponse> {
    ...
    public void validate(Message<ReadCommand> request) {
      ...
      Murmur3Partitioner.LongToken leftToken =
          (Murmur3Partitioner.LongToken) prrc.dataRange().keyRange().left.getToken();
      Murmur3Partitioner.LongToken rightToken =
          (Murmur3Partitioner.LongToken) prrc.dataRange().keyRange().right.getToken();
      assert node.cluster
          .state
          .get()
          .isReadReplicaFor(leftToken.token, rightToken.token, node.matcher);
    }

    @Override
    public Message<ReadResponse> respondTo(Message<ReadCommand> request) {
      ReadCommand command = request.payload;
      return request.responseWith(
          ReadResponse.createDataResponse(
              EmptyIterators.unfilteredPartition(command.metadata()), command));
    }
  }
  ...
  public static class MutationAction extends AbstractSimulatedAction<Mutation, NoPayload> {
    ...
    public void validate(Message<Mutation> request) {
      ...
      Murmur3Partitioner.LongToken requestToken =
          (Murmur3Partitioner.LongToken) command.key().getToken();
      assert node.cluster.state.get().isWriteTargetFor(requestToken.token, node.matcher)
          : String.format(
              "Node %s is not a write target for %s. Write placements: %s",
              node.node.idx(),
              requestToken,
              node.cluster.state.get().writePlacementsFor(requestToken.token));
    }

    public Message<NoPayload> respondTo(Message<Mutation> request) {
      if (shouldRespond.getAsBoolean())
        return Message.remoteResponseForTests(
            request.id(), node.node.addr(), request.verb().responseVerb, NoPayload.noPayload);
      return null;
    }
  }
  ...
  public static long token(Object... pk) {
    return ((Murmur3Partitioner.LongToken)
            DatabaseDescriptor.getPartitioner().decorateKey(toBytes(pk)).getToken())
        .token;
  }

  public static ByteBuffer toBytes(Object... pk) {
    assert pk.length > 0;
    if (pk.length == 1) return ByteUtils.objectToBytes(pk[0]);
