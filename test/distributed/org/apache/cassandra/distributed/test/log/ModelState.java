...
public class ModelState {
  ...
  public static ModelState empty(
      TokenPlacementModel.NodeFactory nodeFactory, int maxClusterSize, int maxConcurrency) {
  ...
  public static Map<String, List<TokenPlacementModel.Node>> groupByDc(
      List<TokenPlacementModel.Node> nodes) {
    ...
    for (TokenPlacementModel.Node node : nodes) {
      grouped.computeIfAbsent(node.dc(), (k) -> new ArrayList<>()).add(node);
    }
    return grouped;
  }
  ...
  private boolean canRemove(TokenPlacementModel.ReplicationFactor rfs) {
    ...
    for (Map.Entry<String, DCReplicas> e : rfs.asMap().entrySet()) {
      ...
      Set<TokenPlacementModel.Node> nodesInDc =
          nodes == null ? new HashSet<>() : new HashSet<>(nodes);
      for (SimulatedOperation op : inFlightOperations) nodesInDc.removeAll(Arrays.asList(op.nodes));
      if (nodesInDc.size() > rf) return true;
    }
    return false;
  }
  ...
  public static class Transformer {
    ...
    public Transformer withMoved(
        TokenPlacementModel.Node movingNode, TokenPlacementModel.Node movedTo) {
      ...
      for (TokenPlacementModel.Node n : tmp) {
        if (n.idx() == movingNode.idx()) currentNodes.add(movedTo);
        else currentNodes.add(n);
      }
      finished[3]++;
    ...
    public Transformer withReplaced(
        TokenPlacementModel.Node oldNode, TokenPlacementModel.Node newNode) {
    ...
    public Transformer updateSimulation(
        PlacementSimulator.SimulatedPlacements simulatedPlacements) {
      this.simulatedPlacements = simulatedPlacements;
      return this;
    }
