...
public class BootstrapTest extends TestBaseImpl {
  ...
  /**
   * Confirm that a normal, non-resumed bootstrap without the reset_bootstrap_progress param
   * specified works without issue.
   *
   * @throws Throwable
   */
  @Test
  ...
  private void bootstrapTest() throws Throwable {
    ...
    try (Cluster cluster =
        builder()
            ...
            .withNodeIdTopology(
                NetworkTopology.singleDcNetworkTopology(expandedNodeCount, "dc0", "rack0"))
      ...
      for (Map.Entry<Integer, Long> e : count(cluster).entrySet())
        assertEquals(
            "Node " + e.getKey() + " has incorrect row state", 100L, e.getValue().longValue());
    }
  }
  ...
  public void readWriteDuringBootstrapTest() throws Throwable {
    ...
    try (Cluster cluster =
      ...
      IInvokableInstance newInstance = cluster.bootstrap(config);
      withProperty(TEST_WRITE_SURVEY, true, () -> newInstance.startup(cluster));
      populate(cluster, 0, 100);
  ...
  public void bootstrapJMXStatus() throws Throwable {
    ...
    try (Cluster cluster =
      ...
      assertEquals(
          "Bootstrap streaming success",
          getMetricGaugeValue(joiningInstance, "BootstrapLastSeenStatus", String.class));
      assertEquals(
          "", getMetricGaugeValue(joiningInstance, "BootstrapLastSeenError", String.class));
    }
  }

  public static <T> T getMetricGaugeValue(
      IInvokableInstance instance, String metricName, Class<T> gaugeReturnType) {
  ...
  public static Object getMetricAttribute(
      IInvokableInstance instance, String metricName, String attributeName) {
    if (instance.isShutdown()) throw new IllegalStateException("Instance is shutdown");
    ...
    try (JMXConnector jmxc = JMXUtil.getJmxConnector(instance.config())) {
      ...
      for (MBeanAttributeInfo a : info.getAttributes()) {
        if (a.getName().equals(attributeName)) return mbsc.getAttribute(metric, a.getName());
      }
  ...
  public static void populate(
      ICluster cluster, int from, int to, int coord, int rf, ConsistencyLevel cl) {
    ...
    for (int i = from; i < to; i++) {
      cluster
          .coordinator(coord)
          .executeWithRetries(
              "INSERT INTO " + KEYSPACE + ".tbl (pk, ck, v) VALUES (?, ?, ?)", cl, i, i, i);
    }
  }

  public static void populateExistingTable(
      ICluster cluster, int from, int to, int coord, ConsistencyLevel cl) {
    for (int i = from; i < to; i++) {
      cluster
          .coordinator(coord)
          .executeWithRetries(
        ...
        .collect(
            Collectors.toMap(
                ...
                nodeId ->
                    (Long)
                        cluster.get(nodeId)
                            .executeInternal("SELECT count(*) FROM " + KEYSPACE + ".tbl")[0][0]));
  }
  ...
  public static class BB {
    ...
    public static void markViewsAsBuilt(@SuperCall Callable<Void> zuper) {
      ...
      if (invocations == 1) throw new RuntimeException("simulated error in bootstrapFinished");
