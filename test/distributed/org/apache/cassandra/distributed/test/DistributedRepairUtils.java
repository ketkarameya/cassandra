...
public final class DistributedRepairUtils {
  ...
  public static NodeToolResult repair(
  ...
  public static <I extends IInvokableInstance, C extends AbstractCluster<I>>
      long getRepairExceptions(C cluster) {
    return getRepairExceptions(cluster, DEFAULT_COORDINATOR);
  }

  public static <I extends IInvokableInstance, C extends AbstractCluster<I>>
      long getRepairExceptions(C cluster, int node) {
    return cluster.get(node).callOnInstance(() -> StorageMetrics.repairExceptions.getCount());
  }

  public static QueryResult queryParentRepairHistory(
      ICluster<IInvokableInstance> cluster, String ks, String table) {
    return queryParentRepairHistory(cluster, DEFAULT_COORDINATOR, ks, table);
  }

  public static QueryResult queryParentRepairHistory(
      ICluster<IInvokableInstance> cluster, int coordinator, String ks, String table) {
    // This is kinda brittle since the caller never gets the ID and can't ask for the ID; it needs
    // to infer the id
    // this logic makes the assumption the ks/table pairs are unique (should be or else create
    // should fail) so any
  ...
  public static void assertParentRepairNotExist(
      ICluster<IInvokableInstance> cluster, String ks, String table) {
  ...
  public static void assertParentRepairNotExist(
      ICluster<IInvokableInstance> cluster, int coordinator, String ks) {
    QueryResult rs = queryParentRepairHistory(cluster, coordinator, ks, null);
    Assert.assertFalse("No repairs should be found but at least one found", rs.hasNext());
  }

  public static void assertParentRepairSuccess(
      ICluster<IInvokableInstance> cluster, String ks, String table) {
  ...
  public static void assertParentRepairSuccess(
      ...
      String ks,
      String table,
      Consumer<Row> moreSuccessCriteria) {
    Assert.assertNotNull("Invalid null value for moreSuccessCriteria", moreSuccessCriteria);
    QueryResult rs = queryParentRepairHistory(cluster, coordinator, ks, table);
    validateExistingParentRepair(
        rs,
        row -> {
          // check completed
          Assert.assertNotNull(
              "finished_at not found, the repair is not complete?",
              row.getTimestamp("finished_at"));
  ...
  public static void assertParentRepairFailedWithMessageContains(
      ICluster<IInvokableInstance> cluster, String ks, String table, String message) {
  ...
  public static void assertNoSSTableLeak(
      ICluster<IInvokableInstance> cluster, String ks, String table) {
    cluster.forEach(
        i -> {
          ...
          i.runOnInstance(
              () -> {
                ...
                for (SSTableReader sstable : cfs.getTracker().getView().liveSSTables()) {
                  ...
                  // repair maybe async, so some participates may still think the repair is active,
                  // which means the sstable SHOULD link to it
                  if (session != null && !session.isCompleted()) continue;
                  // The session is complete, yet the sstable is not updated... is this still
                  // pending in compaction?
                  if (cfs.getCompactionStrategyManager()
                      .hasPendingRepairSSTable(pendingRepair, sstable)) continue;
                  // compaction does not know about the pending repair... race condition since this
                  // check started?
                  if (sstable.getSSTableMetadata().pendingRepair == null)
                    continue; // yep, race condition... ignore
                  throw new AssertionError(
                      String.format(
                          "%s had leak detected on sstable %s", name, sstable.descriptor));
                }
              });
        });
  }
