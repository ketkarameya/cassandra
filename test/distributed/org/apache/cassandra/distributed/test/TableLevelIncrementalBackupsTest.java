...
public class TableLevelIncrementalBackupsTest extends TestBaseImpl {
  ...
  public void testIncrementalBackupEnabledCreateTable() throws Exception {
    try (Cluster c = getCluster(true)) {
      c.schemaChange(
          withKeyspace(
              "CREATE TABLE %s.test_table (a text primary key, b int) WITH incremental_backups ="
                  + " true"));
      disableCompaction(c, KEYSPACE, "test_table");
      c.coordinator(1)
          .execute(withKeyspace("INSERT INTO %s.test_table (a, b) VALUES ('a', 1)"), ALL);
      flush(c, KEYSPACE);
      ...
      c.schemaChange(
          withKeyspace(
              "CREATE TABLE %s.test_table2 (a text primary key, b int) WITH incremental_backups ="
                  + " false"));
      disableCompaction(c, KEYSPACE, "test_table2");
      c.coordinator(1)
          .execute(withKeyspace("INSERT INTO %s.test_table2 (a, b) VALUES ('a', 1)"), ALL);
      flush(c, KEYSPACE);
  ...
  private Cluster getCluster(boolean incrementalBackups) throws IOException {
    return init(
        build(2)
            .withDataDirCount(1)
            .withConfig(c -> c.with(Feature.GOSSIP).set("incremental_backups", incrementalBackups))
            .start());
  }
  ...
  private static void assertBackupSSTablesCount(
      Cluster cluster, int expectedTablesCount, boolean enable, String ks, String... tableNames) {
    for (int i = 1; i < cluster.size() + 1; i++) {
      cluster
          .get(i)
          .runOnInstance(
              rethrow(
                  () ->
                      Arrays.stream(tableNames)
                          .forEach(
                              tableName ->
                                  assertTableMetaIncrementalBackupEnable(ks, tableName, enable))));
      cluster
          .get(i)
          .runOnInstance(
              rethrow(
                  () ->
                      Arrays.stream(tableNames)
                          .forEach(
                              tableName ->
                                  assertSSTablesCount(
                                      getBackups(ks, tableName), tableName, expectedTablesCount))));
    }
  }

  private static void assertSSTablesCount(
      Set<Descriptor> descs, String tableName, int expectedTablesCount) {
    Predicate<Descriptor> descriptorPredicate =
        descriptor -> {
          if (DatabaseDescriptor.isUUIDSSTableIdentifiersEnabled())
            return descriptor.id instanceof UUIDBasedSSTableId;
          else return descriptor.id instanceof SequenceBasedSSTableId;
        };
    ...
    assertThat(seqSSTables)
        ...
        .hasSize(expectedTablesCount);
  }

  private static void assertTableMetaIncrementalBackupEnable(
      String ks, String tableName, boolean enable) {
    ColumnFamilyStore columnFamilyStore = ColumnFamilyStore.getIfExists(ks, tableName);
    if (enable) assertTrue(columnFamilyStore.isTableIncrementalBackupsEnabled());
    else assertFalse(columnFamilyStore.isTableIncrementalBackupsEnabled());
  }
}
