...
public class IndexStreamingTest extends TestBaseImpl {
  ...
  // streaming sends events every 65k, so need to make sure that the files are larger than this to
  // hit
  ...
  public void testIndexComponentStreaming() throws IOException {
    try (Cluster cluster =
      ...
      cluster.schemaChange(
          withKeyspace(
              isWide
                  ? "CREATE TABLE %s.test (pk int, ck int , literal text, numeric int, b blob,"
                        + " PRIMARY KEY(pk, ck)) WITH compression = { 'enabled' : false };"
                  : "CREATE TABLE %s.test (pk int PRIMARY KEY , literal text, numeric int, b blob)"
                        + " WITH compression = { 'enabled' : false };"));

      int numSSTableComponents =
          isWide
              ? V1OnDiskFormat.WIDE_PER_SSTABLE_COMPONENTS.size()
              : V1OnDiskFormat.SKINNY_PER_SSTABLE_COMPONENTS.size();
      int numIndexComponents =
          isLiteral
              ? V1OnDiskFormat.LITERAL_COMPONENTS.size()
              : V1OnDiskFormat.NUMERIC_COMPONENTS.size();
      int numComponents =
          sstableStreamingComponentsCount() + numSSTableComponents + numIndexComponents + 1;
      ...
      cluster.stream()
          .forEach(i -> i.nodetoolResult("disableautocompaction", KEYSPACE).asserts().success());
      ...
      for (int i = 0; i < sstableCount; i++) {
        if (isWide) {
          String insertTemplate =
              "INSERT INTO %s.test(pk, ck, "
                  + (isLiteral ? "literal" : "numeric")
                  + ", b) VALUES (?, ?, ?, ?)";
          first.executeInternal(
              withKeyspace(insertTemplate), i, i, isLiteral ? "v" + i : Integer.valueOf(i), BLOB);
        } else {
          String insertTemplate =
              "INSERT INTO %s.test(pk, "
                  + (isLiteral ? "literal" : "numeric")
                  + ", b) VALUES (?, ?, ?)";
          first.executeInternal(
              withKeyspace(insertTemplate), i, isLiteral ? "v" + i : Integer.valueOf(i), BLOB);
        }
      ...
      SimpleQueryResult qr =
          first.executeInternalWithResult("SELECT * FROM system_views.streaming");
      ...
      QueryResultUtil.assertThat(row)
          ...
          .isEqualTo("success_message", null)
          .isEqualTo("failure_cause", null)
          ...
          .columnsEqualTo(
              "files_received", "files_to_receive", "bytes_received", "bytes_to_receive");
      ...
      QueryResultUtil.assertThat(qr.next())
          ...
          .isEqualTo("success_message", null)
          .isEqualTo("failure_cause", null)
          .columnsEqualTo("files_to_receive", "files_received")
          .isEqualTo("files_received", expectedFiles)
          .columnsEqualTo("bytes_to_receive", "bytes_received")
          .isEqualTo("bytes_received", totalBytes)
          .columnsEqualTo("files_sent", "files_to_send", "bytes_sent", "bytes_to_send")
          .isEqualTo("files_sent", 0L);

      // did we trigger slow event log?
      cluster.forEach(
          i ->
              Assertions.assertThat(
                      i.logs().grep("Handling streaming events took longer than").getResult())
      ...
      for (int i = 0; i < sstableCount; i++) {
        Object[][] rs =
            isLiteral
                ? second.executeInternal(
                    withKeyspace("select pk from %s.test where literal = ?"), "v" + i)
                : second.executeInternal(
                    withKeyspace("select pk from %s.test where numeric = ?"), i);
