...
public class RowUtil {
  public static SimpleQueryResult toQueryResult(ResultMessage res) {
    if (res != null && res.kind == ResultMessage.Kind.ROWS) {
      ...
      return new SimpleQueryResult(
          names, results, warnings == null ? Collections.emptyList() : warnings);
    } else {
      if (res != null) {
        List<String> warnings = res.getWarnings();
        return new SimpleQueryResult(
            new String[0], null, warnings == null ? Collections.emptyList() : warnings);
      } else return QueryResults.empty();
    }
  }
  ...
  public static Object[][] toObjects(List<ColumnSpecification> specs, List<List<ByteBuffer>> rows) {
    ...
    for (int i = 0; i < rows.size(); i++) {
      ...
      for (int j = 0; j < specs.size(); j++) {
        ...
        if (bb != null) result[i][j] = specs.get(j).type.getSerializer().deserialize(bb);
      }
    }
    return result;
  }
  ...
  public static Iterator<Object[]> toIter(
      List<ColumnSpecification> columnSpecs, Iterator<UntypedResultSet.Row> rs) {
  ...
  private static Iterator<Object[]> toIterInternal(
      List<ColumnSpecification> columnSpecs, List<List<ByteBuffer>> rs) {
    return Iterators.transform(
        rs.iterator(),
        (row) -> {
          ...
          for (int i = 0; i < columnSpecs.size(); i++) {
            ...
            if (bb != null) objectRow[i] = columnSpec.type.getSerializer().deserialize(bb);
          }
          return objectRow;
        });
  }
