...
public class IsolatedJmx {
  ...
  public static final String UNKNOWN_JMX_CONNECTION_ERROR =
      "Could not connect to JMX due to an unknown error";
  ...
  public void startJmx() {
    try {
      ...
      registry =
          new JMXServerUtils.JmxRegistry(
              jmxPort, clientSocketFactory, serverSocketFactory, "jmxrmi");
      ...
      // potentially attempt a full GC every `sun.rmi.dgc.server.gcInterval` millis (default is
      // 3600000ms)
      ...
      // Set the port used to create subsequent connections to exported objects over RMI. This
      // simplifies
      // configuration in firewalled environments, but it can't be used in conjuction with SSL
      // sockets.
      // See: CASSANDRA-7087
      int rmiPort = config.jmxPort();

      // We create the underlying RMIJRMPServerImpl so that we can manually bind it to the registry,
      // rather then specifying a binding address in the JMXServiceURL and letting it be done
      // automatically
      // when the server is started. The reason for this is that if the registry is configured with
      // SSL
      // sockets, the JMXConnectorServer acts as its client during the binding which means it needs
      // to
      // have a truststore configured which contains the registry's certificate. Manually binding
      // removes
      // this problem.
      // See CASSANDRA-12109.
      jmxRmiServer = new RMIJRMPServerImpl(rmiPort, clientSocketFactory, serverSocketFactory, env);
      JMXServiceURL serviceURL = new JMXServiceURL("rmi", hostname, rmiPort);
      jmxConnectorServer =
          new RMIConnectorServer(serviceURL, env, jmxRmiServer, wrapper.getMBeanServer());
  ...
  public void stopJmx() {
    if (!config.has(JMX)) return;
    // First, swap the mbean wrapper back to a NoOp wrapper
    // This prevents later attempts to unregister mbeans from failing in Cassandra code, as we're
    // going to
    // unregister all of them here
    ((MBeanWrapper.DelegatingMbeanWrapper) MBeanWrapper.instance)
        .setDelegate(new MBeanWrapper.NoOpMBeanWrapper());
    try {
      wrapper.close();
    } catch (Throwable e) {
    ...
    Uninterruptibles.sleepUninterruptibly(
        2 * RMI_KEEPALIVE_TIME,
        TimeUnit
            .MILLISECONDS); // Double the keep-alive time to give Distributed GC some time to clean
                            // up
  }
}
