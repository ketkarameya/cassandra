...
public class DirectStreamingConnectionFactory {
    ...
    class DirectStreamingChannel implements StreamingChannel {
      class Out extends BufferedDataOutputStreamPlus implements StreamingDataOutputPlus {
        ...
        protected void doFlush(int count) throws IOException {
          ...
          try {
            synchronized (out) {
              while (out.pendingBytes > 0
                  && count + out.pendingBytes > sendBufferSize
                  && !out.isClosed) out.wait();

              if (out.isClosed) throw new ClosedChannelException();
      ...
      class In extends RebufferingInputStream implements StreamingDataInputPlus {
        ...
        protected void reBuffer() throws IOException {
          try {
            synchronized (in) {
              while (in.pendingBytes == 0 && !in.isClosed) in.wait();

              if (in.pendingBytes == 0) throw new ClosedChannelException();

              byte[] bytes = in.pending.poll();
              if (bytes == null) throw new IllegalStateException();
  ...
  public class Factory implements StreamingChannel.Factory {
    ...
    public StreamingChannel create(
        InetSocketAddress to, int messagingVersion, StreamingChannel.Kind kind) {
      long sendBufferSize =
          new OutboundConnectionSettings(getByAddress(to)).socketSendBufferSizeInBytes();
      if (sendBufferSize <= 0) sendBufferSize = 1 << 14;

      DirectConnection connection =
          new DirectConnection(messagingVersion, sendBufferSize, from, to);
      IInvokableInstance instance = cluster.get(to);
      instance.unsafeAcceptOnThisThread(
          (channel, version) ->
              executorFactory()
                  .startThread(
                      channel.description(), new StreamDeserializingTask(null, channel, version)),
          connection.get(from),
          messagingVersion);
  ...
  private DirectStreamingConnectionFactory(ICluster<IInvokableInstance> cluster) {
    ...
    // potentially leading to resource leaks on the hosts (particularly in validateHeader which runs
    // on the host threads)
    this.protocolVersion = current_version;
  }

  public static Function<IInvokableInstance, StreamingChannel.Factory> create(
      ICluster<IInvokableInstance> cluster) {
    return cluster
        .get(1)
        .unsafeApplyOnThisThread(c -> new DirectStreamingConnectionFactory(c)::get, cluster);
  }
