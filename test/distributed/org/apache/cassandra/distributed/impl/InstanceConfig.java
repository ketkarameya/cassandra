...
public class InstanceConfig implements IInstanceConfig {
  ...
  private InstanceConfig(
    ...
    if (CassandraRelevantProperties.DTEST_JVM_DTESTS_USE_LATEST.getBoolean()) {
      ...
      this.set(
              "memtable",
              Map.of("configurations", Map.of("default", Map.of("class_name", "TrieMemtable"))))
          ...
          .set("sstable", Map.of("selected_format", "bti"))
  ...
  public InstanceConfig with(Feature... flags) {
    for (Feature flag : flags) featureFlags.add(flag);
    return this;
  }
  ...
  private Map<String, Object> getParams(String fieldName) {
    ...
    if (fieldName.startsWith("dtest")) map = dtestParams;
    return map;
  }
  ...
  public void validate() {
    // Previous logic would validate vnode was not used, but with vnode support added that
    // validation isn't needed.
    // Rather than attempting validating the configs here, its best to leave that to the instance;
    // this method
  ...
  public static InstanceConfig generate(
      int nodeNum,
  ...
  private static String[] datadirs(int datadirCount, Path root, int nodeNum) {
    ...
    for (int i = 0; i < datadirs.length; i++) datadirs[i] = String.format(datadirFormat, i);
    return datadirs;
  }

  public InstanceConfig forVersion(Semver version) {
    // Versions before 4.0 need to set 'seed_provider' without specifying the port
    if (UpgradeTestBase.v40.compareTo(version) < 0) return this;
    else
