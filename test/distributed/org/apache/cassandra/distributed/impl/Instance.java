...
/**
 * This class is instantiated on the relevant classloader, so its methods invoke the correct target
...
public class Instance extends IsolatedExecutor implements IInvokableInstance {
  ...
  Instance(
    ...
    Object clusterId =
        Objects.requireNonNull(
            config.get(Constants.KEY_DTEST_API_CLUSTER_ID), "cluster_id is not defined");
    ...
    FBUtilities.setBroadcastInetAddressAndPort(
        InetAddressAndPort.getByAddressOverrideDefaults(
            config.broadcastAddress().getAddress(), config.broadcastAddress().getPort()));

    // Set the config at instance creation, possibly before startup() has run on all other
    // instances.
  ...
  public LogAction logs() {
    ...
    File f =
        new File(
            ...
            String.format(
                "build/test/logs/%s/%s/%s/%s/system.log", tag, suite, clusterId, instanceId));
    // when creating a cluster globally in a test class we get the logs without the suite, try
    // finding those logs:
    if (!f.exists())
      f =
          new File(
              FileSystems.getDefault(),
              String.format("build/test/logs/%s/%s/%s/system.log", tag, clusterId, instanceId));
    if (!f.exists())
      throw new AssertionError(
          "Unable to locate system.log under "
              + f.absolutePath()
              + "; make sure ICluster.setup() is called or extend TestBaseImpl and do not define a"
              + " static beforeClass function with @BeforeClass");
  ...
  public static SimpleQueryResult unsafeExecuteInternalWithResult(String query, Object... args) {
    ...
    try {
      ...
      ResultMessage result =
          prepared.statement.executeLocally(
              QueryProcessor.internalQueryState(),
              QueryProcessor.makeInternalOptions(prepared.statement, args));
      CoordinatorWarnings.done();

      if (result != null) result.setWarnings(ClientWarn.instance.getWarnings());
  ...
  public UUID schemaVersion() {
    // we do not use method reference syntax here, because we need to sync on the node-local schema
    // instance
            ...
            (message, to) -> {
              if (!internodeMessagingStarted) {
                inInstancelogger.debug(
                    "Dropping outbound message {} to {} as internode messaging has not been started"
                        + " yet",
                    message,
                    to);
                return false;
    ...
    MessagingService.instance()
        ...
        .add(
            message -> {
              ...
              IMessage serialized =
                  serializeMessage(
                      message.from(), toCassandraInetAddressAndPort(broadcastAddress()), message);
              IInstance from = cluster.get(serialized.from());
              if (from == null) return false;
              int fromNum = from.config().num();
              int toNum =
                  config
                      .num(); // since this instance is reciving the message, to will always be this
                              // instance
    ...
    MessagingService.instance()
        ...
        .add(
            (message, to) -> {
              ...
              int fromNum =
                  config
                      .num(); // since this instance is sending the message, from will always be
                              // this instance
  ...
  public static IMessage serializeMessage(
      InetAddressAndPort from, InetAddressAndPort to, Message<?> messageOut) {
    int fromVersion = MessagingService.instance().versions.get(from);
    int toVersion = MessagingService.instance().versions.get(to);

    // If we're re-serializing a pre-4.0 message for filtering purposes, take into account possible
    // empty payload
    ...
    try (DataOutputBuffer out = new DataOutputBuffer(1024)) {
      // On a 4.0+ node, C* makes a distinction between "local" and "remote" batches, where only the
      // former can
      // be serialized and sent to a remote node, where they are deserialized and written to the
      // batch commitlog
      // without first being converted into mutation objects. Batch serialization is therfore not
      // symmetric, and
      // we use a special procedure here that "re-serializes" a "remote" batch to build the message.
      if (fromVersion >= MessagingService.VERSION_40
      ...
      if (messageOut.serializedSize(toVersion) != bytes.length)
        throw new AssertionError(
            String.format(
                "Message serializedSize(%s) does not match what was written with serialize(out, %s)"
                    + " for verb %s and serializer %s; expected %s, actual %s ",
                toVersion,
                toVersion,
                messageOut.verb(),
                Message.serializer.getClass(),
  ...
  /**
   * Only "local" batches can be passed through {@link Batch.Serializer#serialize(Batch,
   * DataOutputPlus, int)} and sent to a remote node during normal operation, but there are testing
   * scenarios where we may intercept and forward a "remote" batch. This method allows us to put the
   * already encoded mutations back onto a stream.
   */
  ...
  public static Message<?> deserializeMessage(IMessage message) {
    try (DataInputBuffer in = new DataInputBuffer(message.bytes())) {
      return Message.serializer.deserialize(
          in, toCassandraInetAddressAndPort(message.from()), message.version());
    } catch (Throwable t) {
      ...
      if (message.version() > MessagingService.current_version) {
        throw new IllegalStateException(
            String.format(
                ...
                this.config.num(), message.version(), MessagingService.current_version));
      }
      ...
      if (runOnCaller) {
        ...
        if (executor.isShutdown()) {
          ...
          inInstancelogger.warn(
              "Dropping message {} due to stage {} being shutdown", messageIn, header.verb.stage);
          return;
        }
        executor.execute(
  ...
  public int getMessagingVersion() {
    if (DatabaseDescriptor.isDaemonInitialized()) return MessagingService.current_version;
    else return 0;
  }
  ...
  public void setMessagingVersion(InetSocketAddress endpoint, int version) {
    if (DatabaseDescriptor.isDaemonInitialized())
      ...
      inInstancelogger.warn(
          "Skipped setting messaging version for {} to {} as daemon not initialized yet. Stacktrace"
  ...
  public void startup(ICluster cluster) {
    ...
    assert startedAt.compareAndSet(0L, System.nanoTime())
        : String.format(
            "startedAt on instance %d expected to be 0, but was %d",
            config().num(), startedAt.get());
              ...
              cluster.stream()
                  .forEach(
                      reportFromObj -> {
                        ...
                        if (reportFrom == reportTo || reportFrom.isShutdown()) return;

                        int reportFromVersion = reportFrom.getMessagingVersion();
                        if (reportFromVersion
                            == 0) // has not read configuration yet, no accessing messaging version
                        return;
                        // TODO: decide if we need to take care of the minversion
                        reportTo.setMessagingVersion(
                            reportFrom.broadcastAddress(), reportFromVersion);
                      });
            });
  }
  ...
  protected void partialStartup(ICluster<?> cluster)
      throws IOException,
          ...
          StartupException {
    // org.apache.cassandra.distributed.impl.AbstractCluster.startup sets the exception handler for
    // the thread
    // so extract it to populate ExecutorFactory.Global
    ExecutorFactory.Global.tryUnsafeSet(
        new ExecutorFactory.Default(
    ...
    if (!config.has(NETWORK)) {
      propagateMessagingVersions(
          cluster); // fake messaging needs to know messaging version for filters
    }
    ...
    // Re-populate token metadata after commit log recover (new peers might be loaded onto system
    // keyspace #10293)
    ...
    if (config.has(GOSSIP)) {
      ...
      if (ClusterMetadata.current().directory.peerState(self) != NodeState.JOINED && joinRing) {
                ...
                new UnsafeJoin(
                    ...
                    new HashSet<>(
                        BootStrapper.getBootstrapTokens(
                            ClusterMetadata.current(), FBUtilities.getBroadcastAddressAndPort())),
        ...
        if (config.has(BLANK_GOSSIP))
          ...
          peers.forEach(
              peer -> GossipHelper.statusToNormal((IInvokableInstance) peer).accept(this));
        else peers.forEach(peer -> GossipHelper.unsafeStatusToNormal(this, (IInstance) peer));
      }
    ...
    if (!FBUtilities.getBroadcastAddressAndPort()
            .getAddress()
            .equals(broadcastAddress().getAddress())
        || FBUtilities.getBroadcastAddressAndPort().getPort() != broadcastAddress().getPort())
      throw new IllegalStateException(
          String.format("%s != %s", FBUtilities.getBroadcastAddressAndPort(), broadcastAddress()));
    ...
    Future<?> future =
        async(
                (ExecutorService executor) -> {
                  ...
                  error =
                      parallelRun(error, executor, StorageService.instance::disableAutoCompaction);

                  // trigger init early or else it could try to init and touch a thread pool that
                  // got shutdown
                  ...
                  ThrowingRunnable shutdownBatchlogAndHints =
                      () -> {
                        ...
                        try {
                          // Batchlog manager can submit tasks to hints executor, so shut it down
                          // earlier and in sequence
                  ...
                  error =
                      parallelRun(
                          error, executor, () -> ClusterMetadataService.instance().log().close());
                  error =
                      parallelRun(
                          error,
                          executor,
                  ...
                  error =
                      parallelRun(
                          error,
                          executor,
                          // can only shutdown message once, so if the test shutsdown an instance,
                  ...
                  // CommitLog must shut down after Stage, or threads from the latter may attempt to
                  // use the former.
                  ...
                  // ScheduledExecutors shuts down after MessagingService, as MessagingService may
                  // issue tasks to it.
                  error =
                      parallelRun(
                          error,
                  ...
                  // Make sure any shutdown hooks registered for DeleteOnExit are released to
                  // prevent
  ...
  public int liveMemberCount() {
    ...
    return sync(() -> {
          if (!DatabaseDescriptor.isDaemonInitialized() || !Gossiper.instance.isEnabled()) return 0;
  ...
  private static Throwable parallelRun(
      Throwable accumulate, ExecutorService runOn, ThrowingRunnable... runnables) {
